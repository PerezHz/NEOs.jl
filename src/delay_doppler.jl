...
apophis_pv_197(et) = kmsec2auday( getpv(9904406, 0, constant_term(et)) ) # units: au, au/day
apophis_pv_199(et) = kmsec2auday( getpv(2099942, 0, constant_term(et)) ) # units: au, au/day
sun_pv(et) = kmsec2auday( getpv(10, 0, constant_term(et)) ) # units: au, au/day
earth_pv(et) = kmsec2auday( getpv(399, 0, constant_term(et)) ) # units: au, au/day
moon_pv(et) = kmsec2auday( getpv(301, 0, constant_term(et)) ) # units: au, au/day
...
function shapiro_delay(e, p, q)
    ...
    shap_del_days = 2μ[1]*log( (e+p+q+shap)/(e+p-q+shap) )/(c_au_per_day^3) # days
...
function delay_doppler(station_code::Int, et_r_secs::Real, F_tx::Real,
    ...
    doppler_c = (ρ_dot_t+ρ_dot_r)/c_au_per_day
    ...
    doppler_c2_t2 = μ[1]*( (1/r_ts) - (1/r_rs) ) # order c^-2, 2nd term
    doppler_c2_t3 = (  dot(v_t_t_t, v_t_t_t) - dot(v_r_t_r, v_r_t_r)  )/2  # order c^-2, 3rd term
    doppler_c2 = (doppler_c2_t1 + doppler_c2_t2 + doppler_c2_t3)/(c_au_per_day^2)
...
function delay_doppler3(station_code::Int, t_r_utc::DateTime, F_tx::Real,
        niter::Int=10; pm::Bool=true, tc::Real=3.0, xve::Function=earth_pv, xvs::Function=sun_pv, # `tc` not used here, but put there for compatibility
        xva::Function=apophis_pv_197)

    # Transform receiving time from UTC to TDB seconds since j2000
    et_r_secs = str2et(string(t_r_utc))
    # time count (microseconds)
    return delay_doppler3(station_code, et_r_secs, F_tx, niter; pm=pm, xve=xve, xvs=xvs, xva=xva)
end

function delay_doppler3(station_code::Int, et_r_secs::Real, F_tx::Real,
        niter::Int=10; pm::Bool=true, xve::Function=earth_pv, xvs::Function=sun_pv,
        xva::Function=apophis_pv_197)
    # determine the type of asteroid ephemeris
    ast1_et1 = xva(et_r_secs)[1]
    # S = typeof(ast1_et1)
    # t = Taylor1(S, 1)
    one_a = one(ast1_et1)
    # Compute geocentric position/velocity of receiving antenna in inertial frame (au, au/day)
    R_r_, V_r_ = observer_position(station_code, et_r_secs, pm=pm)
    R_r = map((x,y)->Taylor1([x*one_a, y*one_a]), R_r_, V_r_/daysec) #R_r_
    # Earth's barycentric position and velocity at receive time
    rv_e_t_r_ = xve(et_r_secs)
    r_e_t_r_ = rv_e_t_r_[1:3]
    v_e_t_r_ = rv_e_t_r_[4:6]/daysec
    r_e_t_r = map((x,y)->Taylor1([x*one_a, y*one_a]), r_e_t_r_, v_e_t_r_) #r_e_t_r_
    # Receiver barycentric position and velocity at receive time
    r_r_t_r = r_e_t_r + R_r
    # Asteroid barycentric position and velocity at receive time
    rv_a_t_r_ = xva(et_r_secs)
    r_a_t_r_ = rv_a_t_r_[1:3]
    v_a_t_r_ = rv_a_t_r_[4:6]/daysec
    # r_a_t_r = map((x,y)->Taylor1([x, y]), r_a_t_r_, v_a_t_r_)
    r_a_t_r = map((x,y)->Taylor1([x*one_a, y*one_a]), r_a_t_r_, v_a_t_r_) #r_a_t_r_
    # Sun barycentric position and velocity at receive time
    rv_s_t_r_ = xvs(et_r_secs)
    r_s_t_r_ = rv_s_t_r_[1:3]
    v_s_t_r_ = rv_s_t_r_[4:6]/daysec
    r_s_t_r = map((x,y)->Taylor1([x*one_a, y*one_a]), r_s_t_r_, v_s_t_r_) #r_s_t_r_
    # down-leg iteration
    # τ_D first approximation: Eq. (1) Yeomans et al. (1992)
    ρ_vec_r = r_a_t_r - r_r_t_r
    ρ_r = sqrt(ρ_vec_r[1]^2 + ρ_vec_r[2]^2 + ρ_vec_r[3]^2)
    ...
    et_b_secs = et_r_secs - τ_D
    # asteroid barycentric position (in au) at bounce time (TDB)
    rv_a_t_b_ = xva(et_b_secs)
    r_a_t_b_ = rv_a_t_b_[1:3]
    v_a_t_b_ = rv_a_t_b_[4:6]/daysec
    ...
    r_e_t_t_ = rv_e_t_t_[1:3]
    v_e_t_t_ = rv_e_t_t_[4:6]/daysec
    r_e_t_t = map((x,y)->Taylor1([x*one_a, y*one_a]), r_e_t_t_, v_e_t_t_) #r_e_t_t_
    # Transmitter barycentric position and velocity of at transmit time
    r_t_t_t = r_e_t_t + R_t
    # v_t_t_t = v_e_t_t + V_t
    # Eq. (7) Yeomans et al. (1992)
    ...
    # corrections to TT-TDB from Moyer (2003) / Folkner et al. (2014) due to position of measurement station on Earth are of order 0.01μs
    # Δtt_tdb_station_t = - dot(v_e_t_t/daysec, r_t_t_t-r_e_t_t)/c_au_per_sec^2
    tdb_utc_t_ = tdb_utc(constant_term(et_t_secs)) #+ Δtt_tdb_station_t
    dtt_tdb_t_ = -dtt_tdb(constant_term(et_t_secs))
    tdb_utc_t = Taylor1([tdb_utc_t_, dtt_tdb_t_]) # tdb_utc_t_
    # compute TDB-UTC at receive time
    # corrections to TT-TDB from Moyer (2003) / Folkner et al. (2014) due to position of measurement station on Earth  are of order 0.01μs
    # Δtt_tdb_station_r = dot(v_e_t_r/daysec, r_r_t_r-r_e_t_r)/c_au_per_sec^2
    tdb_utc_r_ = tdb_utc(et_r_secs) #+ Δtt_tdb_station_r
    dtt_tdb_r_ = -dtt_tdb(et_r_secs)
    tdb_utc_r = Taylor1([tdb_utc_r_, dtt_tdb_r_]) # tdb_utc_r_

    # compute total time delay (UTC seconds)
    # Eq. (9) Yeomans et al. (1992)
    τ = (τ_U + τ_D) + (tdb_utc_t - tdb_utc_r)

    return 1e6τ[0], -1e6F_tx*τ[1] # total signal delay (μs) and Doppler shift (Hz)
end
