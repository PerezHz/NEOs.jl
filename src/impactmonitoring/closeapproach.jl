...
taylor_order(x::CloseApproach{T, U}) where {T, U <: AbstractSeries} = taylor_order(x.t)
if isdefined(TaylorSeries, :order)
    TaylorSeries.order(x::CloseApproach{T, U}) where {T, U <: AbstractSeries} = taylor_order(x)
else
    TaylorSeries.get_order(x::CloseApproach{T, U}) where {T, U <: AbstractSeries} = taylor_order(x)
end
...
convergence_radius(x::AbstractSeries, ctol::Real) = (ctol / norm(x[end], Inf))^(1 / taylor_order(x))
