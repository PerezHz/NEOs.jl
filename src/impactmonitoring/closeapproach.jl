...
TaylorSeries.order(x::CloseApproach{T, U}) where {T, U <: AbstractSeries} = TaylorSeries.order(x.t)
...
convergence_radius(x::AbstractSeries, ctol::Real) = (ctol / norm(x[end], Inf))^(1 / TaylorSeries.order(x))
