...
Use least squares `method` to minimize the normalized mean square residual of `res`,
...
Newton method for minimizing the normalized mean square of a vector of optical residuals
`res`, starting from initial guess `x0`. If `idxs` (default: `eachindex(x0)`) is given,
perform the minimization over a subset of the parameters.
...
"""
function Newton(res::AbstractVector{OpticalResidual{T, TaylorN{T}}}, x0::Vector{T},
...
vector of optical residuals `res`, starting from initial guess `x0`. If `idxs`
(default: `eachindex(x0)`) is given, perform the minimization over a subset of
the parameters.
...
function DCVB(res::AbstractVector{OpticalResidual{T, TaylorN{T}}},
    ...
    for (j, k) in enumerate(findall(!isoutlier, res))
        ...
        V[2j-1] = sqrt(wra(res[k])) * ra(res[k])
        V[2j] = sqrt(wdec(res[k])) * dec(res[k])
...
vector of optical residuals `res`, starting from initial guess `x0`. If
`idxs` (default: `eachindex(x0)`) is given, perform the minimization over
...
function _lsmethods(res::AbstractVector{OpticalResidual{T, TaylorN{T}}}, x0::Vector{T},
...
- `maxiter::Int`: maximum number of iterations (default: `25`).
"""
function tryls(res::AbstractVector{OpticalResidual{T, TaylorN{T}}}, x0::Vector{T},
