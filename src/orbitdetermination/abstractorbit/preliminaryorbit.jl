...
variances(x::AbstractPreliminaryOrbit) = diag(covariance(x))
...
function GaussOrbit(
    ...
    return GaussOrbit{D, T, U, O}(dynamics, variables, optical, tracklets, bwd, fwd,
        ores, covariance, τ_1, τ_3, ρ_vec, R_vec, D_0, D_mat, a, b, c, r_2,
        r_vec, ρ)
end
...
function zero(::Type{GaussOrbit{D, T, U, O}}) where {D, T, U, O}
    ...
    return GaussOrbit(dynamics, variables, optical, tracklets, bwd, fwd, ores,
        covariance, τ_1, τ_3, ρ_vec, R_vec, D_0, D_mat, a, b, c, r_2,
        r_vec, ρ)
end
...
function evaldeltas(orbit::GaussOrbit{D, T, TaylorN{T}, O},
    ...
    return GaussOrbit(
        ...
        new_fwd, new_ores, orbit.covariance, orbit.τ_1, orbit.τ_3, new_ρ_vec,
        orbit.R_vec, new_D_0, new_D_mat, new_a, new_b, new_c, new_r_2, new_r_vec, new_ρ
    )
end
...
function evaldeltas(orbit::MMOVOrbit{D, T, TaylorN{T}, O},
    ...
    return MMOVOrbit(
        orbit.dynamics, orbit.variables, orbit.optical, orbit.tracklets, new_bwd,
        new_fwd, new_ores, orbit.covariance, orbit.aes, orbit.Qs
    )
end