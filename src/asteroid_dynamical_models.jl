...
#auxiliary function to evaluate pl.eph. case: Lyapunov spectrum
...
@taylorize function RNp1BP_pN_A_J23E_J2S_ng_eph!(dq, q, params, t)
    ...
    local dsj2k = t+(jd0-JD_J2000) # days since J2000.0 = 2.451545e6
    ...
    local zero_q_1 = auxzero(q[1]) # zero(q[1])
    ...
    # temp_accX_j = Array{S}(undef, N)
    # temp_accY_j = Array{S}(undef, N)
    # temp_accZ_j = Array{S}(undef, N)
