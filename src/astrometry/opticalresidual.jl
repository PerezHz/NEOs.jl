...
"""
    residuals(optical, w8s, bias [, outliers]; kwargs...) where {T <: Real}
...
function residuals!(res::Vector{OpticalResidual{T, U}},
    ...
    @allow_boxed_captures tmap!(res, optical, buffer, weight.(res), debias.(res),
                                isoutlier.(res)) do x, buff, w8s, bias, outlier
