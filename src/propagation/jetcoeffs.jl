# # DO NOT MODIFY THIS FILE BY HAND

# # Methods of `TaylorIntegration._allocate_jetcoeffs!` and `TaylorIntegration.jetcoeffs!` generated by @taylorize for the functions
# # in src/propagation/asteroid_dynamical_models.jl

# # To update the functions do the following:
# # 1.- Update the corresponding function in src/propagation/asteroid_dynamical_models.jl
# # 2.- using TaylorIntegration
# # 2.- ex = :(paste here the modified function)
# # 3.- x, y = TaylorIntegration._make_parsed_jetcoeffs(ex)
# # 4.- Paste x and y in this file

# # TaylorIntegration._allocate_jetcoeffs! method for src/propagation/asteroid_dynamical_models.jl: RNp1BP_pN_A_J23E_J2S_ng_eph_threads!
# function TaylorIntegration._allocate_jetcoeffs!(::Val{RNp1BP_pN_A_J23E_J2S_ng_eph_threads!}, t::Taylor1{_T}, q::AbstractArray{Taylor1{_S}, _N}, dq::AbstractArray{Taylor1{_S}, _N}, params) where {_T <: Real, _S <: Number, _N}
#     order = t.order
#     local jd0 = params.jd0
#     local dsj2k = t + (jd0 - JD_J2000)
#     local ss16asteph_t = evaleph(params.sseph, dsj2k, q[1])
#     local acceph_t = evaleph(params.acceph, dsj2k, q[1])
#     local newtonianNb_Potential_t = evaleph(params.poteph, dsj2k, q[1])
#     local S = eltype(q)
#     local UJ_interaction = params.UJ_interaction
#     local N = params.N
#     local Nm1 = N - 1
#     local μ = params.μ
#     local zero_q_1 = auxzero(q[1])
#     X = Array{S}(undef, N)
#     Y = Array{S}(undef, N)
#     Z = Array{S}(undef, N)
#     r_p2 = Array{S}(undef, N)
#     r_p1d2 = Array{S}(undef, N)
#     r_p3d2 = Array{S}(undef, N)
#     r_p7d2 = Array{S}(undef, N)
#     newtonianCoeff = Array{S}(undef, N)
#     ui = Array{S}(undef, N - 1)
#     vi = Array{S}(undef, N - 1)
#     wi = Array{S}(undef, N - 1)
#     U = Array{S}(undef, N)
#     V = Array{S}(undef, N)
#     W = Array{S}(undef, N)
#     _4dq = Array{S}(undef, 3)
#     _4U_m_3X = Array{S}(undef, N)
#     _4V_m_3Y = Array{S}(undef, N)
#     _4W_m_3Z = Array{S}(undef, N)
#     UU = Array{S}(undef, N)
#     VV = Array{S}(undef, N)
#     WW = Array{S}(undef, N)
#     newtonian1b_Potential = Array{S}(undef, N)
#     newtonianNb_Potential = Array{S}(undef, N)
#     newton_acc_X = Array{S}(undef, N)
#     newton_acc_Y = Array{S}(undef, N)
#     newton_acc_Z = Array{S}(undef, N)
#     v2 = Array{S}(undef, N)
#     vi_dot_vj = Array{S}(undef, N)
#     pn2 = Array{S}(undef, N)
#     U_t_pn2 = Array{S}(undef, N)
#     V_t_pn2 = Array{S}(undef, N)
#     W_t_pn2 = Array{S}(undef, N)
#     pn3 = Array{S}(undef, N)
#     pNX_t_pn3 = Array{S}(undef, N)
#     pNY_t_pn3 = Array{S}(undef, N)
#     pNZ_t_pn3 = Array{S}(undef, N)
#     _4ϕj = Array{S}(undef, N)
#     ϕi_plus_4ϕj = Array{S}(undef, N)
#     sj2_plus_2si2_minus_4vivj = Array{S}(undef, N)
#     ϕs_and_vs = Array{S}(undef, N)
#     pn1t1_7 = Array{S}(undef, N)
#     pNX_t_X = Array{S}(undef, N)
#     pNY_t_Y = Array{S}(undef, N)
#     pNZ_t_Z = Array{S}(undef, N)
#     pn1 = Array{S}(undef, N)
#     X_t_pn1 = Array{S}(undef, N)
#     Y_t_pn1 = Array{S}(undef, N)
#     Z_t_pn1 = Array{S}(undef, N)
#     pntempX = Taylor1(identity(constant_term(zero_q_1)), order)
#     pntempY = Taylor1(identity(constant_term(zero_q_1)), order)
#     pntempZ = Taylor1(identity(constant_term(zero_q_1)), order)
#     t31 = Array{S}(undef, N)
#     t32 = Array{S}(undef, N)
#     t33 = Array{S}(undef, N)
#     r_sin_ϕ = Array{S}(undef, N)
#     sin_ϕ = Array{S}(undef, N)
#     ϕ = Array{S}(undef, N)
#     cos_ϕ = Array{S}(undef, N)
#     sin2_ϕ = Array{S}(undef, N)
#     sin3_ϕ = Array{S}(undef, N)
#     sin4_ϕ = Array{S}(undef, N)
#     F_J2_x = Array{S}(undef, N)
#     F_J2_y = Array{S}(undef, N)
#     F_J2_z = Array{S}(undef, N)
#     F_J2_x1 = Array{S}(undef, N)
#     F_J2_y1 = Array{S}(undef, N)
#     F_J2_z1 = Array{S}(undef, N)
#     F_J2_x2 = Array{S}(undef, N)
#     F_J2_y2 = Array{S}(undef, N)
#     F_J2_z2 = Array{S}(undef, N)
#     temp_accX_i = Array{S}(undef, N)
#     temp_accY_i = Array{S}(undef, N)
#     temp_accZ_i = Array{S}(undef, N)
#     P_2_sin_ϕ = Array{S}(undef, N)
#     ∂P_2_sin_ϕ = Array{S}(undef, N)
#     P_3_sin_ϕ = Array{S}(undef, N)
#     ∂P_3_sin_ϕ = Array{S}(undef, N)
#     m_c_ϕ_∂P_2 = Array{S}(undef, N)
#     m_c_ϕ_∂P_3 = Array{S}(undef, N)
#     Λ2j_div_r4 = Array{S}(undef, N)
#     Λ3j_div_r5 = Array{S}(undef, N)
#     F_J_ξ = Array{S}(undef, N)
#     F_J_η = Array{S}(undef, N)
#     F_J_ζ = Array{S}(undef, N)
#     F_J2_ξ = Array{S}(undef, N)
#     F_J2_η = Array{S}(undef, N)
#     F_J2_ζ = Array{S}(undef, N)
#     F_J3_ξ = Array{S}(undef, N)
#     F_J3_η = Array{S}(undef, N)
#     F_J3_ζ = Array{S}(undef, N)
#     ξx = Array{S}(undef, N)
#     ξy = Array{S}(undef, N)
#     ξz = Array{S}(undef, N)
#     ηx = Array{S}(undef, N)
#     ηy = Array{S}(undef, N)
#     ηz = Array{S}(undef, N)
#     ηx1 = Array{S}(undef, N)
#     ηy1 = Array{S}(undef, N)
#     ηz1 = Array{S}(undef, N)
#     ηx2 = Array{S}(undef, N)
#     ηy2 = Array{S}(undef, N)
#     ηz2 = Array{S}(undef, N)
#     ζx = Array{S}(undef, N)
#     ζy = Array{S}(undef, N)
#     ζz = Array{S}(undef, N)
#     ζx1 = Array{S}(undef, N)
#     ζy1 = Array{S}(undef, N)
#     ζz1 = Array{S}(undef, N)
#     ζx2 = Array{S}(undef, N)
#     ζy2 = Array{S}(undef, N)
#     ζz2 = Array{S}(undef, N)
#     accX = Taylor1(identity(constant_term(zero_q_1)), order)
#     accY = Taylor1(identity(constant_term(zero_q_1)), order)
#     accZ = Taylor1(identity(constant_term(zero_q_1)), order)
#     local M_ = Array{S}(undef, 3, 3, N)
#     local M_[:, :, ea] = t2c_jpl_de430(dsj2k) .+ zero_q_1
#     dq[1] = Taylor1(identity(constant_term(q[4])), order)
#     dq[2] = Taylor1(identity(constant_term(q[5])), order)
#     dq[3] = Taylor1(identity(constant_term(q[6])), order)
#     newtonianNb_Potential[N] = Taylor1(identity(constant_term(zero_q_1)), order)
#     _4dq[1] = Taylor1(constant_term(4) * constant_term(dq[1]), order)
#     _4dq[2] = Taylor1(constant_term(4) * constant_term(dq[2]), order)
#     _4dq[3] = Taylor1(constant_term(4) * constant_term(dq[3]), order)
#     tmp625 = Array{Taylor1{_S}}(undef, size(ui))
#     for i = eachindex(tmp625)
#         tmp625[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp628 = Array{Taylor1{_S}}(undef, size(vi))
#     for i = eachindex(tmp628)
#         tmp628[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp631 = Array{Taylor1{_S}}(undef, size(wi))
#     for i = eachindex(tmp631)
#         tmp631[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn2x = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(pn2x)
#         pn2x[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn2y = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(pn2y)
#         pn2y[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn2z = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(pn2z)
#         pn2z[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp639 = Array{Taylor1{_S}}(undef, size(UU))
#     for i = eachindex(tmp639)
#         tmp639[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp642 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp642)
#         tmp642[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp880 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp880)
#         tmp880[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp644 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp644)
#         tmp644[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp881 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp881)
#         tmp881[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp645 = Array{Taylor1{_S}}(undef, size(tmp642))
#     for i = eachindex(tmp645)
#         tmp645[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp647 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp647)
#         tmp647[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp882 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp882)
#         tmp882[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp883 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp883)
#         tmp883[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp884 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp884)
#         tmp884[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp655 = Array{Taylor1{_S}}(undef, size(pn2x))
#     for i = eachindex(tmp655)
#         tmp655[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp656 = Array{Taylor1{_S}}(undef, size(tmp655))
#     for i = eachindex(tmp656)
#         tmp656[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp751 = Array{Taylor1{_S}}(undef, size(ui))
#     for i = eachindex(tmp751)
#         tmp751[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp891 = Array{Taylor1{_S}}(undef, size(ui))
#     for i = eachindex(tmp891)
#         tmp891[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp753 = Array{Taylor1{_S}}(undef, size(vi))
#     for i = eachindex(tmp753)
#         tmp753[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp892 = Array{Taylor1{_S}}(undef, size(vi))
#     for i = eachindex(tmp892)
#         tmp892[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp754 = Array{Taylor1{_S}}(undef, size(tmp751))
#     for i = eachindex(tmp754)
#         tmp754[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp756 = Array{Taylor1{_S}}(undef, size(wi))
#     for i = eachindex(tmp756)
#         tmp756[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp893 = Array{Taylor1{_S}}(undef, size(wi))
#     for i = eachindex(tmp893)
#         tmp893[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp667 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp667)
#         tmp667[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp669 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp669)
#         tmp669[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp671 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp671)
#         tmp671[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp673 = Array{Taylor1{_S}}(undef, size(t31))
#     for i = eachindex(tmp673)
#         tmp673[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp885 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp885)
#         tmp885[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp886 = Array{Taylor1{_S}}(undef, size(ϕ))
#     for i = eachindex(tmp886)
#         tmp886[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp887 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp887)
#         tmp887[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp888 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp888)
#         tmp888[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp683 = Array{Taylor1{_S}}(undef, size(sin2_ϕ))
#     for i = eachindex(tmp683)
#         tmp683[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp689 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp689)
#         tmp689[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp691 = Array{Taylor1{_S}}(undef, size(sin3_ϕ))
#     for i = eachindex(tmp691)
#         tmp691[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp695 = Array{Taylor1{_S}}(undef, size(sin2_ϕ))
#     for i = eachindex(tmp695)
#         tmp695[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp698 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp698)
#         tmp698[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp889 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp889)
#         tmp889[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp699 = Array{Taylor1{_S}}(undef, size(Λ2))
#     for i = eachindex(tmp699)
#         tmp699[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp702 = Array{Taylor1{_S}}(undef, size(r_p1d2))
#     for i = eachindex(tmp702)
#         tmp702[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp890 = Array{Taylor1{_S}}(undef, size(r_p1d2))
#     for i = eachindex(tmp890)
#         tmp890[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp703 = Array{Taylor1{_S}}(undef, size(Λ3))
#     for i = eachindex(tmp703)
#         tmp703[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp705 = Array{Taylor1{_S}}(undef, size(cos_ϕ))
#     for i = eachindex(tmp705)
#         tmp705[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp707 = Array{Taylor1{_S}}(undef, size(cos_ϕ))
#     for i = eachindex(tmp707)
#         tmp707[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp710 = Array{Taylor1{_S}}(undef, size(Λ2j_div_r4))
#     for i = eachindex(tmp710)
#         tmp710[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp714 = Array{Taylor1{_S}}(undef, size(Λ3j_div_r5))
#     for i = eachindex(tmp714)
#         tmp714[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp717 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp717)
#         tmp717[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp719 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp719)
#         tmp719[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp721 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp721)
#         tmp721[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:253 =# Threads.@threads for i = 1:Nm1
#             ui[i] = Taylor1(identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 2])), order)
#             vi[i] = Taylor1(identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 1])), order)
#             wi[i] = Taylor1(identity(constant_term(ss16asteph_t[3 * ((N - 1) + i)])), order)
#             X[i] = Taylor1(constant_term(ss16asteph_t[3i - 2]) - constant_term(q[1]), order)
#             Y[i] = Taylor1(constant_term(ss16asteph_t[3i - 1]) - constant_term(q[2]), order)
#             Z[i] = Taylor1(constant_term(ss16asteph_t[3i]) - constant_term(q[3]), order)
#             U[i] = Taylor1(constant_term(ui[i]) - constant_term(dq[1]), order)
#             V[i] = Taylor1(constant_term(vi[i]) - constant_term(dq[2]), order)
#             W[i] = Taylor1(constant_term(wi[i]) - constant_term(dq[3]), order)
#             tmp625[i] = Taylor1(constant_term(-3) * constant_term(ui[i]), order)
#             _4U_m_3X[i] = Taylor1(constant_term(tmp625[i]) + constant_term(_4dq[1]), order)
#             tmp628[i] = Taylor1(constant_term(-3) * constant_term(vi[i]), order)
#             _4V_m_3Y[i] = Taylor1(constant_term(tmp628[i]) + constant_term(_4dq[2]), order)
#             tmp631[i] = Taylor1(constant_term(-3) * constant_term(wi[i]), order)
#             _4W_m_3Z[i] = Taylor1(constant_term(tmp631[i]) + constant_term(_4dq[3]), order)
#             pn2x[i] = Taylor1(constant_term(X[i]) * constant_term(_4U_m_3X[i]), order)
#             pn2y[i] = Taylor1(constant_term(Y[i]) * constant_term(_4V_m_3Y[i]), order)
#             pn2z[i] = Taylor1(constant_term(Z[i]) * constant_term(_4W_m_3Z[i]), order)
#             UU[i] = Taylor1(constant_term(ui[i]) * constant_term(dq[1]), order)
#             VV[i] = Taylor1(constant_term(vi[i]) * constant_term(dq[2]), order)
#             WW[i] = Taylor1(constant_term(wi[i]) * constant_term(dq[3]), order)
#             tmp639[i] = Taylor1(constant_term(UU[i]) + constant_term(VV[i]), order)
#             vi_dot_vj[i] = Taylor1(constant_term(tmp639[i]) + constant_term(WW[i]), order)
#             tmp642[i] = Taylor1(constant_term(X[i]) ^ float(constant_term(2)), order)
#             tmp880[i] = Taylor1(zero(constant_term(X[i])), order)
#             tmp644[i] = Taylor1(constant_term(Y[i]) ^ float(constant_term(2)), order)
#             tmp881[i] = Taylor1(zero(constant_term(Y[i])), order)
#             tmp645[i] = Taylor1(constant_term(tmp642[i]) + constant_term(tmp644[i]), order)
#             tmp647[i] = Taylor1(constant_term(Z[i]) ^ float(constant_term(2)), order)
#             tmp882[i] = Taylor1(zero(constant_term(Z[i])), order)
#             r_p2[i] = Taylor1(constant_term(tmp645[i]) + constant_term(tmp647[i]), order)
#             r_p1d2[i] = Taylor1(sqrt(constant_term(r_p2[i])), order)
#             r_p3d2[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(1.5)), order)
#             tmp883[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#             r_p7d2[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(3.5)), order)
#             tmp884[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#             newtonianCoeff[i] = Taylor1(constant_term(μ[i]) / constant_term(r_p3d2[i]), order)
#             tmp655[i] = Taylor1(constant_term(pn2x[i]) + constant_term(pn2y[i]), order)
#             tmp656[i] = Taylor1(constant_term(tmp655[i]) + constant_term(pn2z[i]), order)
#             pn2[i] = Taylor1(constant_term(newtonianCoeff[i]) * constant_term(tmp656[i]), order)
#             newton_acc_X[i] = Taylor1(constant_term(X[i]) * constant_term(newtonianCoeff[i]), order)
#             newton_acc_Y[i] = Taylor1(constant_term(Y[i]) * constant_term(newtonianCoeff[i]), order)
#             newton_acc_Z[i] = Taylor1(constant_term(Z[i]) * constant_term(newtonianCoeff[i]), order)
#             newtonian1b_Potential[i] = Taylor1(constant_term(μ[i]) / constant_term(r_p1d2[i]), order)
#             pn3[i] = Taylor1(constant_term(3.5) * constant_term(newtonian1b_Potential[i]), order)
#             U_t_pn2[i] = Taylor1(constant_term(pn2[i]) * constant_term(U[i]), order)
#             V_t_pn2[i] = Taylor1(constant_term(pn2[i]) * constant_term(V[i]), order)
#             W_t_pn2[i] = Taylor1(constant_term(pn2[i]) * constant_term(W[i]), order)
#             if UJ_interaction[i]
#                 tmp667[i] = Taylor1(-(constant_term(X[i])), order)
#                 t31[i] = Taylor1(constant_term(tmp667[i]) * constant_term(M_[1, 3, i]), order)
#                 tmp669[i] = Taylor1(-(constant_term(Y[i])), order)
#                 t32[i] = Taylor1(constant_term(tmp669[i]) * constant_term(M_[2, 3, i]), order)
#                 tmp671[i] = Taylor1(-(constant_term(Z[i])), order)
#                 t33[i] = Taylor1(constant_term(tmp671[i]) * constant_term(M_[3, 3, i]), order)
#                 tmp673[i] = Taylor1(constant_term(t31[i]) + constant_term(t32[i]), order)
#                 r_sin_ϕ[i] = Taylor1(constant_term(tmp673[i]) + constant_term(t33[i]), order)
#                 sin_ϕ[i] = Taylor1(constant_term(r_sin_ϕ[i]) / constant_term(r_p1d2[i]), order)
#                 ϕ[i] = Taylor1(asin(constant_term(sin_ϕ[i])), order)
#                 tmp885[i] = Taylor1(sqrt(1 - constant_term(sin_ϕ[i]) ^ 2), order)
#                 cos_ϕ[i] = Taylor1(cos(constant_term(ϕ[i])), order)
#                 tmp886[i] = Taylor1(sin(constant_term(ϕ[i])), order)
#                 sin2_ϕ[i] = Taylor1(constant_term(sin_ϕ[i]) ^ float(constant_term(2)), order)
#                 tmp887[i] = Taylor1(zero(constant_term(sin_ϕ[i])), order)
#                 sin3_ϕ[i] = Taylor1(constant_term(sin_ϕ[i]) ^ float(constant_term(3)), order)
#                 tmp888[i] = Taylor1(zero(constant_term(sin_ϕ[i])), order)
#                 tmp683[i] = Taylor1(constant_term(1.5) * constant_term(sin2_ϕ[i]), order)
#                 P_2_sin_ϕ[i] = Taylor1(constant_term(tmp683[i]) - constant_term(0.5), order)
#                 ∂P_2_sin_ϕ[i] = Taylor1(constant_term(3) * constant_term(sin_ϕ[i]), order)
#                 tmp689[i] = Taylor1(constant_term(-1.5) * constant_term(sin_ϕ[i]), order)
#                 tmp691[i] = Taylor1(constant_term(2.5) * constant_term(sin3_ϕ[i]), order)
#                 P_3_sin_ϕ[i] = Taylor1(constant_term(tmp689[i]) + constant_term(tmp691[i]), order)
#                 tmp695[i] = Taylor1(constant_term(7.5) * constant_term(sin2_ϕ[i]), order)
#                 ∂P_3_sin_ϕ[i] = Taylor1(constant_term(-1.5) + constant_term(tmp695[i]), order)
#                 tmp698[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(2)), order)
#                 tmp889[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#                 tmp699[i] = Taylor1(constant_term(Λ2[i]) / constant_term(tmp698[i]), order)
#                 Λ2j_div_r4[i] = Taylor1(-(constant_term(tmp699[i])), order)
#                 tmp702[i] = Taylor1(constant_term(r_p1d2[i]) ^ float(constant_term(5)), order)
#                 tmp890[i] = Taylor1(zero(constant_term(r_p1d2[i])), order)
#                 tmp703[i] = Taylor1(constant_term(Λ3[i]) / constant_term(tmp702[i]), order)
#                 Λ3j_div_r5[i] = Taylor1(-(constant_term(tmp703[i])), order)
#                 tmp705[i] = Taylor1(-(constant_term(cos_ϕ[i])), order)
#                 m_c_ϕ_∂P_2[i] = Taylor1(constant_term(tmp705[i]) * constant_term(∂P_2_sin_ϕ[i]), order)
#                 tmp707[i] = Taylor1(-(constant_term(cos_ϕ[i])), order)
#                 m_c_ϕ_∂P_3[i] = Taylor1(constant_term(tmp707[i]) * constant_term(∂P_3_sin_ϕ[i]), order)
#                 tmp710[i] = Taylor1(constant_term(Λ2j_div_r4[i]) * constant_term(3), order)
#                 F_J2_ξ[i] = Taylor1(constant_term(tmp710[i]) * constant_term(P_2_sin_ϕ[i]), order)
#                 F_J2_ζ[i] = Taylor1(constant_term(Λ2j_div_r4[i]) * constant_term(m_c_ϕ_∂P_2[i]), order)
#                 tmp714[i] = Taylor1(constant_term(Λ3j_div_r5[i]) * constant_term(4), order)
#                 F_J3_ξ[i] = Taylor1(constant_term(tmp714[i]) * constant_term(P_3_sin_ϕ[i]), order)
#                 F_J3_ζ[i] = Taylor1(constant_term(Λ3j_div_r5[i]) * constant_term(m_c_ϕ_∂P_3[i]), order)
#                 F_J_ξ[i] = Taylor1(identity(constant_term(F_J2_ξ[i])), order)
#                 F_J_ζ[i] = Taylor1(identity(constant_term(F_J2_ζ[i])), order)
#                 tmp717[i] = Taylor1(-(constant_term(X[i])), order)
#                 ξx[i] = Taylor1(constant_term(tmp717[i]) / constant_term(r_p1d2[i]), order)
#                 tmp719[i] = Taylor1(-(constant_term(Y[i])), order)
#                 ξy[i] = Taylor1(constant_term(tmp719[i]) / constant_term(r_p1d2[i]), order)
#                 tmp721[i] = Taylor1(-(constant_term(Z[i])), order)
#                 ξz[i] = Taylor1(constant_term(tmp721[i]) / constant_term(r_p1d2[i]), order)
#                 ηx1[i] = Taylor1(constant_term(M_[2, 3, i]) * constant_term(ξz[i]), order)
#                 ηy1[i] = Taylor1(constant_term(M_[3, 3, i]) * constant_term(ξx[i]), order)
#                 ηz1[i] = Taylor1(constant_term(M_[1, 3, i]) * constant_term(ξy[i]), order)
#                 ηx2[i] = Taylor1(constant_term(M_[3, 3, i]) * constant_term(ξy[i]), order)
#                 ηy2[i] = Taylor1(constant_term(M_[1, 3, i]) * constant_term(ξz[i]), order)
#                 ηz2[i] = Taylor1(constant_term(M_[2, 3, i]) * constant_term(ξx[i]), order)
#                 ηx[i] = Taylor1(constant_term(ηx1[i]) - constant_term(ηx2[i]), order)
#                 ηy[i] = Taylor1(constant_term(ηy1[i]) - constant_term(ηy2[i]), order)
#                 ηz[i] = Taylor1(constant_term(ηz1[i]) - constant_term(ηz2[i]), order)
#                 ζx1[i] = Taylor1(constant_term(ξy[i]) * constant_term(ηz[i]), order)
#                 ζy1[i] = Taylor1(constant_term(ξz[i]) * constant_term(ηx[i]), order)
#                 ζz1[i] = Taylor1(constant_term(ξx[i]) * constant_term(ηy[i]), order)
#                 ζx2[i] = Taylor1(constant_term(ξz[i]) * constant_term(ηy[i]), order)
#                 ζy2[i] = Taylor1(constant_term(ξx[i]) * constant_term(ηz[i]), order)
#                 ζz2[i] = Taylor1(constant_term(ξy[i]) * constant_term(ηx[i]), order)
#                 ζx[i] = Taylor1(constant_term(ζx1[i]) - constant_term(ζx2[i]), order)
#                 ζy[i] = Taylor1(constant_term(ζy1[i]) - constant_term(ζy2[i]), order)
#                 ζz[i] = Taylor1(constant_term(ζz1[i]) - constant_term(ζz2[i]), order)
#                 F_J2_x1[i] = Taylor1(constant_term(F_J_ξ[i]) * constant_term(ξx[i]), order)
#                 F_J2_y1[i] = Taylor1(constant_term(F_J_ξ[i]) * constant_term(ξy[i]), order)
#                 F_J2_z1[i] = Taylor1(constant_term(F_J_ξ[i]) * constant_term(ξz[i]), order)
#                 F_J2_x2[i] = Taylor1(constant_term(F_J_ζ[i]) * constant_term(ζx[i]), order)
#                 F_J2_y2[i] = Taylor1(constant_term(F_J_ζ[i]) * constant_term(ζy[i]), order)
#                 F_J2_z2[i] = Taylor1(constant_term(F_J_ζ[i]) * constant_term(ζz[i]), order)
#                 F_J2_x[i] = Taylor1(constant_term(F_J2_x1[i]) + constant_term(F_J2_x2[i]), order)
#                 F_J2_y[i] = Taylor1(constant_term(F_J2_y1[i]) + constant_term(F_J2_y2[i]), order)
#                 F_J2_z[i] = Taylor1(constant_term(F_J2_z1[i]) + constant_term(F_J2_z2[i]), order)
#             end
#             tmp751[i] = Taylor1(constant_term(ui[i]) ^ float(constant_term(2)), order)
#             tmp891[i] = Taylor1(zero(constant_term(ui[i])), order)
#             tmp753[i] = Taylor1(constant_term(vi[i]) ^ float(constant_term(2)), order)
#             tmp892[i] = Taylor1(zero(constant_term(vi[i])), order)
#             tmp754[i] = Taylor1(constant_term(tmp751[i]) + constant_term(tmp753[i]), order)
#             tmp756[i] = Taylor1(constant_term(wi[i]) ^ float(constant_term(2)), order)
#             tmp893[i] = Taylor1(zero(constant_term(wi[i])), order)
#             v2[i] = Taylor1(constant_term(tmp754[i]) + constant_term(tmp756[i]), order)
#         end
#     tmp759 = Taylor1(constant_term(q[4]) ^ float(constant_term(2)), order)
#     tmp894 = Taylor1(zero(constant_term(q[4])), order)
#     tmp761 = Taylor1(constant_term(q[5]) ^ float(constant_term(2)), order)
#     tmp895 = Taylor1(zero(constant_term(q[5])), order)
#     tmp762 = Taylor1(constant_term(tmp759) + constant_term(tmp761), order)
#     tmp764 = Taylor1(constant_term(q[6]) ^ float(constant_term(2)), order)
#     tmp896 = Taylor1(zero(constant_term(q[6])), order)
#     v2[N] = Taylor1(constant_term(tmp762) + constant_term(tmp764), order)
#     temp_004 = Array{Taylor1{_S}}(undef, size(newtonian1b_Potential))
#     for i = eachindex(temp_004)
#         temp_004[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp767 = Array{Taylor1{_S}}(undef, size(μ))
#     for i = eachindex(tmp767)
#         tmp767[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp769 = Array{Taylor1{_S}}(undef, size(μ))
#     for i = eachindex(tmp769)
#         tmp769[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp771 = Array{Taylor1{_S}}(undef, size(μ))
#     for i = eachindex(tmp771)
#         tmp771[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     for i = 1:Nm1
#         temp_004[i] = Taylor1(constant_term(newtonian1b_Potential[i]) + constant_term(newtonianNb_Potential[N]), order)
#         newtonianNb_Potential[N] = Taylor1(identity(constant_term(temp_004[i])), order)
#         if UJ_interaction[i]
#             tmp767[i] = Taylor1(constant_term(μ[i]) * constant_term(F_J2_x[i]), order)
#             temp_accX_i[i] = Taylor1(constant_term(accX) - constant_term(tmp767[i]), order)
#             accX = Taylor1(identity(constant_term(temp_accX_i[i])), order)
#             tmp769[i] = Taylor1(constant_term(μ[i]) * constant_term(F_J2_y[i]), order)
#             temp_accY_i[i] = Taylor1(constant_term(accY) - constant_term(tmp769[i]), order)
#             accY = Taylor1(identity(constant_term(temp_accY_i[i])), order)
#             tmp771[i] = Taylor1(constant_term(μ[i]) * constant_term(F_J2_z[i]), order)
#             temp_accZ_i[i] = Taylor1(constant_term(accZ) - constant_term(tmp771[i]), order)
#             accZ = Taylor1(identity(constant_term(temp_accZ_i[i])), order)
#         end
#     end
#     _4ϕj[N] = Taylor1(constant_term(4) * constant_term(newtonianNb_Potential[N]), order)
#     tmp777 = Array{Taylor1{_S}}(undef, size(v2))
#     for i = eachindex(tmp777)
#         tmp777[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp779 = Array{Taylor1{_S}}(undef, size(vi_dot_vj))
#     for i = eachindex(tmp779)
#         tmp779[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp780 = Array{Taylor1{_S}}(undef, size(tmp777))
#     for i = eachindex(tmp780)
#         tmp780[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Xij_t_Ui = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(Xij_t_Ui)
#         Xij_t_Ui[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Yij_t_Vi = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(Yij_t_Vi)
#         Yij_t_Vi[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Zij_t_Wi = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(Zij_t_Wi)
#         Zij_t_Wi[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp786 = Array{Taylor1{_S}}(undef, size(Xij_t_Ui))
#     for i = eachindex(tmp786)
#         tmp786[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Rij_dot_Vi = Array{Taylor1{_S}}(undef, size(tmp786))
#     for i = eachindex(Rij_dot_Vi)
#         Rij_dot_Vi[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp789 = Array{Taylor1{_S}}(undef, size(Rij_dot_Vi))
#     for i = eachindex(tmp789)
#         tmp789[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp897 = Array{Taylor1{_S}}(undef, size(Rij_dot_Vi))
#     for i = eachindex(tmp897)
#         tmp897[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn1t7 = Array{Taylor1{_S}}(undef, size(tmp789))
#     for i = eachindex(pn1t7)
#         pn1t7[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp792 = Array{Taylor1{_S}}(undef, size(pn1t7))
#     for i = eachindex(tmp792)
#         tmp792[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn1t2_7 = Array{Taylor1{_S}}(undef, size(ϕs_and_vs))
#     for i = eachindex(pn1t2_7)
#         pn1t2_7[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp799 = Array{Taylor1{_S}}(undef, size(pNX_t_X))
#     for i = eachindex(tmp799)
#         tmp799[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp800 = Array{Taylor1{_S}}(undef, size(tmp799))
#     for i = eachindex(tmp800)
#         tmp800[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp801 = Array{Taylor1{_S}}(undef, size(tmp800))
#     for i = eachindex(tmp801)
#         tmp801[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:455 =# Threads.@threads for i = 1:10
#             ϕi_plus_4ϕj[i] = Taylor1(constant_term(newtonianNb_Potential_t[i]) + constant_term(_4ϕj[N]), order)
#             tmp777[i] = Taylor1(constant_term(2) * constant_term(v2[i]), order)
#             tmp779[i] = Taylor1(constant_term(4) * constant_term(vi_dot_vj[i]), order)
#             tmp780[i] = Taylor1(constant_term(tmp777[i]) - constant_term(tmp779[i]), order)
#             sj2_plus_2si2_minus_4vivj[i] = Taylor1(constant_term(tmp780[i]) + constant_term(v2[N]), order)
#             ϕs_and_vs[i] = Taylor1(constant_term(sj2_plus_2si2_minus_4vivj[i]) - constant_term(ϕi_plus_4ϕj[i]), order)
#             Xij_t_Ui[i] = Taylor1(constant_term(X[i]) * constant_term(ui[i]), order)
#             Yij_t_Vi[i] = Taylor1(constant_term(Y[i]) * constant_term(vi[i]), order)
#             Zij_t_Wi[i] = Taylor1(constant_term(Z[i]) * constant_term(wi[i]), order)
#             tmp786[i] = Taylor1(constant_term(Xij_t_Ui[i]) + constant_term(Yij_t_Vi[i]), order)
#             Rij_dot_Vi[i] = Taylor1(constant_term(tmp786[i]) + constant_term(Zij_t_Wi[i]), order)
#             tmp789[i] = Taylor1(constant_term(Rij_dot_Vi[i]) ^ float(constant_term(2)), order)
#             tmp897[i] = Taylor1(zero(constant_term(Rij_dot_Vi[i])), order)
#             pn1t7[i] = Taylor1(constant_term(tmp789[i]) / constant_term(r_p2[i]), order)
#             tmp792[i] = Taylor1(constant_term(1.5) * constant_term(pn1t7[i]), order)
#             pn1t2_7[i] = Taylor1(constant_term(ϕs_and_vs[i]) - constant_term(tmp792[i]), order)
#             pn1t1_7[i] = Taylor1(constant_term(c_p2) + constant_term(pn1t2_7[i]), order)
#             pNX_t_X[i] = Taylor1(constant_term(acceph_t[3i - 2]) * constant_term(X[i]), order)
#             pNY_t_Y[i] = Taylor1(constant_term(acceph_t[3i - 1]) * constant_term(Y[i]), order)
#             pNZ_t_Z[i] = Taylor1(constant_term(acceph_t[3i]) * constant_term(Z[i]), order)
#             tmp799[i] = Taylor1(constant_term(pNX_t_X[i]) + constant_term(pNY_t_Y[i]), order)
#             tmp800[i] = Taylor1(constant_term(tmp799[i]) + constant_term(pNZ_t_Z[i]), order)
#             tmp801[i] = Taylor1(constant_term(0.5) * constant_term(tmp800[i]), order)
#             pn1[i] = Taylor1(constant_term(pn1t1_7[i]) + constant_term(tmp801[i]), order)
#             X_t_pn1[i] = Taylor1(constant_term(newton_acc_X[i]) * constant_term(pn1[i]), order)
#             Y_t_pn1[i] = Taylor1(constant_term(newton_acc_Y[i]) * constant_term(pn1[i]), order)
#             Z_t_pn1[i] = Taylor1(constant_term(newton_acc_Z[i]) * constant_term(pn1[i]), order)
#             pNX_t_pn3[i] = Taylor1(constant_term(acceph_t[3i - 2]) * constant_term(pn3[i]), order)
#             pNY_t_pn3[i] = Taylor1(constant_term(acceph_t[3i - 1]) * constant_term(pn3[i]), order)
#             pNZ_t_pn3[i] = Taylor1(constant_term(acceph_t[3i]) * constant_term(pn3[i]), order)
#         end
#     tmp809 = Array{Taylor1{_S}}(undef, size(U_t_pn2))
#     for i = eachindex(tmp809)
#         tmp809[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     termpnx = Array{Taylor1{_S}}(undef, size(X_t_pn1))
#     for i = eachindex(termpnx)
#         termpnx[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     sumpnx = Array{Taylor1{_S}}(undef, size(termpnx))
#     for i = eachindex(sumpnx)
#         sumpnx[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp812 = Array{Taylor1{_S}}(undef, size(V_t_pn2))
#     for i = eachindex(tmp812)
#         tmp812[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     termpny = Array{Taylor1{_S}}(undef, size(Y_t_pn1))
#     for i = eachindex(termpny)
#         termpny[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     sumpny = Array{Taylor1{_S}}(undef, size(termpny))
#     for i = eachindex(sumpny)
#         sumpny[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp815 = Array{Taylor1{_S}}(undef, size(W_t_pn2))
#     for i = eachindex(tmp815)
#         tmp815[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     termpnz = Array{Taylor1{_S}}(undef, size(Z_t_pn1))
#     for i = eachindex(termpnz)
#         termpnz[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     sumpnz = Array{Taylor1{_S}}(undef, size(termpnz))
#     for i = eachindex(sumpnz)
#         sumpnz[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     for i = 1:10
#         tmp809[i] = Taylor1(constant_term(U_t_pn2[i]) + constant_term(pNX_t_pn3[i]), order)
#         termpnx[i] = Taylor1(constant_term(X_t_pn1[i]) + constant_term(tmp809[i]), order)
#         sumpnx[i] = Taylor1(constant_term(pntempX) + constant_term(termpnx[i]), order)
#         pntempX = Taylor1(identity(constant_term(sumpnx[i])), order)
#         tmp812[i] = Taylor1(constant_term(V_t_pn2[i]) + constant_term(pNY_t_pn3[i]), order)
#         termpny[i] = Taylor1(constant_term(Y_t_pn1[i]) + constant_term(tmp812[i]), order)
#         sumpny[i] = Taylor1(constant_term(pntempY) + constant_term(termpny[i]), order)
#         pntempY = Taylor1(identity(constant_term(sumpny[i])), order)
#         tmp815[i] = Taylor1(constant_term(W_t_pn2[i]) + constant_term(pNZ_t_pn3[i]), order)
#         termpnz[i] = Taylor1(constant_term(Z_t_pn1[i]) + constant_term(tmp815[i]), order)
#         sumpnz[i] = Taylor1(constant_term(pntempZ) + constant_term(termpnz[i]), order)
#         pntempZ = Taylor1(identity(constant_term(sumpnz[i])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:508 =# Threads.@threads for i = 11:Nm1
#             X_t_pn1[i] = Taylor1(constant_term(c_p2) * constant_term(newton_acc_X[i]), order)
#             Y_t_pn1[i] = Taylor1(constant_term(c_p2) * constant_term(newton_acc_Y[i]), order)
#             Z_t_pn1[i] = Taylor1(constant_term(c_p2) * constant_term(newton_acc_Z[i]), order)
#         end
#     for i = 11:Nm1
#         termpnx[i] = Taylor1(identity(constant_term(X_t_pn1[i])), order)
#         sumpnx[i] = Taylor1(constant_term(pntempX) + constant_term(termpnx[i]), order)
#         pntempX = Taylor1(identity(constant_term(sumpnx[i])), order)
#         termpny[i] = Taylor1(identity(constant_term(Y_t_pn1[i])), order)
#         sumpny[i] = Taylor1(constant_term(pntempY) + constant_term(termpny[i]), order)
#         pntempY = Taylor1(identity(constant_term(sumpny[i])), order)
#         termpnz[i] = Taylor1(identity(constant_term(Z_t_pn1[i])), order)
#         sumpnz[i] = Taylor1(constant_term(pntempZ) + constant_term(termpnz[i]), order)
#         pntempZ = Taylor1(identity(constant_term(sumpnz[i])), order)
#     end
#     postNewtonX = Taylor1(constant_term(pntempX) * constant_term(c_m2), order)
#     postNewtonY = Taylor1(constant_term(pntempY) * constant_term(c_m2), order)
#     postNewtonZ = Taylor1(constant_term(pntempZ) * constant_term(c_m2), order)
#     tmp827 = Taylor1(constant_term(Y[1]) * constant_term(W[1]), order)
#     tmp828 = Taylor1(constant_term(Z[1]) * constant_term(V[1]), order)
#     hx = Taylor1(constant_term(tmp827) - constant_term(tmp828), order)
#     tmp830 = Taylor1(constant_term(Z[1]) * constant_term(U[1]), order)
#     tmp831 = Taylor1(constant_term(X[1]) * constant_term(W[1]), order)
#     hy = Taylor1(constant_term(tmp830) - constant_term(tmp831), order)
#     tmp833 = Taylor1(constant_term(X[1]) * constant_term(V[1]), order)
#     tmp834 = Taylor1(constant_term(Y[1]) * constant_term(U[1]), order)
#     hz = Taylor1(constant_term(tmp833) - constant_term(tmp834), order)
#     tmp836 = Taylor1(constant_term(hz) * constant_term(Y[1]), order)
#     tmp837 = Taylor1(constant_term(hy) * constant_term(Z[1]), order)
#     t_x = Taylor1(constant_term(tmp836) - constant_term(tmp837), order)
#     tmp839 = Taylor1(constant_term(hx) * constant_term(Z[1]), order)
#     tmp840 = Taylor1(constant_term(hz) * constant_term(X[1]), order)
#     t_y = Taylor1(constant_term(tmp839) - constant_term(tmp840), order)
#     tmp842 = Taylor1(constant_term(hy) * constant_term(X[1]), order)
#     tmp843 = Taylor1(constant_term(hx) * constant_term(Y[1]), order)
#     t_z = Taylor1(constant_term(tmp842) - constant_term(tmp843), order)
#     tmp846 = Taylor1(constant_term(t_x) ^ float(constant_term(2)), order)
#     tmp898 = Taylor1(zero(constant_term(t_x)), order)
#     tmp848 = Taylor1(constant_term(t_y) ^ float(constant_term(2)), order)
#     tmp899 = Taylor1(zero(constant_term(t_y)), order)
#     tmp849 = Taylor1(constant_term(tmp846) + constant_term(tmp848), order)
#     tmp851 = Taylor1(constant_term(t_z) ^ float(constant_term(2)), order)
#     tmp900 = Taylor1(zero(constant_term(t_z)), order)
#     tmp852 = Taylor1(constant_term(tmp849) + constant_term(tmp851), order)
#     t_norm = Taylor1(sqrt(constant_term(tmp852)), order)
#     t_x_unit = Taylor1(constant_term(t_x) / constant_term(t_norm), order)
#     t_y_unit = Taylor1(constant_term(t_y) / constant_term(t_norm), order)
#     t_z_unit = Taylor1(constant_term(t_z) / constant_term(t_norm), order)
#     tmp857 = Taylor1(constant_term(X[1]) / constant_term(r_p1d2[1]), order)
#     r_x_unit = Taylor1(-(constant_term(tmp857)), order)
#     tmp859 = Taylor1(constant_term(Y[1]) / constant_term(r_p1d2[1]), order)
#     r_y_unit = Taylor1(-(constant_term(tmp859)), order)
#     tmp861 = Taylor1(constant_term(Z[1]) / constant_term(r_p1d2[1]), order)
#     r_z_unit = Taylor1(-(constant_term(tmp861)), order)
#     g_r = Taylor1(identity(constant_term(r_p2[1])), order)
#     A2_t_g_r = Taylor1(constant_term(q[7]) / constant_term(g_r), order)
#     A1_t_g_r = Taylor1(constant_term(q[8]) / constant_term(g_r), order)
#     tmp865 = Taylor1(constant_term(A2_t_g_r) * constant_term(t_x_unit), order)
#     tmp866 = Taylor1(constant_term(A1_t_g_r) * constant_term(r_x_unit), order)
#     NGAx = Taylor1(constant_term(tmp865) + constant_term(tmp866), order)
#     tmp868 = Taylor1(constant_term(A2_t_g_r) * constant_term(t_y_unit), order)
#     tmp869 = Taylor1(constant_term(A1_t_g_r) * constant_term(r_y_unit), order)
#     NGAy = Taylor1(constant_term(tmp868) + constant_term(tmp869), order)
#     tmp871 = Taylor1(constant_term(A2_t_g_r) * constant_term(t_z_unit), order)
#     tmp872 = Taylor1(constant_term(A1_t_g_r) * constant_term(r_z_unit), order)
#     NGAz = Taylor1(constant_term(tmp871) + constant_term(tmp872), order)
#     tmp874 = Taylor1(constant_term(postNewtonX) + constant_term(accX), order)
#     dq[4] = Taylor1(constant_term(tmp874) + constant_term(NGAx), order)
#     tmp876 = Taylor1(constant_term(postNewtonY) + constant_term(accY), order)
#     dq[5] = Taylor1(constant_term(tmp876) + constant_term(NGAy), order)
#     tmp878 = Taylor1(constant_term(postNewtonZ) + constant_term(accZ), order)
#     dq[6] = Taylor1(constant_term(tmp878) + constant_term(NGAz), order)
#     dq[7] = Taylor1(identity(constant_term(zero_q_1)), order)
#     dq[8] = Taylor1(identity(constant_term(zero_q_1)), order)
#     return TaylorIntegration.RetAlloc{Taylor1{_S}}([tmp759, tmp761, tmp762, tmp764, tmp827, tmp828, tmp830, tmp831, tmp833, tmp834, tmp836, tmp837, tmp839, tmp840, tmp842, tmp843, tmp846, tmp848, tmp849, tmp851, tmp852, tmp857, tmp859, tmp861, tmp865, tmp866, tmp868, tmp869, tmp871, tmp872, tmp874, tmp876, tmp878, pntempX, pntempY, pntempZ, accX, accY, accZ, tmp894, tmp895, tmp896, postNewtonX, postNewtonY, postNewtonZ, hx, hy, hz, t_x, t_y, t_z, tmp898, tmp899, tmp900, t_norm, t_x_unit, t_y_unit, t_z_unit, r_x_unit, r_y_unit, r_z_unit, g_r, A2_t_g_r, A1_t_g_r, NGAx, NGAy, NGAz], [X, Y, Z, r_p2, r_p1d2, r_p3d2, r_p7d2, newtonianCoeff, ui, vi, wi, U, V, W, _4dq, _4U_m_3X, _4V_m_3Y, _4W_m_3Z, UU, VV, WW, newtonian1b_Potential, newtonianNb_Potential, newton_acc_X, newton_acc_Y, newton_acc_Z, v2, vi_dot_vj, pn2, U_t_pn2, V_t_pn2, W_t_pn2, pn3, pNX_t_pn3, pNY_t_pn3, pNZ_t_pn3, _4ϕj, ϕi_plus_4ϕj, sj2_plus_2si2_minus_4vivj, ϕs_and_vs, pn1t1_7, pNX_t_X, pNY_t_Y, pNZ_t_Z, pn1, X_t_pn1, Y_t_pn1, Z_t_pn1, t31, t32, t33, r_sin_ϕ, sin_ϕ, ϕ, cos_ϕ, sin2_ϕ, sin3_ϕ, F_J2_x, F_J2_y, F_J2_z, F_J2_x1, F_J2_y1, F_J2_z1, F_J2_x2, F_J2_y2, F_J2_z2, temp_accX_i, temp_accY_i, temp_accZ_i, P_2_sin_ϕ, ∂P_2_sin_ϕ, P_3_sin_ϕ, ∂P_3_sin_ϕ, m_c_ϕ_∂P_2, m_c_ϕ_∂P_3, Λ2j_div_r4, Λ3j_div_r5, F_J_ξ, F_J_ζ, F_J2_ξ, F_J2_ζ, F_J3_ξ, F_J3_ζ, ξx, ξy, ξz, ηx, ηy, ηz, ηx1, ηy1, ηz1, ηx2, ηy2, ηz2, ζx, ζy, ζz, ζx1, ζy1, ζz1, ζx2, ζy2, ζz2, tmp625, tmp628, tmp631, pn2x, pn2y, pn2z, tmp639, tmp642, tmp880, tmp644, tmp881, tmp645, tmp647, tmp882, tmp883, tmp884, tmp655, tmp656, tmp667, tmp669, tmp671, tmp673, tmp885, tmp886, tmp887, tmp888, tmp683, tmp689, tmp691, tmp695, tmp698, tmp889, tmp699, tmp702, tmp890, tmp703, tmp705, tmp707, tmp710, tmp714, tmp717, tmp719, tmp721, tmp751, tmp891, tmp753, tmp892, tmp754, tmp756, tmp893, temp_004, tmp767, tmp769, tmp771, tmp777, tmp779, tmp780, Xij_t_Ui, Yij_t_Vi, Zij_t_Wi, tmp786, Rij_dot_Vi, tmp789, tmp897, pn1t7, tmp792, pn1t2_7, tmp799, tmp800, tmp801, tmp809, termpnx, sumpnx, tmp812, termpny, sumpny, tmp815, termpnz, sumpnz], [Array{Taylor1{_S}, 2}(undef, 0, 0)], [Array{Taylor1{_S}, 3}(undef, 0, 0, 0)], [Array{Taylor1{_S}, 4}(undef, 0, 0, 0, 0)])
# end

# # TaylorIntegration.jetcoeffs! method for src/propagation/asteroid_dynamical_models.jl: RNp1BP_pN_A_J23E_J2S_ng_eph_threads!
# function TaylorIntegration.jetcoeffs!(::Val{RNp1BP_pN_A_J23E_J2S_ng_eph_threads!}, t::Taylor1{_T}, q::AbstractArray{Taylor1{_S}, _N}, dq::AbstractArray{Taylor1{_S}, _N}, params, __ralloc::TaylorIntegration.RetAlloc{Taylor1{_S}}) where {_T <: Real, _S <: Number, _N}
#     order = t.order
#     tmp759 = __ralloc.v0[1]
#     tmp761 = __ralloc.v0[2]
#     tmp762 = __ralloc.v0[3]
#     tmp764 = __ralloc.v0[4]
#     tmp827 = __ralloc.v0[5]
#     tmp828 = __ralloc.v0[6]
#     tmp830 = __ralloc.v0[7]
#     tmp831 = __ralloc.v0[8]
#     tmp833 = __ralloc.v0[9]
#     tmp834 = __ralloc.v0[10]
#     tmp836 = __ralloc.v0[11]
#     tmp837 = __ralloc.v0[12]
#     tmp839 = __ralloc.v0[13]
#     tmp840 = __ralloc.v0[14]
#     tmp842 = __ralloc.v0[15]
#     tmp843 = __ralloc.v0[16]
#     tmp846 = __ralloc.v0[17]
#     tmp848 = __ralloc.v0[18]
#     tmp849 = __ralloc.v0[19]
#     tmp851 = __ralloc.v0[20]
#     tmp852 = __ralloc.v0[21]
#     tmp857 = __ralloc.v0[22]
#     tmp859 = __ralloc.v0[23]
#     tmp861 = __ralloc.v0[24]
#     tmp865 = __ralloc.v0[25]
#     tmp866 = __ralloc.v0[26]
#     tmp868 = __ralloc.v0[27]
#     tmp869 = __ralloc.v0[28]
#     tmp871 = __ralloc.v0[29]
#     tmp872 = __ralloc.v0[30]
#     tmp874 = __ralloc.v0[31]
#     tmp876 = __ralloc.v0[32]
#     tmp878 = __ralloc.v0[33]
#     pntempX = __ralloc.v0[34]
#     pntempY = __ralloc.v0[35]
#     pntempZ = __ralloc.v0[36]
#     accX = __ralloc.v0[37]
#     accY = __ralloc.v0[38]
#     accZ = __ralloc.v0[39]
#     tmp894 = __ralloc.v0[40]
#     tmp895 = __ralloc.v0[41]
#     tmp896 = __ralloc.v0[42]
#     postNewtonX = __ralloc.v0[43]
#     postNewtonY = __ralloc.v0[44]
#     postNewtonZ = __ralloc.v0[45]
#     hx = __ralloc.v0[46]
#     hy = __ralloc.v0[47]
#     hz = __ralloc.v0[48]
#     t_x = __ralloc.v0[49]
#     t_y = __ralloc.v0[50]
#     t_z = __ralloc.v0[51]
#     tmp898 = __ralloc.v0[52]
#     tmp899 = __ralloc.v0[53]
#     tmp900 = __ralloc.v0[54]
#     t_norm = __ralloc.v0[55]
#     t_x_unit = __ralloc.v0[56]
#     t_y_unit = __ralloc.v0[57]
#     t_z_unit = __ralloc.v0[58]
#     r_x_unit = __ralloc.v0[59]
#     r_y_unit = __ralloc.v0[60]
#     r_z_unit = __ralloc.v0[61]
#     g_r = __ralloc.v0[62]
#     A2_t_g_r = __ralloc.v0[63]
#     A1_t_g_r = __ralloc.v0[64]
#     NGAx = __ralloc.v0[65]
#     NGAy = __ralloc.v0[66]
#     NGAz = __ralloc.v0[67]
#     X = __ralloc.v1[1]
#     Y = __ralloc.v1[2]
#     Z = __ralloc.v1[3]
#     r_p2 = __ralloc.v1[4]
#     r_p1d2 = __ralloc.v1[5]
#     r_p3d2 = __ralloc.v1[6]
#     r_p7d2 = __ralloc.v1[7]
#     newtonianCoeff = __ralloc.v1[8]
#     ui = __ralloc.v1[9]
#     vi = __ralloc.v1[10]
#     wi = __ralloc.v1[11]
#     U = __ralloc.v1[12]
#     V = __ralloc.v1[13]
#     W = __ralloc.v1[14]
#     _4dq = __ralloc.v1[15]
#     _4U_m_3X = __ralloc.v1[16]
#     _4V_m_3Y = __ralloc.v1[17]
#     _4W_m_3Z = __ralloc.v1[18]
#     UU = __ralloc.v1[19]
#     VV = __ralloc.v1[20]
#     WW = __ralloc.v1[21]
#     newtonian1b_Potential = __ralloc.v1[22]
#     newtonianNb_Potential = __ralloc.v1[23]
#     newton_acc_X = __ralloc.v1[24]
#     newton_acc_Y = __ralloc.v1[25]
#     newton_acc_Z = __ralloc.v1[26]
#     v2 = __ralloc.v1[27]
#     vi_dot_vj = __ralloc.v1[28]
#     pn2 = __ralloc.v1[29]
#     U_t_pn2 = __ralloc.v1[30]
#     V_t_pn2 = __ralloc.v1[31]
#     W_t_pn2 = __ralloc.v1[32]
#     pn3 = __ralloc.v1[33]
#     pNX_t_pn3 = __ralloc.v1[34]
#     pNY_t_pn3 = __ralloc.v1[35]
#     pNZ_t_pn3 = __ralloc.v1[36]
#     _4ϕj = __ralloc.v1[37]
#     ϕi_plus_4ϕj = __ralloc.v1[38]
#     sj2_plus_2si2_minus_4vivj = __ralloc.v1[39]
#     ϕs_and_vs = __ralloc.v1[40]
#     pn1t1_7 = __ralloc.v1[41]
#     pNX_t_X = __ralloc.v1[42]
#     pNY_t_Y = __ralloc.v1[43]
#     pNZ_t_Z = __ralloc.v1[44]
#     pn1 = __ralloc.v1[45]
#     X_t_pn1 = __ralloc.v1[46]
#     Y_t_pn1 = __ralloc.v1[47]
#     Z_t_pn1 = __ralloc.v1[48]
#     t31 = __ralloc.v1[49]
#     t32 = __ralloc.v1[50]
#     t33 = __ralloc.v1[51]
#     r_sin_ϕ = __ralloc.v1[52]
#     sin_ϕ = __ralloc.v1[53]
#     ϕ = __ralloc.v1[54]
#     cos_ϕ = __ralloc.v1[55]
#     sin2_ϕ = __ralloc.v1[56]
#     sin3_ϕ = __ralloc.v1[57]
#     F_J2_x = __ralloc.v1[58]
#     F_J2_y = __ralloc.v1[59]
#     F_J2_z = __ralloc.v1[60]
#     F_J2_x1 = __ralloc.v1[61]
#     F_J2_y1 = __ralloc.v1[62]
#     F_J2_z1 = __ralloc.v1[63]
#     F_J2_x2 = __ralloc.v1[64]
#     F_J2_y2 = __ralloc.v1[65]
#     F_J2_z2 = __ralloc.v1[66]
#     temp_accX_i = __ralloc.v1[67]
#     temp_accY_i = __ralloc.v1[68]
#     temp_accZ_i = __ralloc.v1[69]
#     P_2_sin_ϕ = __ralloc.v1[70]
#     ∂P_2_sin_ϕ = __ralloc.v1[71]
#     P_3_sin_ϕ = __ralloc.v1[72]
#     ∂P_3_sin_ϕ = __ralloc.v1[73]
#     m_c_ϕ_∂P_2 = __ralloc.v1[74]
#     m_c_ϕ_∂P_3 = __ralloc.v1[75]
#     Λ2j_div_r4 = __ralloc.v1[76]
#     Λ3j_div_r5 = __ralloc.v1[77]
#     F_J_ξ = __ralloc.v1[78]
#     F_J_ζ = __ralloc.v1[79]
#     F_J2_ξ = __ralloc.v1[80]
#     F_J2_ζ = __ralloc.v1[81]
#     F_J3_ξ = __ralloc.v1[82]
#     F_J3_ζ = __ralloc.v1[83]
#     ξx = __ralloc.v1[84]
#     ξy = __ralloc.v1[85]
#     ξz = __ralloc.v1[86]
#     ηx = __ralloc.v1[87]
#     ηy = __ralloc.v1[88]
#     ηz = __ralloc.v1[89]
#     ηx1 = __ralloc.v1[90]
#     ηy1 = __ralloc.v1[91]
#     ηz1 = __ralloc.v1[92]
#     ηx2 = __ralloc.v1[93]
#     ηy2 = __ralloc.v1[94]
#     ηz2 = __ralloc.v1[95]
#     ζx = __ralloc.v1[96]
#     ζy = __ralloc.v1[97]
#     ζz = __ralloc.v1[98]
#     ζx1 = __ralloc.v1[99]
#     ζy1 = __ralloc.v1[100]
#     ζz1 = __ralloc.v1[101]
#     ζx2 = __ralloc.v1[102]
#     ζy2 = __ralloc.v1[103]
#     ζz2 = __ralloc.v1[104]
#     tmp625 = __ralloc.v1[105]
#     tmp628 = __ralloc.v1[106]
#     tmp631 = __ralloc.v1[107]
#     pn2x = __ralloc.v1[108]
#     pn2y = __ralloc.v1[109]
#     pn2z = __ralloc.v1[110]
#     tmp639 = __ralloc.v1[111]
#     tmp642 = __ralloc.v1[112]
#     tmp880 = __ralloc.v1[113]
#     tmp644 = __ralloc.v1[114]
#     tmp881 = __ralloc.v1[115]
#     tmp645 = __ralloc.v1[116]
#     tmp647 = __ralloc.v1[117]
#     tmp882 = __ralloc.v1[118]
#     tmp883 = __ralloc.v1[119]
#     tmp884 = __ralloc.v1[120]
#     tmp655 = __ralloc.v1[121]
#     tmp656 = __ralloc.v1[122]
#     tmp667 = __ralloc.v1[123]
#     tmp669 = __ralloc.v1[124]
#     tmp671 = __ralloc.v1[125]
#     tmp673 = __ralloc.v1[126]
#     tmp885 = __ralloc.v1[127]
#     tmp886 = __ralloc.v1[128]
#     tmp887 = __ralloc.v1[129]
#     tmp888 = __ralloc.v1[130]
#     tmp683 = __ralloc.v1[131]
#     tmp689 = __ralloc.v1[132]
#     tmp691 = __ralloc.v1[133]
#     tmp695 = __ralloc.v1[134]
#     tmp698 = __ralloc.v1[135]
#     tmp889 = __ralloc.v1[136]
#     tmp699 = __ralloc.v1[137]
#     tmp702 = __ralloc.v1[138]
#     tmp890 = __ralloc.v1[139]
#     tmp703 = __ralloc.v1[140]
#     tmp705 = __ralloc.v1[141]
#     tmp707 = __ralloc.v1[142]
#     tmp710 = __ralloc.v1[143]
#     tmp714 = __ralloc.v1[144]
#     tmp717 = __ralloc.v1[145]
#     tmp719 = __ralloc.v1[146]
#     tmp721 = __ralloc.v1[147]
#     tmp751 = __ralloc.v1[148]
#     tmp891 = __ralloc.v1[149]
#     tmp753 = __ralloc.v1[150]
#     tmp892 = __ralloc.v1[151]
#     tmp754 = __ralloc.v1[152]
#     tmp756 = __ralloc.v1[153]
#     tmp893 = __ralloc.v1[154]
#     temp_004 = __ralloc.v1[155]
#     tmp767 = __ralloc.v1[156]
#     tmp769 = __ralloc.v1[157]
#     tmp771 = __ralloc.v1[158]
#     tmp777 = __ralloc.v1[159]
#     tmp779 = __ralloc.v1[160]
#     tmp780 = __ralloc.v1[161]
#     Xij_t_Ui = __ralloc.v1[162]
#     Yij_t_Vi = __ralloc.v1[163]
#     Zij_t_Wi = __ralloc.v1[164]
#     tmp786 = __ralloc.v1[165]
#     Rij_dot_Vi = __ralloc.v1[166]
#     tmp789 = __ralloc.v1[167]
#     tmp897 = __ralloc.v1[168]
#     pn1t7 = __ralloc.v1[169]
#     tmp792 = __ralloc.v1[170]
#     pn1t2_7 = __ralloc.v1[171]
#     tmp799 = __ralloc.v1[172]
#     tmp800 = __ralloc.v1[173]
#     tmp801 = __ralloc.v1[174]
#     tmp809 = __ralloc.v1[175]
#     termpnx = __ralloc.v1[176]
#     sumpnx = __ralloc.v1[177]
#     tmp812 = __ralloc.v1[178]
#     termpny = __ralloc.v1[179]
#     sumpny = __ralloc.v1[180]
#     tmp815 = __ralloc.v1[181]
#     termpnz = __ralloc.v1[182]
#     sumpnz = __ralloc.v1[183]
#     local jd0 = params.jd0
#     local dsj2k = t + (jd0 - JD_J2000)
#     local ss16asteph_t = evaleph(params.sseph, dsj2k, q[1])
#     local acceph_t = evaleph(params.acceph, dsj2k, q[1])
#     local newtonianNb_Potential_t = evaleph(params.poteph, dsj2k, q[1])
#     local S = eltype(q)
#     local UJ_interaction = params.UJ_interaction
#     local N = params.N
#     local Nm1 = N - 1
#     local μ = params.μ
#     local zero_q_1 = auxzero(q[1])
#     TaylorSeries.zero!(pntempX)
#     pntempX.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(pntempY)
#     pntempY.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(pntempZ)
#     pntempZ.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accX)
#     accX.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accY)
#     accY.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accZ)
#     accZ.coeffs[1] = identity(constant_term(zero_q_1))
#     local M_ = Array{S}(undef, 3, 3, N)
#     local M_[:, :, ea] = t2c_jpl_de430(dsj2k) .+ zero_q_1
#     TaylorSeries.zero!(dq[1])
#     (dq[1]).coeffs[1] = identity(constant_term(q[4]))
#     TaylorSeries.zero!(dq[2])
#     (dq[2]).coeffs[1] = identity(constant_term(q[5]))
#     TaylorSeries.zero!(dq[3])
#     (dq[3]).coeffs[1] = identity(constant_term(q[6]))
#     TaylorSeries.zero!(newtonianNb_Potential[N])
#     (newtonianNb_Potential[N]).coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(_4dq[1])
#     (_4dq[1]).coeffs[1] = constant_term(4) * constant_term(dq[1])
#     TaylorSeries.zero!(_4dq[2])
#     (_4dq[2]).coeffs[1] = constant_term(4) * constant_term(dq[2])
#     TaylorSeries.zero!(_4dq[3])
#     (_4dq[3]).coeffs[1] = constant_term(4) * constant_term(dq[3])
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:253 =# Threads.@threads for i = 1:Nm1
#             TaylorSeries.zero!(ui[i])
#             (ui[i]).coeffs[1] = identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 2]))
#             TaylorSeries.zero!(vi[i])
#             (vi[i]).coeffs[1] = identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 1]))
#             TaylorSeries.zero!(wi[i])
#             (wi[i]).coeffs[1] = identity(constant_term(ss16asteph_t[3 * ((N - 1) + i)]))
#             TaylorSeries.zero!(X[i])
#             (X[i]).coeffs[1] = constant_term(ss16asteph_t[3i - 2]) - constant_term(q[1])
#             TaylorSeries.zero!(Y[i])
#             (Y[i]).coeffs[1] = constant_term(ss16asteph_t[3i - 1]) - constant_term(q[2])
#             TaylorSeries.zero!(Z[i])
#             (Z[i]).coeffs[1] = constant_term(ss16asteph_t[3i]) - constant_term(q[3])
#             TaylorSeries.zero!(U[i])
#             (U[i]).coeffs[1] = constant_term(ui[i]) - constant_term(dq[1])
#             TaylorSeries.zero!(V[i])
#             (V[i]).coeffs[1] = constant_term(vi[i]) - constant_term(dq[2])
#             TaylorSeries.zero!(W[i])
#             (W[i]).coeffs[1] = constant_term(wi[i]) - constant_term(dq[3])
#             TaylorSeries.zero!(tmp625[i])
#             (tmp625[i]).coeffs[1] = constant_term(-3) * constant_term(ui[i])
#             TaylorSeries.zero!(_4U_m_3X[i])
#             (_4U_m_3X[i]).coeffs[1] = constant_term(tmp625[i]) + constant_term(_4dq[1])
#             TaylorSeries.zero!(tmp628[i])
#             (tmp628[i]).coeffs[1] = constant_term(-3) * constant_term(vi[i])
#             TaylorSeries.zero!(_4V_m_3Y[i])
#             (_4V_m_3Y[i]).coeffs[1] = constant_term(tmp628[i]) + constant_term(_4dq[2])
#             TaylorSeries.zero!(tmp631[i])
#             (tmp631[i]).coeffs[1] = constant_term(-3) * constant_term(wi[i])
#             TaylorSeries.zero!(_4W_m_3Z[i])
#             (_4W_m_3Z[i]).coeffs[1] = constant_term(tmp631[i]) + constant_term(_4dq[3])
#             TaylorSeries.zero!(pn2x[i])
#             (pn2x[i]).coeffs[1] = constant_term(X[i]) * constant_term(_4U_m_3X[i])
#             TaylorSeries.zero!(pn2y[i])
#             (pn2y[i]).coeffs[1] = constant_term(Y[i]) * constant_term(_4V_m_3Y[i])
#             TaylorSeries.zero!(pn2z[i])
#             (pn2z[i]).coeffs[1] = constant_term(Z[i]) * constant_term(_4W_m_3Z[i])
#             TaylorSeries.zero!(UU[i])
#             (UU[i]).coeffs[1] = constant_term(ui[i]) * constant_term(dq[1])
#             TaylorSeries.zero!(VV[i])
#             (VV[i]).coeffs[1] = constant_term(vi[i]) * constant_term(dq[2])
#             TaylorSeries.zero!(WW[i])
#             (WW[i]).coeffs[1] = constant_term(wi[i]) * constant_term(dq[3])
#             TaylorSeries.zero!(tmp639[i])
#             (tmp639[i]).coeffs[1] = constant_term(UU[i]) + constant_term(VV[i])
#             TaylorSeries.zero!(vi_dot_vj[i])
#             (vi_dot_vj[i]).coeffs[1] = constant_term(tmp639[i]) + constant_term(WW[i])
#             TaylorSeries.zero!(tmp642[i])
#             (tmp642[i]).coeffs[1] = constant_term(X[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp880[i])
#             (tmp880[i]).coeffs[1] = zero(constant_term(X[i]))
#             TaylorSeries.zero!(tmp644[i])
#             (tmp644[i]).coeffs[1] = constant_term(Y[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp881[i])
#             (tmp881[i]).coeffs[1] = zero(constant_term(Y[i]))
#             TaylorSeries.zero!(tmp645[i])
#             (tmp645[i]).coeffs[1] = constant_term(tmp642[i]) + constant_term(tmp644[i])
#             TaylorSeries.zero!(tmp647[i])
#             (tmp647[i]).coeffs[1] = constant_term(Z[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp882[i])
#             (tmp882[i]).coeffs[1] = zero(constant_term(Z[i]))
#             TaylorSeries.zero!(r_p2[i])
#             (r_p2[i]).coeffs[1] = constant_term(tmp645[i]) + constant_term(tmp647[i])
#             TaylorSeries.zero!(r_p1d2[i])
#             (r_p1d2[i]).coeffs[1] = sqrt(constant_term(r_p2[i]))
#             TaylorSeries.zero!(r_p3d2[i])
#             (r_p3d2[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(1.5))
#             TaylorSeries.zero!(tmp883[i])
#             (tmp883[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#             TaylorSeries.zero!(r_p7d2[i])
#             (r_p7d2[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(3.5))
#             TaylorSeries.zero!(tmp884[i])
#             (tmp884[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#             TaylorSeries.zero!(newtonianCoeff[i])
#             (newtonianCoeff[i]).coeffs[1] = constant_term(μ[i]) / constant_term(r_p3d2[i])
#             TaylorSeries.zero!(tmp655[i])
#             (tmp655[i]).coeffs[1] = constant_term(pn2x[i]) + constant_term(pn2y[i])
#             TaylorSeries.zero!(tmp656[i])
#             (tmp656[i]).coeffs[1] = constant_term(tmp655[i]) + constant_term(pn2z[i])
#             TaylorSeries.zero!(pn2[i])
#             (pn2[i]).coeffs[1] = constant_term(newtonianCoeff[i]) * constant_term(tmp656[i])
#             TaylorSeries.zero!(newton_acc_X[i])
#             (newton_acc_X[i]).coeffs[1] = constant_term(X[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newton_acc_Y[i])
#             (newton_acc_Y[i]).coeffs[1] = constant_term(Y[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newton_acc_Z[i])
#             (newton_acc_Z[i]).coeffs[1] = constant_term(Z[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newtonian1b_Potential[i])
#             (newtonian1b_Potential[i]).coeffs[1] = constant_term(μ[i]) / constant_term(r_p1d2[i])
#             TaylorSeries.zero!(pn3[i])
#             (pn3[i]).coeffs[1] = constant_term(3.5) * constant_term(newtonian1b_Potential[i])
#             TaylorSeries.zero!(U_t_pn2[i])
#             (U_t_pn2[i]).coeffs[1] = constant_term(pn2[i]) * constant_term(U[i])
#             TaylorSeries.zero!(V_t_pn2[i])
#             (V_t_pn2[i]).coeffs[1] = constant_term(pn2[i]) * constant_term(V[i])
#             TaylorSeries.zero!(W_t_pn2[i])
#             (W_t_pn2[i]).coeffs[1] = constant_term(pn2[i]) * constant_term(W[i])
#             if UJ_interaction[i]
#                 TaylorSeries.zero!(tmp667[i])
#                 (tmp667[i]).coeffs[1] = -(constant_term(X[i]))
#                 TaylorSeries.zero!(t31[i])
#                 (t31[i]).coeffs[1] = constant_term(tmp667[i]) * constant_term(M_[1, 3, i])
#                 TaylorSeries.zero!(tmp669[i])
#                 (tmp669[i]).coeffs[1] = -(constant_term(Y[i]))
#                 TaylorSeries.zero!(t32[i])
#                 (t32[i]).coeffs[1] = constant_term(tmp669[i]) * constant_term(M_[2, 3, i])
#                 TaylorSeries.zero!(tmp671[i])
#                 (tmp671[i]).coeffs[1] = -(constant_term(Z[i]))
#                 TaylorSeries.zero!(t33[i])
#                 (t33[i]).coeffs[1] = constant_term(tmp671[i]) * constant_term(M_[3, 3, i])
#                 TaylorSeries.zero!(tmp673[i])
#                 (tmp673[i]).coeffs[1] = constant_term(t31[i]) + constant_term(t32[i])
#                 TaylorSeries.zero!(r_sin_ϕ[i])
#                 (r_sin_ϕ[i]).coeffs[1] = constant_term(tmp673[i]) + constant_term(t33[i])
#                 TaylorSeries.zero!(sin_ϕ[i])
#                 (sin_ϕ[i]).coeffs[1] = constant_term(r_sin_ϕ[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(ϕ[i])
#                 (ϕ[i]).coeffs[1] = asin(constant_term(sin_ϕ[i]))
#                 TaylorSeries.zero!(tmp885[i])
#                 (tmp885[i]).coeffs[1] = sqrt(1 - constant_term(sin_ϕ[i]) ^ 2)
#                 TaylorSeries.zero!(cos_ϕ[i])
#                 (cos_ϕ[i]).coeffs[1] = cos(constant_term(ϕ[i]))
#                 TaylorSeries.zero!(tmp886[i])
#                 (tmp886[i]).coeffs[1] = sin(constant_term(ϕ[i]))
#                 TaylorSeries.zero!(sin2_ϕ[i])
#                 (sin2_ϕ[i]).coeffs[1] = constant_term(sin_ϕ[i]) ^ float(constant_term(2))
#                 TaylorSeries.zero!(tmp887[i])
#                 (tmp887[i]).coeffs[1] = zero(constant_term(sin_ϕ[i]))
#                 TaylorSeries.zero!(sin3_ϕ[i])
#                 (sin3_ϕ[i]).coeffs[1] = constant_term(sin_ϕ[i]) ^ float(constant_term(3))
#                 TaylorSeries.zero!(tmp888[i])
#                 (tmp888[i]).coeffs[1] = zero(constant_term(sin_ϕ[i]))
#                 TaylorSeries.zero!(tmp683[i])
#                 (tmp683[i]).coeffs[1] = constant_term(1.5) * constant_term(sin2_ϕ[i])
#                 TaylorSeries.zero!(P_2_sin_ϕ[i])
#                 (P_2_sin_ϕ[i]).coeffs[1] = constant_term(tmp683[i]) - constant_term(0.5)
#                 TaylorSeries.zero!(∂P_2_sin_ϕ[i])
#                 (∂P_2_sin_ϕ[i]).coeffs[1] = constant_term(3) * constant_term(sin_ϕ[i])
#                 TaylorSeries.zero!(tmp689[i])
#                 (tmp689[i]).coeffs[1] = constant_term(-1.5) * constant_term(sin_ϕ[i])
#                 TaylorSeries.zero!(tmp691[i])
#                 (tmp691[i]).coeffs[1] = constant_term(2.5) * constant_term(sin3_ϕ[i])
#                 TaylorSeries.zero!(P_3_sin_ϕ[i])
#                 (P_3_sin_ϕ[i]).coeffs[1] = constant_term(tmp689[i]) + constant_term(tmp691[i])
#                 TaylorSeries.zero!(tmp695[i])
#                 (tmp695[i]).coeffs[1] = constant_term(7.5) * constant_term(sin2_ϕ[i])
#                 TaylorSeries.zero!(∂P_3_sin_ϕ[i])
#                 (∂P_3_sin_ϕ[i]).coeffs[1] = constant_term(-1.5) + constant_term(tmp695[i])
#                 TaylorSeries.zero!(tmp698[i])
#                 (tmp698[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(2))
#                 TaylorSeries.zero!(tmp889[i])
#                 (tmp889[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#                 TaylorSeries.zero!(tmp699[i])
#                 (tmp699[i]).coeffs[1] = constant_term(Λ2[i]) / constant_term(tmp698[i])
#                 TaylorSeries.zero!(Λ2j_div_r4[i])
#                 (Λ2j_div_r4[i]).coeffs[1] = -(constant_term(tmp699[i]))
#                 TaylorSeries.zero!(tmp702[i])
#                 (tmp702[i]).coeffs[1] = constant_term(r_p1d2[i]) ^ float(constant_term(5))
#                 TaylorSeries.zero!(tmp890[i])
#                 (tmp890[i]).coeffs[1] = zero(constant_term(r_p1d2[i]))
#                 TaylorSeries.zero!(tmp703[i])
#                 (tmp703[i]).coeffs[1] = constant_term(Λ3[i]) / constant_term(tmp702[i])
#                 TaylorSeries.zero!(Λ3j_div_r5[i])
#                 (Λ3j_div_r5[i]).coeffs[1] = -(constant_term(tmp703[i]))
#                 TaylorSeries.zero!(tmp705[i])
#                 (tmp705[i]).coeffs[1] = -(constant_term(cos_ϕ[i]))
#                 TaylorSeries.zero!(m_c_ϕ_∂P_2[i])
#                 (m_c_ϕ_∂P_2[i]).coeffs[1] = constant_term(tmp705[i]) * constant_term(∂P_2_sin_ϕ[i])
#                 TaylorSeries.zero!(tmp707[i])
#                 (tmp707[i]).coeffs[1] = -(constant_term(cos_ϕ[i]))
#                 TaylorSeries.zero!(m_c_ϕ_∂P_3[i])
#                 (m_c_ϕ_∂P_3[i]).coeffs[1] = constant_term(tmp707[i]) * constant_term(∂P_3_sin_ϕ[i])
#                 TaylorSeries.zero!(tmp710[i])
#                 (tmp710[i]).coeffs[1] = constant_term(Λ2j_div_r4[i]) * constant_term(3)
#                 TaylorSeries.zero!(F_J2_ξ[i])
#                 (F_J2_ξ[i]).coeffs[1] = constant_term(tmp710[i]) * constant_term(P_2_sin_ϕ[i])
#                 TaylorSeries.zero!(F_J2_ζ[i])
#                 (F_J2_ζ[i]).coeffs[1] = constant_term(Λ2j_div_r4[i]) * constant_term(m_c_ϕ_∂P_2[i])
#                 TaylorSeries.zero!(tmp714[i])
#                 (tmp714[i]).coeffs[1] = constant_term(Λ3j_div_r5[i]) * constant_term(4)
#                 TaylorSeries.zero!(F_J3_ξ[i])
#                 (F_J3_ξ[i]).coeffs[1] = constant_term(tmp714[i]) * constant_term(P_3_sin_ϕ[i])
#                 TaylorSeries.zero!(F_J3_ζ[i])
#                 (F_J3_ζ[i]).coeffs[1] = constant_term(Λ3j_div_r5[i]) * constant_term(m_c_ϕ_∂P_3[i])
#                 TaylorSeries.zero!(F_J_ξ[i])
#                 (F_J_ξ[i]).coeffs[1] = identity(constant_term(F_J2_ξ[i]))
#                 TaylorSeries.zero!(F_J_ζ[i])
#                 (F_J_ζ[i]).coeffs[1] = identity(constant_term(F_J2_ζ[i]))
#                 TaylorSeries.zero!(tmp717[i])
#                 (tmp717[i]).coeffs[1] = -(constant_term(X[i]))
#                 TaylorSeries.zero!(ξx[i])
#                 (ξx[i]).coeffs[1] = constant_term(tmp717[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(tmp719[i])
#                 (tmp719[i]).coeffs[1] = -(constant_term(Y[i]))
#                 TaylorSeries.zero!(ξy[i])
#                 (ξy[i]).coeffs[1] = constant_term(tmp719[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(tmp721[i])
#                 (tmp721[i]).coeffs[1] = -(constant_term(Z[i]))
#                 TaylorSeries.zero!(ξz[i])
#                 (ξz[i]).coeffs[1] = constant_term(tmp721[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(ηx1[i])
#                 (ηx1[i]).coeffs[1] = constant_term(M_[2, 3, i]) * constant_term(ξz[i])
#                 TaylorSeries.zero!(ηy1[i])
#                 (ηy1[i]).coeffs[1] = constant_term(M_[3, 3, i]) * constant_term(ξx[i])
#                 TaylorSeries.zero!(ηz1[i])
#                 (ηz1[i]).coeffs[1] = constant_term(M_[1, 3, i]) * constant_term(ξy[i])
#                 TaylorSeries.zero!(ηx2[i])
#                 (ηx2[i]).coeffs[1] = constant_term(M_[3, 3, i]) * constant_term(ξy[i])
#                 TaylorSeries.zero!(ηy2[i])
#                 (ηy2[i]).coeffs[1] = constant_term(M_[1, 3, i]) * constant_term(ξz[i])
#                 TaylorSeries.zero!(ηz2[i])
#                 (ηz2[i]).coeffs[1] = constant_term(M_[2, 3, i]) * constant_term(ξx[i])
#                 TaylorSeries.zero!(ηx[i])
#                 (ηx[i]).coeffs[1] = constant_term(ηx1[i]) - constant_term(ηx2[i])
#                 TaylorSeries.zero!(ηy[i])
#                 (ηy[i]).coeffs[1] = constant_term(ηy1[i]) - constant_term(ηy2[i])
#                 TaylorSeries.zero!(ηz[i])
#                 (ηz[i]).coeffs[1] = constant_term(ηz1[i]) - constant_term(ηz2[i])
#                 TaylorSeries.zero!(ζx1[i])
#                 (ζx1[i]).coeffs[1] = constant_term(ξy[i]) * constant_term(ηz[i])
#                 TaylorSeries.zero!(ζy1[i])
#                 (ζy1[i]).coeffs[1] = constant_term(ξz[i]) * constant_term(ηx[i])
#                 TaylorSeries.zero!(ζz1[i])
#                 (ζz1[i]).coeffs[1] = constant_term(ξx[i]) * constant_term(ηy[i])
#                 TaylorSeries.zero!(ζx2[i])
#                 (ζx2[i]).coeffs[1] = constant_term(ξz[i]) * constant_term(ηy[i])
#                 TaylorSeries.zero!(ζy2[i])
#                 (ζy2[i]).coeffs[1] = constant_term(ξx[i]) * constant_term(ηz[i])
#                 TaylorSeries.zero!(ζz2[i])
#                 (ζz2[i]).coeffs[1] = constant_term(ξy[i]) * constant_term(ηx[i])
#                 TaylorSeries.zero!(ζx[i])
#                 (ζx[i]).coeffs[1] = constant_term(ζx1[i]) - constant_term(ζx2[i])
#                 TaylorSeries.zero!(ζy[i])
#                 (ζy[i]).coeffs[1] = constant_term(ζy1[i]) - constant_term(ζy2[i])
#                 TaylorSeries.zero!(ζz[i])
#                 (ζz[i]).coeffs[1] = constant_term(ζz1[i]) - constant_term(ζz2[i])
#                 TaylorSeries.zero!(F_J2_x1[i])
#                 (F_J2_x1[i]).coeffs[1] = constant_term(F_J_ξ[i]) * constant_term(ξx[i])
#                 TaylorSeries.zero!(F_J2_y1[i])
#                 (F_J2_y1[i]).coeffs[1] = constant_term(F_J_ξ[i]) * constant_term(ξy[i])
#                 TaylorSeries.zero!(F_J2_z1[i])
#                 (F_J2_z1[i]).coeffs[1] = constant_term(F_J_ξ[i]) * constant_term(ξz[i])
#                 TaylorSeries.zero!(F_J2_x2[i])
#                 (F_J2_x2[i]).coeffs[1] = constant_term(F_J_ζ[i]) * constant_term(ζx[i])
#                 TaylorSeries.zero!(F_J2_y2[i])
#                 (F_J2_y2[i]).coeffs[1] = constant_term(F_J_ζ[i]) * constant_term(ζy[i])
#                 TaylorSeries.zero!(F_J2_z2[i])
#                 (F_J2_z2[i]).coeffs[1] = constant_term(F_J_ζ[i]) * constant_term(ζz[i])
#                 TaylorSeries.zero!(F_J2_x[i])
#                 (F_J2_x[i]).coeffs[1] = constant_term(F_J2_x1[i]) + constant_term(F_J2_x2[i])
#                 TaylorSeries.zero!(F_J2_y[i])
#                 (F_J2_y[i]).coeffs[1] = constant_term(F_J2_y1[i]) + constant_term(F_J2_y2[i])
#                 TaylorSeries.zero!(F_J2_z[i])
#                 (F_J2_z[i]).coeffs[1] = constant_term(F_J2_z1[i]) + constant_term(F_J2_z2[i])
#             end
#             TaylorSeries.zero!(tmp751[i])
#             (tmp751[i]).coeffs[1] = constant_term(ui[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp891[i])
#             (tmp891[i]).coeffs[1] = zero(constant_term(ui[i]))
#             TaylorSeries.zero!(tmp753[i])
#             (tmp753[i]).coeffs[1] = constant_term(vi[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp892[i])
#             (tmp892[i]).coeffs[1] = zero(constant_term(vi[i]))
#             TaylorSeries.zero!(tmp754[i])
#             (tmp754[i]).coeffs[1] = constant_term(tmp751[i]) + constant_term(tmp753[i])
#             TaylorSeries.zero!(tmp756[i])
#             (tmp756[i]).coeffs[1] = constant_term(wi[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp893[i])
#             (tmp893[i]).coeffs[1] = zero(constant_term(wi[i]))
#             TaylorSeries.zero!(v2[i])
#             (v2[i]).coeffs[1] = constant_term(tmp754[i]) + constant_term(tmp756[i])
#         end
#     TaylorSeries.zero!(tmp759)
#     tmp759.coeffs[1] = constant_term(q[4]) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp894)
#     tmp894.coeffs[1] = zero(constant_term(q[4]))
#     TaylorSeries.zero!(tmp761)
#     tmp761.coeffs[1] = constant_term(q[5]) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp895)
#     tmp895.coeffs[1] = zero(constant_term(q[5]))
#     TaylorSeries.zero!(tmp762)
#     tmp762.coeffs[1] = constant_term(tmp759) + constant_term(tmp761)
#     TaylorSeries.zero!(tmp764)
#     tmp764.coeffs[1] = constant_term(q[6]) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp896)
#     tmp896.coeffs[1] = zero(constant_term(q[6]))
#     TaylorSeries.zero!(v2[N])
#     (v2[N]).coeffs[1] = constant_term(tmp762) + constant_term(tmp764)
#     for i = 1:Nm1
#         TaylorSeries.zero!(temp_004[i])
#         (temp_004[i]).coeffs[1] = constant_term(newtonian1b_Potential[i]) + constant_term(newtonianNb_Potential[N])
#         TaylorSeries.zero!(newtonianNb_Potential[N])
#         (newtonianNb_Potential[N]).coeffs[1] = identity(constant_term(temp_004[i]))
#         if UJ_interaction[i]
#             TaylorSeries.zero!(tmp767[i])
#             (tmp767[i]).coeffs[1] = constant_term(μ[i]) * constant_term(F_J2_x[i])
#             TaylorSeries.zero!(temp_accX_i[i])
#             (temp_accX_i[i]).coeffs[1] = constant_term(accX) - constant_term(tmp767[i])
#             TaylorSeries.zero!(accX)
#             accX.coeffs[1] = identity(constant_term(temp_accX_i[i]))
#             TaylorSeries.zero!(tmp769[i])
#             (tmp769[i]).coeffs[1] = constant_term(μ[i]) * constant_term(F_J2_y[i])
#             TaylorSeries.zero!(temp_accY_i[i])
#             (temp_accY_i[i]).coeffs[1] = constant_term(accY) - constant_term(tmp769[i])
#             TaylorSeries.zero!(accY)
#             accY.coeffs[1] = identity(constant_term(temp_accY_i[i]))
#             TaylorSeries.zero!(tmp771[i])
#             (tmp771[i]).coeffs[1] = constant_term(μ[i]) * constant_term(F_J2_z[i])
#             TaylorSeries.zero!(temp_accZ_i[i])
#             (temp_accZ_i[i]).coeffs[1] = constant_term(accZ) - constant_term(tmp771[i])
#             TaylorSeries.zero!(accZ)
#             accZ.coeffs[1] = identity(constant_term(temp_accZ_i[i]))
#         end
#     end
#     TaylorSeries.zero!(_4ϕj[N])
#     (_4ϕj[N]).coeffs[1] = constant_term(4) * constant_term(newtonianNb_Potential[N])
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:455 =# Threads.@threads for i = 1:10
#             TaylorSeries.zero!(ϕi_plus_4ϕj[i])
#             (ϕi_plus_4ϕj[i]).coeffs[1] = constant_term(newtonianNb_Potential_t[i]) + constant_term(_4ϕj[N])
#             TaylorSeries.zero!(tmp777[i])
#             (tmp777[i]).coeffs[1] = constant_term(2) * constant_term(v2[i])
#             TaylorSeries.zero!(tmp779[i])
#             (tmp779[i]).coeffs[1] = constant_term(4) * constant_term(vi_dot_vj[i])
#             TaylorSeries.zero!(tmp780[i])
#             (tmp780[i]).coeffs[1] = constant_term(tmp777[i]) - constant_term(tmp779[i])
#             TaylorSeries.zero!(sj2_plus_2si2_minus_4vivj[i])
#             (sj2_plus_2si2_minus_4vivj[i]).coeffs[1] = constant_term(tmp780[i]) + constant_term(v2[N])
#             TaylorSeries.zero!(ϕs_and_vs[i])
#             (ϕs_and_vs[i]).coeffs[1] = constant_term(sj2_plus_2si2_minus_4vivj[i]) - constant_term(ϕi_plus_4ϕj[i])
#             TaylorSeries.zero!(Xij_t_Ui[i])
#             (Xij_t_Ui[i]).coeffs[1] = constant_term(X[i]) * constant_term(ui[i])
#             TaylorSeries.zero!(Yij_t_Vi[i])
#             (Yij_t_Vi[i]).coeffs[1] = constant_term(Y[i]) * constant_term(vi[i])
#             TaylorSeries.zero!(Zij_t_Wi[i])
#             (Zij_t_Wi[i]).coeffs[1] = constant_term(Z[i]) * constant_term(wi[i])
#             TaylorSeries.zero!(tmp786[i])
#             (tmp786[i]).coeffs[1] = constant_term(Xij_t_Ui[i]) + constant_term(Yij_t_Vi[i])
#             TaylorSeries.zero!(Rij_dot_Vi[i])
#             (Rij_dot_Vi[i]).coeffs[1] = constant_term(tmp786[i]) + constant_term(Zij_t_Wi[i])
#             TaylorSeries.zero!(tmp789[i])
#             (tmp789[i]).coeffs[1] = constant_term(Rij_dot_Vi[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp897[i])
#             (tmp897[i]).coeffs[1] = zero(constant_term(Rij_dot_Vi[i]))
#             TaylorSeries.zero!(pn1t7[i])
#             (pn1t7[i]).coeffs[1] = constant_term(tmp789[i]) / constant_term(r_p2[i])
#             TaylorSeries.zero!(tmp792[i])
#             (tmp792[i]).coeffs[1] = constant_term(1.5) * constant_term(pn1t7[i])
#             TaylorSeries.zero!(pn1t2_7[i])
#             (pn1t2_7[i]).coeffs[1] = constant_term(ϕs_and_vs[i]) - constant_term(tmp792[i])
#             TaylorSeries.zero!(pn1t1_7[i])
#             (pn1t1_7[i]).coeffs[1] = constant_term(c_p2) + constant_term(pn1t2_7[i])
#             TaylorSeries.zero!(pNX_t_X[i])
#             (pNX_t_X[i]).coeffs[1] = constant_term(acceph_t[3i - 2]) * constant_term(X[i])
#             TaylorSeries.zero!(pNY_t_Y[i])
#             (pNY_t_Y[i]).coeffs[1] = constant_term(acceph_t[3i - 1]) * constant_term(Y[i])
#             TaylorSeries.zero!(pNZ_t_Z[i])
#             (pNZ_t_Z[i]).coeffs[1] = constant_term(acceph_t[3i]) * constant_term(Z[i])
#             TaylorSeries.zero!(tmp799[i])
#             (tmp799[i]).coeffs[1] = constant_term(pNX_t_X[i]) + constant_term(pNY_t_Y[i])
#             TaylorSeries.zero!(tmp800[i])
#             (tmp800[i]).coeffs[1] = constant_term(tmp799[i]) + constant_term(pNZ_t_Z[i])
#             TaylorSeries.zero!(tmp801[i])
#             (tmp801[i]).coeffs[1] = constant_term(0.5) * constant_term(tmp800[i])
#             TaylorSeries.zero!(pn1[i])
#             (pn1[i]).coeffs[1] = constant_term(pn1t1_7[i]) + constant_term(tmp801[i])
#             TaylorSeries.zero!(X_t_pn1[i])
#             (X_t_pn1[i]).coeffs[1] = constant_term(newton_acc_X[i]) * constant_term(pn1[i])
#             TaylorSeries.zero!(Y_t_pn1[i])
#             (Y_t_pn1[i]).coeffs[1] = constant_term(newton_acc_Y[i]) * constant_term(pn1[i])
#             TaylorSeries.zero!(Z_t_pn1[i])
#             (Z_t_pn1[i]).coeffs[1] = constant_term(newton_acc_Z[i]) * constant_term(pn1[i])
#             TaylorSeries.zero!(pNX_t_pn3[i])
#             (pNX_t_pn3[i]).coeffs[1] = constant_term(acceph_t[3i - 2]) * constant_term(pn3[i])
#             TaylorSeries.zero!(pNY_t_pn3[i])
#             (pNY_t_pn3[i]).coeffs[1] = constant_term(acceph_t[3i - 1]) * constant_term(pn3[i])
#             TaylorSeries.zero!(pNZ_t_pn3[i])
#             (pNZ_t_pn3[i]).coeffs[1] = constant_term(acceph_t[3i]) * constant_term(pn3[i])
#         end
#     for i = 1:10
#         TaylorSeries.zero!(tmp809[i])
#         (tmp809[i]).coeffs[1] = constant_term(U_t_pn2[i]) + constant_term(pNX_t_pn3[i])
#         TaylorSeries.zero!(termpnx[i])
#         (termpnx[i]).coeffs[1] = constant_term(X_t_pn1[i]) + constant_term(tmp809[i])
#         TaylorSeries.zero!(sumpnx[i])
#         (sumpnx[i]).coeffs[1] = constant_term(pntempX) + constant_term(termpnx[i])
#         TaylorSeries.zero!(pntempX)
#         pntempX.coeffs[1] = identity(constant_term(sumpnx[i]))
#         TaylorSeries.zero!(tmp812[i])
#         (tmp812[i]).coeffs[1] = constant_term(V_t_pn2[i]) + constant_term(pNY_t_pn3[i])
#         TaylorSeries.zero!(termpny[i])
#         (termpny[i]).coeffs[1] = constant_term(Y_t_pn1[i]) + constant_term(tmp812[i])
#         TaylorSeries.zero!(sumpny[i])
#         (sumpny[i]).coeffs[1] = constant_term(pntempY) + constant_term(termpny[i])
#         TaylorSeries.zero!(pntempY)
#         pntempY.coeffs[1] = identity(constant_term(sumpny[i]))
#         TaylorSeries.zero!(tmp815[i])
#         (tmp815[i]).coeffs[1] = constant_term(W_t_pn2[i]) + constant_term(pNZ_t_pn3[i])
#         TaylorSeries.zero!(termpnz[i])
#         (termpnz[i]).coeffs[1] = constant_term(Z_t_pn1[i]) + constant_term(tmp815[i])
#         TaylorSeries.zero!(sumpnz[i])
#         (sumpnz[i]).coeffs[1] = constant_term(pntempZ) + constant_term(termpnz[i])
#         TaylorSeries.zero!(pntempZ)
#         pntempZ.coeffs[1] = identity(constant_term(sumpnz[i]))
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:508 =# Threads.@threads for i = 11:Nm1
#             TaylorSeries.zero!(X_t_pn1[i])
#             (X_t_pn1[i]).coeffs[1] = constant_term(c_p2) * constant_term(newton_acc_X[i])
#             TaylorSeries.zero!(Y_t_pn1[i])
#             (Y_t_pn1[i]).coeffs[1] = constant_term(c_p2) * constant_term(newton_acc_Y[i])
#             TaylorSeries.zero!(Z_t_pn1[i])
#             (Z_t_pn1[i]).coeffs[1] = constant_term(c_p2) * constant_term(newton_acc_Z[i])
#         end
#     for i = 11:Nm1
#         TaylorSeries.zero!(termpnx[i])
#         (termpnx[i]).coeffs[1] = identity(constant_term(X_t_pn1[i]))
#         TaylorSeries.zero!(sumpnx[i])
#         (sumpnx[i]).coeffs[1] = constant_term(pntempX) + constant_term(termpnx[i])
#         TaylorSeries.zero!(pntempX)
#         pntempX.coeffs[1] = identity(constant_term(sumpnx[i]))
#         TaylorSeries.zero!(termpny[i])
#         (termpny[i]).coeffs[1] = identity(constant_term(Y_t_pn1[i]))
#         TaylorSeries.zero!(sumpny[i])
#         (sumpny[i]).coeffs[1] = constant_term(pntempY) + constant_term(termpny[i])
#         TaylorSeries.zero!(pntempY)
#         pntempY.coeffs[1] = identity(constant_term(sumpny[i]))
#         TaylorSeries.zero!(termpnz[i])
#         (termpnz[i]).coeffs[1] = identity(constant_term(Z_t_pn1[i]))
#         TaylorSeries.zero!(sumpnz[i])
#         (sumpnz[i]).coeffs[1] = constant_term(pntempZ) + constant_term(termpnz[i])
#         TaylorSeries.zero!(pntempZ)
#         pntempZ.coeffs[1] = identity(constant_term(sumpnz[i]))
#     end
#     TaylorSeries.zero!(postNewtonX)
#     postNewtonX.coeffs[1] = constant_term(pntempX) * constant_term(c_m2)
#     TaylorSeries.zero!(postNewtonY)
#     postNewtonY.coeffs[1] = constant_term(pntempY) * constant_term(c_m2)
#     TaylorSeries.zero!(postNewtonZ)
#     postNewtonZ.coeffs[1] = constant_term(pntempZ) * constant_term(c_m2)
#     TaylorSeries.zero!(tmp827)
#     tmp827.coeffs[1] = constant_term(Y[1]) * constant_term(W[1])
#     TaylorSeries.zero!(tmp828)
#     tmp828.coeffs[1] = constant_term(Z[1]) * constant_term(V[1])
#     TaylorSeries.zero!(hx)
#     hx.coeffs[1] = constant_term(tmp827) - constant_term(tmp828)
#     TaylorSeries.zero!(tmp830)
#     tmp830.coeffs[1] = constant_term(Z[1]) * constant_term(U[1])
#     TaylorSeries.zero!(tmp831)
#     tmp831.coeffs[1] = constant_term(X[1]) * constant_term(W[1])
#     TaylorSeries.zero!(hy)
#     hy.coeffs[1] = constant_term(tmp830) - constant_term(tmp831)
#     TaylorSeries.zero!(tmp833)
#     tmp833.coeffs[1] = constant_term(X[1]) * constant_term(V[1])
#     TaylorSeries.zero!(tmp834)
#     tmp834.coeffs[1] = constant_term(Y[1]) * constant_term(U[1])
#     TaylorSeries.zero!(hz)
#     hz.coeffs[1] = constant_term(tmp833) - constant_term(tmp834)
#     TaylorSeries.zero!(tmp836)
#     tmp836.coeffs[1] = constant_term(hz) * constant_term(Y[1])
#     TaylorSeries.zero!(tmp837)
#     tmp837.coeffs[1] = constant_term(hy) * constant_term(Z[1])
#     TaylorSeries.zero!(t_x)
#     t_x.coeffs[1] = constant_term(tmp836) - constant_term(tmp837)
#     TaylorSeries.zero!(tmp839)
#     tmp839.coeffs[1] = constant_term(hx) * constant_term(Z[1])
#     TaylorSeries.zero!(tmp840)
#     tmp840.coeffs[1] = constant_term(hz) * constant_term(X[1])
#     TaylorSeries.zero!(t_y)
#     t_y.coeffs[1] = constant_term(tmp839) - constant_term(tmp840)
#     TaylorSeries.zero!(tmp842)
#     tmp842.coeffs[1] = constant_term(hy) * constant_term(X[1])
#     TaylorSeries.zero!(tmp843)
#     tmp843.coeffs[1] = constant_term(hx) * constant_term(Y[1])
#     TaylorSeries.zero!(t_z)
#     t_z.coeffs[1] = constant_term(tmp842) - constant_term(tmp843)
#     TaylorSeries.zero!(tmp846)
#     tmp846.coeffs[1] = constant_term(t_x) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp898)
#     tmp898.coeffs[1] = zero(constant_term(t_x))
#     TaylorSeries.zero!(tmp848)
#     tmp848.coeffs[1] = constant_term(t_y) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp899)
#     tmp899.coeffs[1] = zero(constant_term(t_y))
#     TaylorSeries.zero!(tmp849)
#     tmp849.coeffs[1] = constant_term(tmp846) + constant_term(tmp848)
#     TaylorSeries.zero!(tmp851)
#     tmp851.coeffs[1] = constant_term(t_z) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp900)
#     tmp900.coeffs[1] = zero(constant_term(t_z))
#     TaylorSeries.zero!(tmp852)
#     tmp852.coeffs[1] = constant_term(tmp849) + constant_term(tmp851)
#     TaylorSeries.zero!(t_norm)
#     t_norm.coeffs[1] = sqrt(constant_term(tmp852))
#     TaylorSeries.zero!(t_x_unit)
#     t_x_unit.coeffs[1] = constant_term(t_x) / constant_term(t_norm)
#     TaylorSeries.zero!(t_y_unit)
#     t_y_unit.coeffs[1] = constant_term(t_y) / constant_term(t_norm)
#     TaylorSeries.zero!(t_z_unit)
#     t_z_unit.coeffs[1] = constant_term(t_z) / constant_term(t_norm)
#     TaylorSeries.zero!(tmp857)
#     tmp857.coeffs[1] = constant_term(X[1]) / constant_term(r_p1d2[1])
#     TaylorSeries.zero!(r_x_unit)
#     r_x_unit.coeffs[1] = -(constant_term(tmp857))
#     TaylorSeries.zero!(tmp859)
#     tmp859.coeffs[1] = constant_term(Y[1]) / constant_term(r_p1d2[1])
#     TaylorSeries.zero!(r_y_unit)
#     r_y_unit.coeffs[1] = -(constant_term(tmp859))
#     TaylorSeries.zero!(tmp861)
#     tmp861.coeffs[1] = constant_term(Z[1]) / constant_term(r_p1d2[1])
#     TaylorSeries.zero!(r_z_unit)
#     r_z_unit.coeffs[1] = -(constant_term(tmp861))
#     TaylorSeries.zero!(g_r)
#     g_r.coeffs[1] = identity(constant_term(r_p2[1]))
#     TaylorSeries.zero!(A2_t_g_r)
#     A2_t_g_r.coeffs[1] = constant_term(q[7]) / constant_term(g_r)
#     TaylorSeries.zero!(A1_t_g_r)
#     A1_t_g_r.coeffs[1] = constant_term(q[8]) / constant_term(g_r)
#     TaylorSeries.zero!(tmp865)
#     tmp865.coeffs[1] = constant_term(A2_t_g_r) * constant_term(t_x_unit)
#     TaylorSeries.zero!(tmp866)
#     tmp866.coeffs[1] = constant_term(A1_t_g_r) * constant_term(r_x_unit)
#     TaylorSeries.zero!(NGAx)
#     NGAx.coeffs[1] = constant_term(tmp865) + constant_term(tmp866)
#     TaylorSeries.zero!(tmp868)
#     tmp868.coeffs[1] = constant_term(A2_t_g_r) * constant_term(t_y_unit)
#     TaylorSeries.zero!(tmp869)
#     tmp869.coeffs[1] = constant_term(A1_t_g_r) * constant_term(r_y_unit)
#     TaylorSeries.zero!(NGAy)
#     NGAy.coeffs[1] = constant_term(tmp868) + constant_term(tmp869)
#     TaylorSeries.zero!(tmp871)
#     tmp871.coeffs[1] = constant_term(A2_t_g_r) * constant_term(t_z_unit)
#     TaylorSeries.zero!(tmp872)
#     tmp872.coeffs[1] = constant_term(A1_t_g_r) * constant_term(r_z_unit)
#     TaylorSeries.zero!(NGAz)
#     NGAz.coeffs[1] = constant_term(tmp871) + constant_term(tmp872)
#     TaylorSeries.zero!(tmp874)
#     tmp874.coeffs[1] = constant_term(postNewtonX) + constant_term(accX)
#     TaylorSeries.zero!(dq[4])
#     (dq[4]).coeffs[1] = constant_term(tmp874) + constant_term(NGAx)
#     TaylorSeries.zero!(tmp876)
#     tmp876.coeffs[1] = constant_term(postNewtonY) + constant_term(accY)
#     TaylorSeries.zero!(dq[5])
#     (dq[5]).coeffs[1] = constant_term(tmp876) + constant_term(NGAy)
#     TaylorSeries.zero!(tmp878)
#     tmp878.coeffs[1] = constant_term(postNewtonZ) + constant_term(accZ)
#     TaylorSeries.zero!(dq[6])
#     (dq[6]).coeffs[1] = constant_term(tmp878) + constant_term(NGAz)
#     TaylorSeries.zero!(dq[7])
#     (dq[7]).coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(dq[8])
#     (dq[8]).coeffs[1] = identity(constant_term(zero_q_1))
#     for __idx = eachindex(q)
#         (q[__idx]).coeffs[2] = (dq[__idx]).coeffs[1]
#     end
#     for ord = 1:order - 1
#         ordnext = ord + 1
#         TaylorSeries.identity!(pntempX, zero_q_1, ord)
#         TaylorSeries.identity!(pntempY, zero_q_1, ord)
#         TaylorSeries.identity!(pntempZ, zero_q_1, ord)
#         TaylorSeries.identity!(accX, zero_q_1, ord)
#         TaylorSeries.identity!(accY, zero_q_1, ord)
#         TaylorSeries.identity!(accZ, zero_q_1, ord)
#         TaylorSeries.identity!(dq[1], q[4], ord)
#         TaylorSeries.identity!(dq[2], q[5], ord)
#         TaylorSeries.identity!(dq[3], q[6], ord)
#         TaylorSeries.identity!(newtonianNb_Potential[N], zero_q_1, ord)
#         TaylorSeries.mul!(_4dq[1], 4, dq[1], ord)
#         TaylorSeries.mul!(_4dq[2], 4, dq[2], ord)
#         TaylorSeries.mul!(_4dq[3], 4, dq[3], ord)
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:253 =# Threads.@threads for i = 1:Nm1
#                 TaylorSeries.identity!(ui[i], ss16asteph_t[3 * ((N - 1) + i) - 2], ord)
#                 TaylorSeries.identity!(vi[i], ss16asteph_t[3 * ((N - 1) + i) - 1], ord)
#                 TaylorSeries.identity!(wi[i], ss16asteph_t[3 * ((N - 1) + i)], ord)
#                 TaylorSeries.subst!(X[i], ss16asteph_t[3i - 2], q[1], ord)
#                 TaylorSeries.subst!(Y[i], ss16asteph_t[3i - 1], q[2], ord)
#                 TaylorSeries.subst!(Z[i], ss16asteph_t[3i], q[3], ord)
#                 TaylorSeries.subst!(U[i], ui[i], dq[1], ord)
#                 TaylorSeries.subst!(V[i], vi[i], dq[2], ord)
#                 TaylorSeries.subst!(W[i], wi[i], dq[3], ord)
#                 TaylorSeries.mul!(tmp625[i], -3, ui[i], ord)
#                 TaylorSeries.add!(_4U_m_3X[i], tmp625[i], _4dq[1], ord)
#                 TaylorSeries.mul!(tmp628[i], -3, vi[i], ord)
#                 TaylorSeries.add!(_4V_m_3Y[i], tmp628[i], _4dq[2], ord)
#                 TaylorSeries.mul!(tmp631[i], -3, wi[i], ord)
#                 TaylorSeries.add!(_4W_m_3Z[i], tmp631[i], _4dq[3], ord)
#                 TaylorSeries.mul!(pn2x[i], X[i], _4U_m_3X[i], ord)
#                 TaylorSeries.mul!(pn2y[i], Y[i], _4V_m_3Y[i], ord)
#                 TaylorSeries.mul!(pn2z[i], Z[i], _4W_m_3Z[i], ord)
#                 TaylorSeries.mul!(UU[i], ui[i], dq[1], ord)
#                 TaylorSeries.mul!(VV[i], vi[i], dq[2], ord)
#                 TaylorSeries.mul!(WW[i], wi[i], dq[3], ord)
#                 TaylorSeries.add!(tmp639[i], UU[i], VV[i], ord)
#                 TaylorSeries.add!(vi_dot_vj[i], tmp639[i], WW[i], ord)
#                 TaylorSeries.pow!(tmp642[i], X[i], tmp880[i], 2, ord)
#                 TaylorSeries.pow!(tmp644[i], Y[i], tmp881[i], 2, ord)
#                 TaylorSeries.add!(tmp645[i], tmp642[i], tmp644[i], ord)
#                 TaylorSeries.pow!(tmp647[i], Z[i], tmp882[i], 2, ord)
#                 TaylorSeries.add!(r_p2[i], tmp645[i], tmp647[i], ord)
#                 TaylorSeries.sqrt!(r_p1d2[i], r_p2[i], ord)
#                 TaylorSeries.pow!(r_p3d2[i], r_p2[i], tmp883[i], 1.5, ord)
#                 TaylorSeries.pow!(r_p7d2[i], r_p2[i], tmp884[i], 3.5, ord)
#                 TaylorSeries.div!(newtonianCoeff[i], μ[i], r_p3d2[i], ord)
#                 TaylorSeries.add!(tmp655[i], pn2x[i], pn2y[i], ord)
#                 TaylorSeries.add!(tmp656[i], tmp655[i], pn2z[i], ord)
#                 TaylorSeries.mul!(pn2[i], newtonianCoeff[i], tmp656[i], ord)
#                 TaylorSeries.mul!(newton_acc_X[i], X[i], newtonianCoeff[i], ord)
#                 TaylorSeries.mul!(newton_acc_Y[i], Y[i], newtonianCoeff[i], ord)
#                 TaylorSeries.mul!(newton_acc_Z[i], Z[i], newtonianCoeff[i], ord)
#                 TaylorSeries.div!(newtonian1b_Potential[i], μ[i], r_p1d2[i], ord)
#                 TaylorSeries.mul!(pn3[i], 3.5, newtonian1b_Potential[i], ord)
#                 TaylorSeries.mul!(U_t_pn2[i], pn2[i], U[i], ord)
#                 TaylorSeries.mul!(V_t_pn2[i], pn2[i], V[i], ord)
#                 TaylorSeries.mul!(W_t_pn2[i], pn2[i], W[i], ord)
#                 if UJ_interaction[i]
#                     TaylorSeries.subst!(tmp667[i], X[i], ord)
#                     TaylorSeries.mul!(t31[i], tmp667[i], M_[1, 3, i], ord)
#                     TaylorSeries.subst!(tmp669[i], Y[i], ord)
#                     TaylorSeries.mul!(t32[i], tmp669[i], M_[2, 3, i], ord)
#                     TaylorSeries.subst!(tmp671[i], Z[i], ord)
#                     TaylorSeries.mul!(t33[i], tmp671[i], M_[3, 3, i], ord)
#                     TaylorSeries.add!(tmp673[i], t31[i], t32[i], ord)
#                     TaylorSeries.add!(r_sin_ϕ[i], tmp673[i], t33[i], ord)
#                     TaylorSeries.div!(sin_ϕ[i], r_sin_ϕ[i], r_p1d2[i], ord)
#                     TaylorSeries.asin!(ϕ[i], sin_ϕ[i], tmp885[i], ord)
#                     TaylorSeries.sincos!(tmp886[i], cos_ϕ[i], ϕ[i], ord)
#                     TaylorSeries.pow!(sin2_ϕ[i], sin_ϕ[i], tmp887[i], 2, ord)
#                     TaylorSeries.pow!(sin3_ϕ[i], sin_ϕ[i], tmp888[i], 3, ord)
#                     TaylorSeries.mul!(tmp683[i], 1.5, sin2_ϕ[i], ord)
#                     TaylorSeries.subst!(P_2_sin_ϕ[i], tmp683[i], 0.5, ord)
#                     TaylorSeries.mul!(∂P_2_sin_ϕ[i], 3, sin_ϕ[i], ord)
#                     TaylorSeries.mul!(tmp689[i], -1.5, sin_ϕ[i], ord)
#                     TaylorSeries.mul!(tmp691[i], 2.5, sin3_ϕ[i], ord)
#                     TaylorSeries.add!(P_3_sin_ϕ[i], tmp689[i], tmp691[i], ord)
#                     TaylorSeries.mul!(tmp695[i], 7.5, sin2_ϕ[i], ord)
#                     TaylorSeries.add!(∂P_3_sin_ϕ[i], -1.5, tmp695[i], ord)
#                     TaylorSeries.pow!(tmp698[i], r_p2[i], tmp889[i], 2, ord)
#                     TaylorSeries.div!(tmp699[i], Λ2[i], tmp698[i], ord)
#                     TaylorSeries.subst!(Λ2j_div_r4[i], tmp699[i], ord)
#                     TaylorSeries.pow!(tmp702[i], r_p1d2[i], tmp890[i], 5, ord)
#                     TaylorSeries.div!(tmp703[i], Λ3[i], tmp702[i], ord)
#                     TaylorSeries.subst!(Λ3j_div_r5[i], tmp703[i], ord)
#                     TaylorSeries.subst!(tmp705[i], cos_ϕ[i], ord)
#                     TaylorSeries.mul!(m_c_ϕ_∂P_2[i], tmp705[i], ∂P_2_sin_ϕ[i], ord)
#                     TaylorSeries.subst!(tmp707[i], cos_ϕ[i], ord)
#                     TaylorSeries.mul!(m_c_ϕ_∂P_3[i], tmp707[i], ∂P_3_sin_ϕ[i], ord)
#                     TaylorSeries.mul!(tmp710[i], Λ2j_div_r4[i], 3, ord)
#                     TaylorSeries.mul!(F_J2_ξ[i], tmp710[i], P_2_sin_ϕ[i], ord)
#                     TaylorSeries.mul!(F_J2_ζ[i], Λ2j_div_r4[i], m_c_ϕ_∂P_2[i], ord)
#                     TaylorSeries.mul!(tmp714[i], Λ3j_div_r5[i], 4, ord)
#                     TaylorSeries.mul!(F_J3_ξ[i], tmp714[i], P_3_sin_ϕ[i], ord)
#                     TaylorSeries.mul!(F_J3_ζ[i], Λ3j_div_r5[i], m_c_ϕ_∂P_3[i], ord)
#                     TaylorSeries.identity!(F_J_ξ[i], F_J2_ξ[i], ord)
#                     TaylorSeries.identity!(F_J_ζ[i], F_J2_ζ[i], ord)
#                     TaylorSeries.subst!(tmp717[i], X[i], ord)
#                     TaylorSeries.div!(ξx[i], tmp717[i], r_p1d2[i], ord)
#                     TaylorSeries.subst!(tmp719[i], Y[i], ord)
#                     TaylorSeries.div!(ξy[i], tmp719[i], r_p1d2[i], ord)
#                     TaylorSeries.subst!(tmp721[i], Z[i], ord)
#                     TaylorSeries.div!(ξz[i], tmp721[i], r_p1d2[i], ord)
#                     TaylorSeries.mul!(ηx1[i], M_[2, 3, i], ξz[i], ord)
#                     TaylorSeries.mul!(ηy1[i], M_[3, 3, i], ξx[i], ord)
#                     TaylorSeries.mul!(ηz1[i], M_[1, 3, i], ξy[i], ord)
#                     TaylorSeries.mul!(ηx2[i], M_[3, 3, i], ξy[i], ord)
#                     TaylorSeries.mul!(ηy2[i], M_[1, 3, i], ξz[i], ord)
#                     TaylorSeries.mul!(ηz2[i], M_[2, 3, i], ξx[i], ord)
#                     TaylorSeries.subst!(ηx[i], ηx1[i], ηx2[i], ord)
#                     TaylorSeries.subst!(ηy[i], ηy1[i], ηy2[i], ord)
#                     TaylorSeries.subst!(ηz[i], ηz1[i], ηz2[i], ord)
#                     TaylorSeries.mul!(ζx1[i], ξy[i], ηz[i], ord)
#                     TaylorSeries.mul!(ζy1[i], ξz[i], ηx[i], ord)
#                     TaylorSeries.mul!(ζz1[i], ξx[i], ηy[i], ord)
#                     TaylorSeries.mul!(ζx2[i], ξz[i], ηy[i], ord)
#                     TaylorSeries.mul!(ζy2[i], ξx[i], ηz[i], ord)
#                     TaylorSeries.mul!(ζz2[i], ξy[i], ηx[i], ord)
#                     TaylorSeries.subst!(ζx[i], ζx1[i], ζx2[i], ord)
#                     TaylorSeries.subst!(ζy[i], ζy1[i], ζy2[i], ord)
#                     TaylorSeries.subst!(ζz[i], ζz1[i], ζz2[i], ord)
#                     TaylorSeries.mul!(F_J2_x1[i], F_J_ξ[i], ξx[i], ord)
#                     TaylorSeries.mul!(F_J2_y1[i], F_J_ξ[i], ξy[i], ord)
#                     TaylorSeries.mul!(F_J2_z1[i], F_J_ξ[i], ξz[i], ord)
#                     TaylorSeries.mul!(F_J2_x2[i], F_J_ζ[i], ζx[i], ord)
#                     TaylorSeries.mul!(F_J2_y2[i], F_J_ζ[i], ζy[i], ord)
#                     TaylorSeries.mul!(F_J2_z2[i], F_J_ζ[i], ζz[i], ord)
#                     TaylorSeries.add!(F_J2_x[i], F_J2_x1[i], F_J2_x2[i], ord)
#                     TaylorSeries.add!(F_J2_y[i], F_J2_y1[i], F_J2_y2[i], ord)
#                     TaylorSeries.add!(F_J2_z[i], F_J2_z1[i], F_J2_z2[i], ord)
#                 end
#                 TaylorSeries.pow!(tmp751[i], ui[i], tmp891[i], 2, ord)
#                 TaylorSeries.pow!(tmp753[i], vi[i], tmp892[i], 2, ord)
#                 TaylorSeries.add!(tmp754[i], tmp751[i], tmp753[i], ord)
#                 TaylorSeries.pow!(tmp756[i], wi[i], tmp893[i], 2, ord)
#                 TaylorSeries.add!(v2[i], tmp754[i], tmp756[i], ord)
#             end
#         TaylorSeries.pow!(tmp759, q[4], tmp894, 2, ord)
#         TaylorSeries.pow!(tmp761, q[5], tmp895, 2, ord)
#         TaylorSeries.add!(tmp762, tmp759, tmp761, ord)
#         TaylorSeries.pow!(tmp764, q[6], tmp896, 2, ord)
#         TaylorSeries.add!(v2[N], tmp762, tmp764, ord)
#         for i = 1:Nm1
#             TaylorSeries.add!(temp_004[i], newtonian1b_Potential[i], newtonianNb_Potential[N], ord)
#             TaylorSeries.identity!(newtonianNb_Potential[N], temp_004[i], ord)
#             if UJ_interaction[i]
#                 TaylorSeries.mul!(tmp767[i], μ[i], F_J2_x[i], ord)
#                 TaylorSeries.subst!(temp_accX_i[i], accX, tmp767[i], ord)
#                 TaylorSeries.identity!(accX, temp_accX_i[i], ord)
#                 TaylorSeries.mul!(tmp769[i], μ[i], F_J2_y[i], ord)
#                 TaylorSeries.subst!(temp_accY_i[i], accY, tmp769[i], ord)
#                 TaylorSeries.identity!(accY, temp_accY_i[i], ord)
#                 TaylorSeries.mul!(tmp771[i], μ[i], F_J2_z[i], ord)
#                 TaylorSeries.subst!(temp_accZ_i[i], accZ, tmp771[i], ord)
#                 TaylorSeries.identity!(accZ, temp_accZ_i[i], ord)
#             end
#         end
#         TaylorSeries.mul!(_4ϕj[N], 4, newtonianNb_Potential[N], ord)
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:455 =# Threads.@threads for i = 1:10
#                 TaylorSeries.add!(ϕi_plus_4ϕj[i], newtonianNb_Potential_t[i], _4ϕj[N], ord)
#                 TaylorSeries.mul!(tmp777[i], 2, v2[i], ord)
#                 TaylorSeries.mul!(tmp779[i], 4, vi_dot_vj[i], ord)
#                 TaylorSeries.subst!(tmp780[i], tmp777[i], tmp779[i], ord)
#                 TaylorSeries.add!(sj2_plus_2si2_minus_4vivj[i], tmp780[i], v2[N], ord)
#                 TaylorSeries.subst!(ϕs_and_vs[i], sj2_plus_2si2_minus_4vivj[i], ϕi_plus_4ϕj[i], ord)
#                 TaylorSeries.mul!(Xij_t_Ui[i], X[i], ui[i], ord)
#                 TaylorSeries.mul!(Yij_t_Vi[i], Y[i], vi[i], ord)
#                 TaylorSeries.mul!(Zij_t_Wi[i], Z[i], wi[i], ord)
#                 TaylorSeries.add!(tmp786[i], Xij_t_Ui[i], Yij_t_Vi[i], ord)
#                 TaylorSeries.add!(Rij_dot_Vi[i], tmp786[i], Zij_t_Wi[i], ord)
#                 TaylorSeries.pow!(tmp789[i], Rij_dot_Vi[i], tmp897[i], 2, ord)
#                 TaylorSeries.div!(pn1t7[i], tmp789[i], r_p2[i], ord)
#                 TaylorSeries.mul!(tmp792[i], 1.5, pn1t7[i], ord)
#                 TaylorSeries.subst!(pn1t2_7[i], ϕs_and_vs[i], tmp792[i], ord)
#                 TaylorSeries.add!(pn1t1_7[i], c_p2, pn1t2_7[i], ord)
#                 TaylorSeries.mul!(pNX_t_X[i], acceph_t[3i - 2], X[i], ord)
#                 TaylorSeries.mul!(pNY_t_Y[i], acceph_t[3i - 1], Y[i], ord)
#                 TaylorSeries.mul!(pNZ_t_Z[i], acceph_t[3i], Z[i], ord)
#                 TaylorSeries.add!(tmp799[i], pNX_t_X[i], pNY_t_Y[i], ord)
#                 TaylorSeries.add!(tmp800[i], tmp799[i], pNZ_t_Z[i], ord)
#                 TaylorSeries.mul!(tmp801[i], 0.5, tmp800[i], ord)
#                 TaylorSeries.add!(pn1[i], pn1t1_7[i], tmp801[i], ord)
#                 TaylorSeries.mul!(X_t_pn1[i], newton_acc_X[i], pn1[i], ord)
#                 TaylorSeries.mul!(Y_t_pn1[i], newton_acc_Y[i], pn1[i], ord)
#                 TaylorSeries.mul!(Z_t_pn1[i], newton_acc_Z[i], pn1[i], ord)
#                 TaylorSeries.mul!(pNX_t_pn3[i], acceph_t[3i - 2], pn3[i], ord)
#                 TaylorSeries.mul!(pNY_t_pn3[i], acceph_t[3i - 1], pn3[i], ord)
#                 TaylorSeries.mul!(pNZ_t_pn3[i], acceph_t[3i], pn3[i], ord)
#             end
#         for i = 1:10
#             TaylorSeries.add!(tmp809[i], U_t_pn2[i], pNX_t_pn3[i], ord)
#             TaylorSeries.add!(termpnx[i], X_t_pn1[i], tmp809[i], ord)
#             TaylorSeries.add!(sumpnx[i], pntempX, termpnx[i], ord)
#             TaylorSeries.identity!(pntempX, sumpnx[i], ord)
#             TaylorSeries.add!(tmp812[i], V_t_pn2[i], pNY_t_pn3[i], ord)
#             TaylorSeries.add!(termpny[i], Y_t_pn1[i], tmp812[i], ord)
#             TaylorSeries.add!(sumpny[i], pntempY, termpny[i], ord)
#             TaylorSeries.identity!(pntempY, sumpny[i], ord)
#             TaylorSeries.add!(tmp815[i], W_t_pn2[i], pNZ_t_pn3[i], ord)
#             TaylorSeries.add!(termpnz[i], Z_t_pn1[i], tmp815[i], ord)
#             TaylorSeries.add!(sumpnz[i], pntempZ, termpnz[i], ord)
#             TaylorSeries.identity!(pntempZ, sumpnz[i], ord)
#         end
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:508 =# Threads.@threads for i = 11:Nm1
#                 TaylorSeries.mul!(X_t_pn1[i], c_p2, newton_acc_X[i], ord)
#                 TaylorSeries.mul!(Y_t_pn1[i], c_p2, newton_acc_Y[i], ord)
#                 TaylorSeries.mul!(Z_t_pn1[i], c_p2, newton_acc_Z[i], ord)
#             end
#         for i = 11:Nm1
#             TaylorSeries.identity!(termpnx[i], X_t_pn1[i], ord)
#             TaylorSeries.add!(sumpnx[i], pntempX, termpnx[i], ord)
#             TaylorSeries.identity!(pntempX, sumpnx[i], ord)
#             TaylorSeries.identity!(termpny[i], Y_t_pn1[i], ord)
#             TaylorSeries.add!(sumpny[i], pntempY, termpny[i], ord)
#             TaylorSeries.identity!(pntempY, sumpny[i], ord)
#             TaylorSeries.identity!(termpnz[i], Z_t_pn1[i], ord)
#             TaylorSeries.add!(sumpnz[i], pntempZ, termpnz[i], ord)
#             TaylorSeries.identity!(pntempZ, sumpnz[i], ord)
#         end
#         TaylorSeries.mul!(postNewtonX, pntempX, c_m2, ord)
#         TaylorSeries.mul!(postNewtonY, pntempY, c_m2, ord)
#         TaylorSeries.mul!(postNewtonZ, pntempZ, c_m2, ord)
#         TaylorSeries.mul!(tmp827, Y[1], W[1], ord)
#         TaylorSeries.mul!(tmp828, Z[1], V[1], ord)
#         TaylorSeries.subst!(hx, tmp827, tmp828, ord)
#         TaylorSeries.mul!(tmp830, Z[1], U[1], ord)
#         TaylorSeries.mul!(tmp831, X[1], W[1], ord)
#         TaylorSeries.subst!(hy, tmp830, tmp831, ord)
#         TaylorSeries.mul!(tmp833, X[1], V[1], ord)
#         TaylorSeries.mul!(tmp834, Y[1], U[1], ord)
#         TaylorSeries.subst!(hz, tmp833, tmp834, ord)
#         TaylorSeries.mul!(tmp836, hz, Y[1], ord)
#         TaylorSeries.mul!(tmp837, hy, Z[1], ord)
#         TaylorSeries.subst!(t_x, tmp836, tmp837, ord)
#         TaylorSeries.mul!(tmp839, hx, Z[1], ord)
#         TaylorSeries.mul!(tmp840, hz, X[1], ord)
#         TaylorSeries.subst!(t_y, tmp839, tmp840, ord)
#         TaylorSeries.mul!(tmp842, hy, X[1], ord)
#         TaylorSeries.mul!(tmp843, hx, Y[1], ord)
#         TaylorSeries.subst!(t_z, tmp842, tmp843, ord)
#         TaylorSeries.pow!(tmp846, t_x, tmp898, 2, ord)
#         TaylorSeries.pow!(tmp848, t_y, tmp899, 2, ord)
#         TaylorSeries.add!(tmp849, tmp846, tmp848, ord)
#         TaylorSeries.pow!(tmp851, t_z, tmp900, 2, ord)
#         TaylorSeries.add!(tmp852, tmp849, tmp851, ord)
#         TaylorSeries.sqrt!(t_norm, tmp852, ord)
#         TaylorSeries.div!(t_x_unit, t_x, t_norm, ord)
#         TaylorSeries.div!(t_y_unit, t_y, t_norm, ord)
#         TaylorSeries.div!(t_z_unit, t_z, t_norm, ord)
#         TaylorSeries.div!(tmp857, X[1], r_p1d2[1], ord)
#         TaylorSeries.subst!(r_x_unit, tmp857, ord)
#         TaylorSeries.div!(tmp859, Y[1], r_p1d2[1], ord)
#         TaylorSeries.subst!(r_y_unit, tmp859, ord)
#         TaylorSeries.div!(tmp861, Z[1], r_p1d2[1], ord)
#         TaylorSeries.subst!(r_z_unit, tmp861, ord)
#         TaylorSeries.identity!(g_r, r_p2[1], ord)
#         TaylorSeries.div!(A2_t_g_r, q[7], g_r, ord)
#         TaylorSeries.div!(A1_t_g_r, q[8], g_r, ord)
#         TaylorSeries.mul!(tmp865, A2_t_g_r, t_x_unit, ord)
#         TaylorSeries.mul!(tmp866, A1_t_g_r, r_x_unit, ord)
#         TaylorSeries.add!(NGAx, tmp865, tmp866, ord)
#         TaylorSeries.mul!(tmp868, A2_t_g_r, t_y_unit, ord)
#         TaylorSeries.mul!(tmp869, A1_t_g_r, r_y_unit, ord)
#         TaylorSeries.add!(NGAy, tmp868, tmp869, ord)
#         TaylorSeries.mul!(tmp871, A2_t_g_r, t_z_unit, ord)
#         TaylorSeries.mul!(tmp872, A1_t_g_r, r_z_unit, ord)
#         TaylorSeries.add!(NGAz, tmp871, tmp872, ord)
#         TaylorSeries.add!(tmp874, postNewtonX, accX, ord)
#         TaylorSeries.add!(dq[4], tmp874, NGAx, ord)
#         TaylorSeries.add!(tmp876, postNewtonY, accY, ord)
#         TaylorSeries.add!(dq[5], tmp876, NGAy, ord)
#         TaylorSeries.add!(tmp878, postNewtonZ, accZ, ord)
#         TaylorSeries.add!(dq[6], tmp878, NGAz, ord)
#         TaylorSeries.identity!(dq[7], zero_q_1, ord)
#         TaylorSeries.identity!(dq[8], zero_q_1, ord)
#         for __idx = eachindex(q)
#             TaylorIntegration.diffeq!(q[__idx], dq[__idx], ordnext)
#         end
#     end
#     return nothing
# end

# # TaylorIntegration._allocate_jetcoeffs! method for src/propagation/asteroid_dynamical_models.jl: RNp1BP_pN_A_J23E_J2S_eph_threads!
# function TaylorIntegration._allocate_jetcoeffs!(::Val{RNp1BP_pN_A_J23E_J2S_eph_threads!}, t::Taylor1{_T}, q::AbstractArray{Taylor1{_S}, _N}, dq::AbstractArray{Taylor1{_S}, _N}, params) where {_T <: Real, _S <: Number, _N}
#     order = t.order
#     local jd0 = params.jd0
#     local dsj2k = t + (jd0 - JD_J2000)
#     local ss16asteph_t = evaleph(params.sseph, dsj2k, q[1])
#     local acceph_t = evaleph(params.acceph, dsj2k, q[1])
#     local newtonianNb_Potential_t = evaleph(params.poteph, dsj2k, q[1])
#     local S = eltype(q)
#     local UJ_interaction = params.UJ_interaction
#     local N = params.N
#     local Nm1 = N - 1
#     local μ = params.μ
#     local zero_q_1 = auxzero(q[1])
#     X = Array{S}(undef, N)
#     Y = Array{S}(undef, N)
#     Z = Array{S}(undef, N)
#     r_p2 = Array{S}(undef, N)
#     r_p1d2 = Array{S}(undef, N)
#     r_p3d2 = Array{S}(undef, N)
#     r_p7d2 = Array{S}(undef, N)
#     newtonianCoeff = Array{S}(undef, N)
#     ui = Array{S}(undef, N - 1)
#     vi = Array{S}(undef, N - 1)
#     wi = Array{S}(undef, N - 1)
#     U = Array{S}(undef, N)
#     V = Array{S}(undef, N)
#     W = Array{S}(undef, N)
#     _4dq = Array{S}(undef, 3)
#     _4U_m_3X = Array{S}(undef, N)
#     _4V_m_3Y = Array{S}(undef, N)
#     _4W_m_3Z = Array{S}(undef, N)
#     UU = Array{S}(undef, N)
#     VV = Array{S}(undef, N)
#     WW = Array{S}(undef, N)
#     newtonian1b_Potential = Array{S}(undef, N)
#     newtonianNb_Potential = Array{S}(undef, N)
#     newton_acc_X = Array{S}(undef, N)
#     newton_acc_Y = Array{S}(undef, N)
#     newton_acc_Z = Array{S}(undef, N)
#     v2 = Array{S}(undef, N)
#     vi_dot_vj = Array{S}(undef, N)
#     pn2 = Array{S}(undef, N)
#     U_t_pn2 = Array{S}(undef, N)
#     V_t_pn2 = Array{S}(undef, N)
#     W_t_pn2 = Array{S}(undef, N)
#     pn3 = Array{S}(undef, N)
#     pNX_t_pn3 = Array{S}(undef, N)
#     pNY_t_pn3 = Array{S}(undef, N)
#     pNZ_t_pn3 = Array{S}(undef, N)
#     _4ϕj = Array{S}(undef, N)
#     ϕi_plus_4ϕj = Array{S}(undef, N)
#     sj2_plus_2si2_minus_4vivj = Array{S}(undef, N)
#     ϕs_and_vs = Array{S}(undef, N)
#     pn1t1_7 = Array{S}(undef, N)
#     pNX_t_X = Array{S}(undef, N)
#     pNY_t_Y = Array{S}(undef, N)
#     pNZ_t_Z = Array{S}(undef, N)
#     pn1 = Array{S}(undef, N)
#     X_t_pn1 = Array{S}(undef, N)
#     Y_t_pn1 = Array{S}(undef, N)
#     Z_t_pn1 = Array{S}(undef, N)
#     pntempX = Taylor1(identity(constant_term(zero_q_1)), order)
#     pntempY = Taylor1(identity(constant_term(zero_q_1)), order)
#     pntempZ = Taylor1(identity(constant_term(zero_q_1)), order)
#     t31 = Array{S}(undef, N)
#     t32 = Array{S}(undef, N)
#     t33 = Array{S}(undef, N)
#     r_sin_ϕ = Array{S}(undef, N)
#     sin_ϕ = Array{S}(undef, N)
#     ϕ = Array{S}(undef, N)
#     cos_ϕ = Array{S}(undef, N)
#     sin2_ϕ = Array{S}(undef, N)
#     sin3_ϕ = Array{S}(undef, N)
#     sin4_ϕ = Array{S}(undef, N)
#     F_J2_x = Array{S}(undef, N)
#     F_J2_y = Array{S}(undef, N)
#     F_J2_z = Array{S}(undef, N)
#     F_J2_x1 = Array{S}(undef, N)
#     F_J2_y1 = Array{S}(undef, N)
#     F_J2_z1 = Array{S}(undef, N)
#     F_J2_x2 = Array{S}(undef, N)
#     F_J2_y2 = Array{S}(undef, N)
#     F_J2_z2 = Array{S}(undef, N)
#     temp_accX_i = Array{S}(undef, N)
#     temp_accY_i = Array{S}(undef, N)
#     temp_accZ_i = Array{S}(undef, N)
#     P_2_sin_ϕ = Array{S}(undef, N)
#     ∂P_2_sin_ϕ = Array{S}(undef, N)
#     P_3_sin_ϕ = Array{S}(undef, N)
#     ∂P_3_sin_ϕ = Array{S}(undef, N)
#     m_c_ϕ_∂P_2 = Array{S}(undef, N)
#     m_c_ϕ_∂P_3 = Array{S}(undef, N)
#     Λ2j_div_r4 = Array{S}(undef, N)
#     Λ3j_div_r5 = Array{S}(undef, N)
#     F_J_ξ = Array{S}(undef, N)
#     F_J_η = Array{S}(undef, N)
#     F_J_ζ = Array{S}(undef, N)
#     F_J2_ξ = Array{S}(undef, N)
#     F_J2_η = Array{S}(undef, N)
#     F_J2_ζ = Array{S}(undef, N)
#     F_J3_ξ = Array{S}(undef, N)
#     F_J3_η = Array{S}(undef, N)
#     F_J3_ζ = Array{S}(undef, N)
#     ξx = Array{S}(undef, N)
#     ξy = Array{S}(undef, N)
#     ξz = Array{S}(undef, N)
#     ηx = Array{S}(undef, N)
#     ηy = Array{S}(undef, N)
#     ηz = Array{S}(undef, N)
#     ηx1 = Array{S}(undef, N)
#     ηy1 = Array{S}(undef, N)
#     ηz1 = Array{S}(undef, N)
#     ηx2 = Array{S}(undef, N)
#     ηy2 = Array{S}(undef, N)
#     ηz2 = Array{S}(undef, N)
#     ζx = Array{S}(undef, N)
#     ζy = Array{S}(undef, N)
#     ζz = Array{S}(undef, N)
#     ζx1 = Array{S}(undef, N)
#     ζy1 = Array{S}(undef, N)
#     ζz1 = Array{S}(undef, N)
#     ζx2 = Array{S}(undef, N)
#     ζy2 = Array{S}(undef, N)
#     ζz2 = Array{S}(undef, N)
#     accX = Taylor1(identity(constant_term(zero_q_1)), order)
#     accY = Taylor1(identity(constant_term(zero_q_1)), order)
#     accZ = Taylor1(identity(constant_term(zero_q_1)), order)
#     local M_ = Array{S}(undef, 3, 3, N)
#     local M_[:, :, ea] = t2c_jpl_de430(dsj2k) .+ zero_q_1
#     dq[1] = Taylor1(identity(constant_term(q[4])), order)
#     dq[2] = Taylor1(identity(constant_term(q[5])), order)
#     dq[3] = Taylor1(identity(constant_term(q[6])), order)
#     newtonianNb_Potential[N] = Taylor1(identity(constant_term(zero_q_1)), order)
#     _4dq[1] = Taylor1(constant_term(4) * constant_term(dq[1]), order)
#     _4dq[2] = Taylor1(constant_term(4) * constant_term(dq[2]), order)
#     _4dq[3] = Taylor1(constant_term(4) * constant_term(dq[3]), order)
#     tmp1272 = Array{Taylor1{_S}}(undef, size(ui))
#     for i = eachindex(tmp1272)
#         tmp1272[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1275 = Array{Taylor1{_S}}(undef, size(vi))
#     for i = eachindex(tmp1275)
#         tmp1275[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1278 = Array{Taylor1{_S}}(undef, size(wi))
#     for i = eachindex(tmp1278)
#         tmp1278[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn2x = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(pn2x)
#         pn2x[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn2y = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(pn2y)
#         pn2y[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn2z = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(pn2z)
#         pn2z[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1286 = Array{Taylor1{_S}}(undef, size(UU))
#     for i = eachindex(tmp1286)
#         tmp1286[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1289 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp1289)
#         tmp1289[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1477 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp1477)
#         tmp1477[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1291 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp1291)
#         tmp1291[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1478 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp1478)
#         tmp1478[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1292 = Array{Taylor1{_S}}(undef, size(tmp1289))
#     for i = eachindex(tmp1292)
#         tmp1292[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1294 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp1294)
#         tmp1294[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1479 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp1479)
#         tmp1479[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1480 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp1480)
#         tmp1480[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1481 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp1481)
#         tmp1481[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1302 = Array{Taylor1{_S}}(undef, size(pn2x))
#     for i = eachindex(tmp1302)
#         tmp1302[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1303 = Array{Taylor1{_S}}(undef, size(tmp1302))
#     for i = eachindex(tmp1303)
#         tmp1303[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1398 = Array{Taylor1{_S}}(undef, size(ui))
#     for i = eachindex(tmp1398)
#         tmp1398[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1488 = Array{Taylor1{_S}}(undef, size(ui))
#     for i = eachindex(tmp1488)
#         tmp1488[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1400 = Array{Taylor1{_S}}(undef, size(vi))
#     for i = eachindex(tmp1400)
#         tmp1400[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1489 = Array{Taylor1{_S}}(undef, size(vi))
#     for i = eachindex(tmp1489)
#         tmp1489[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1401 = Array{Taylor1{_S}}(undef, size(tmp1398))
#     for i = eachindex(tmp1401)
#         tmp1401[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1403 = Array{Taylor1{_S}}(undef, size(wi))
#     for i = eachindex(tmp1403)
#         tmp1403[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1490 = Array{Taylor1{_S}}(undef, size(wi))
#     for i = eachindex(tmp1490)
#         tmp1490[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1314 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp1314)
#         tmp1314[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1316 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp1316)
#         tmp1316[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1318 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp1318)
#         tmp1318[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1320 = Array{Taylor1{_S}}(undef, size(t31))
#     for i = eachindex(tmp1320)
#         tmp1320[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1482 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp1482)
#         tmp1482[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1483 = Array{Taylor1{_S}}(undef, size(ϕ))
#     for i = eachindex(tmp1483)
#         tmp1483[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1484 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp1484)
#         tmp1484[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1485 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp1485)
#         tmp1485[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1330 = Array{Taylor1{_S}}(undef, size(sin2_ϕ))
#     for i = eachindex(tmp1330)
#         tmp1330[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1336 = Array{Taylor1{_S}}(undef, size(sin_ϕ))
#     for i = eachindex(tmp1336)
#         tmp1336[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1338 = Array{Taylor1{_S}}(undef, size(sin3_ϕ))
#     for i = eachindex(tmp1338)
#         tmp1338[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1342 = Array{Taylor1{_S}}(undef, size(sin2_ϕ))
#     for i = eachindex(tmp1342)
#         tmp1342[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1345 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp1345)
#         tmp1345[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1486 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp1486)
#         tmp1486[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1346 = Array{Taylor1{_S}}(undef, size(Λ2))
#     for i = eachindex(tmp1346)
#         tmp1346[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1349 = Array{Taylor1{_S}}(undef, size(r_p1d2))
#     for i = eachindex(tmp1349)
#         tmp1349[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1487 = Array{Taylor1{_S}}(undef, size(r_p1d2))
#     for i = eachindex(tmp1487)
#         tmp1487[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1350 = Array{Taylor1{_S}}(undef, size(Λ3))
#     for i = eachindex(tmp1350)
#         tmp1350[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1352 = Array{Taylor1{_S}}(undef, size(cos_ϕ))
#     for i = eachindex(tmp1352)
#         tmp1352[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1354 = Array{Taylor1{_S}}(undef, size(cos_ϕ))
#     for i = eachindex(tmp1354)
#         tmp1354[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1357 = Array{Taylor1{_S}}(undef, size(Λ2j_div_r4))
#     for i = eachindex(tmp1357)
#         tmp1357[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1361 = Array{Taylor1{_S}}(undef, size(Λ3j_div_r5))
#     for i = eachindex(tmp1361)
#         tmp1361[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1364 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp1364)
#         tmp1364[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1366 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp1366)
#         tmp1366[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1368 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp1368)
#         tmp1368[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:832 =# Threads.@threads for i = 1:Nm1
#             ui[i] = Taylor1(identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 2])), order)
#             vi[i] = Taylor1(identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 1])), order)
#             wi[i] = Taylor1(identity(constant_term(ss16asteph_t[3 * ((N - 1) + i)])), order)
#             X[i] = Taylor1(constant_term(ss16asteph_t[3i - 2]) - constant_term(q[1]), order)
#             Y[i] = Taylor1(constant_term(ss16asteph_t[3i - 1]) - constant_term(q[2]), order)
#             Z[i] = Taylor1(constant_term(ss16asteph_t[3i]) - constant_term(q[3]), order)
#             U[i] = Taylor1(constant_term(ui[i]) - constant_term(dq[1]), order)
#             V[i] = Taylor1(constant_term(vi[i]) - constant_term(dq[2]), order)
#             W[i] = Taylor1(constant_term(wi[i]) - constant_term(dq[3]), order)
#             tmp1272[i] = Taylor1(constant_term(-3) * constant_term(ui[i]), order)
#             _4U_m_3X[i] = Taylor1(constant_term(tmp1272[i]) + constant_term(_4dq[1]), order)
#             tmp1275[i] = Taylor1(constant_term(-3) * constant_term(vi[i]), order)
#             _4V_m_3Y[i] = Taylor1(constant_term(tmp1275[i]) + constant_term(_4dq[2]), order)
#             tmp1278[i] = Taylor1(constant_term(-3) * constant_term(wi[i]), order)
#             _4W_m_3Z[i] = Taylor1(constant_term(tmp1278[i]) + constant_term(_4dq[3]), order)
#             pn2x[i] = Taylor1(constant_term(X[i]) * constant_term(_4U_m_3X[i]), order)
#             pn2y[i] = Taylor1(constant_term(Y[i]) * constant_term(_4V_m_3Y[i]), order)
#             pn2z[i] = Taylor1(constant_term(Z[i]) * constant_term(_4W_m_3Z[i]), order)
#             UU[i] = Taylor1(constant_term(ui[i]) * constant_term(dq[1]), order)
#             VV[i] = Taylor1(constant_term(vi[i]) * constant_term(dq[2]), order)
#             WW[i] = Taylor1(constant_term(wi[i]) * constant_term(dq[3]), order)
#             tmp1286[i] = Taylor1(constant_term(UU[i]) + constant_term(VV[i]), order)
#             vi_dot_vj[i] = Taylor1(constant_term(tmp1286[i]) + constant_term(WW[i]), order)
#             tmp1289[i] = Taylor1(constant_term(X[i]) ^ float(constant_term(2)), order)
#             tmp1477[i] = Taylor1(zero(constant_term(X[i])), order)
#             tmp1291[i] = Taylor1(constant_term(Y[i]) ^ float(constant_term(2)), order)
#             tmp1478[i] = Taylor1(zero(constant_term(Y[i])), order)
#             tmp1292[i] = Taylor1(constant_term(tmp1289[i]) + constant_term(tmp1291[i]), order)
#             tmp1294[i] = Taylor1(constant_term(Z[i]) ^ float(constant_term(2)), order)
#             tmp1479[i] = Taylor1(zero(constant_term(Z[i])), order)
#             r_p2[i] = Taylor1(constant_term(tmp1292[i]) + constant_term(tmp1294[i]), order)
#             r_p1d2[i] = Taylor1(sqrt(constant_term(r_p2[i])), order)
#             r_p3d2[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(1.5)), order)
#             tmp1480[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#             r_p7d2[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(3.5)), order)
#             tmp1481[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#             newtonianCoeff[i] = Taylor1(constant_term(μ[i]) / constant_term(r_p3d2[i]), order)
#             tmp1302[i] = Taylor1(constant_term(pn2x[i]) + constant_term(pn2y[i]), order)
#             tmp1303[i] = Taylor1(constant_term(tmp1302[i]) + constant_term(pn2z[i]), order)
#             pn2[i] = Taylor1(constant_term(newtonianCoeff[i]) * constant_term(tmp1303[i]), order)
#             newton_acc_X[i] = Taylor1(constant_term(X[i]) * constant_term(newtonianCoeff[i]), order)
#             newton_acc_Y[i] = Taylor1(constant_term(Y[i]) * constant_term(newtonianCoeff[i]), order)
#             newton_acc_Z[i] = Taylor1(constant_term(Z[i]) * constant_term(newtonianCoeff[i]), order)
#             newtonian1b_Potential[i] = Taylor1(constant_term(μ[i]) / constant_term(r_p1d2[i]), order)
#             pn3[i] = Taylor1(constant_term(3.5) * constant_term(newtonian1b_Potential[i]), order)
#             U_t_pn2[i] = Taylor1(constant_term(pn2[i]) * constant_term(U[i]), order)
#             V_t_pn2[i] = Taylor1(constant_term(pn2[i]) * constant_term(V[i]), order)
#             W_t_pn2[i] = Taylor1(constant_term(pn2[i]) * constant_term(W[i]), order)
#             if UJ_interaction[i]
#                 tmp1314[i] = Taylor1(-(constant_term(X[i])), order)
#                 t31[i] = Taylor1(constant_term(tmp1314[i]) * constant_term(M_[1, 3, i]), order)
#                 tmp1316[i] = Taylor1(-(constant_term(Y[i])), order)
#                 t32[i] = Taylor1(constant_term(tmp1316[i]) * constant_term(M_[2, 3, i]), order)
#                 tmp1318[i] = Taylor1(-(constant_term(Z[i])), order)
#                 t33[i] = Taylor1(constant_term(tmp1318[i]) * constant_term(M_[3, 3, i]), order)
#                 tmp1320[i] = Taylor1(constant_term(t31[i]) + constant_term(t32[i]), order)
#                 r_sin_ϕ[i] = Taylor1(constant_term(tmp1320[i]) + constant_term(t33[i]), order)
#                 sin_ϕ[i] = Taylor1(constant_term(r_sin_ϕ[i]) / constant_term(r_p1d2[i]), order)
#                 ϕ[i] = Taylor1(asin(constant_term(sin_ϕ[i])), order)
#                 tmp1482[i] = Taylor1(sqrt(1 - constant_term(sin_ϕ[i]) ^ 2), order)
#                 cos_ϕ[i] = Taylor1(cos(constant_term(ϕ[i])), order)
#                 tmp1483[i] = Taylor1(sin(constant_term(ϕ[i])), order)
#                 sin2_ϕ[i] = Taylor1(constant_term(sin_ϕ[i]) ^ float(constant_term(2)), order)
#                 tmp1484[i] = Taylor1(zero(constant_term(sin_ϕ[i])), order)
#                 sin3_ϕ[i] = Taylor1(constant_term(sin_ϕ[i]) ^ float(constant_term(3)), order)
#                 tmp1485[i] = Taylor1(zero(constant_term(sin_ϕ[i])), order)
#                 tmp1330[i] = Taylor1(constant_term(1.5) * constant_term(sin2_ϕ[i]), order)
#                 P_2_sin_ϕ[i] = Taylor1(constant_term(tmp1330[i]) - constant_term(0.5), order)
#                 ∂P_2_sin_ϕ[i] = Taylor1(constant_term(3) * constant_term(sin_ϕ[i]), order)
#                 tmp1336[i] = Taylor1(constant_term(-1.5) * constant_term(sin_ϕ[i]), order)
#                 tmp1338[i] = Taylor1(constant_term(2.5) * constant_term(sin3_ϕ[i]), order)
#                 P_3_sin_ϕ[i] = Taylor1(constant_term(tmp1336[i]) + constant_term(tmp1338[i]), order)
#                 tmp1342[i] = Taylor1(constant_term(7.5) * constant_term(sin2_ϕ[i]), order)
#                 ∂P_3_sin_ϕ[i] = Taylor1(constant_term(-1.5) + constant_term(tmp1342[i]), order)
#                 tmp1345[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(2)), order)
#                 tmp1486[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#                 tmp1346[i] = Taylor1(constant_term(Λ2[i]) / constant_term(tmp1345[i]), order)
#                 Λ2j_div_r4[i] = Taylor1(-(constant_term(tmp1346[i])), order)
#                 tmp1349[i] = Taylor1(constant_term(r_p1d2[i]) ^ float(constant_term(5)), order)
#                 tmp1487[i] = Taylor1(zero(constant_term(r_p1d2[i])), order)
#                 tmp1350[i] = Taylor1(constant_term(Λ3[i]) / constant_term(tmp1349[i]), order)
#                 Λ3j_div_r5[i] = Taylor1(-(constant_term(tmp1350[i])), order)
#                 tmp1352[i] = Taylor1(-(constant_term(cos_ϕ[i])), order)
#                 m_c_ϕ_∂P_2[i] = Taylor1(constant_term(tmp1352[i]) * constant_term(∂P_2_sin_ϕ[i]), order)
#                 tmp1354[i] = Taylor1(-(constant_term(cos_ϕ[i])), order)
#                 m_c_ϕ_∂P_3[i] = Taylor1(constant_term(tmp1354[i]) * constant_term(∂P_3_sin_ϕ[i]), order)
#                 tmp1357[i] = Taylor1(constant_term(Λ2j_div_r4[i]) * constant_term(3), order)
#                 F_J2_ξ[i] = Taylor1(constant_term(tmp1357[i]) * constant_term(P_2_sin_ϕ[i]), order)
#                 F_J2_ζ[i] = Taylor1(constant_term(Λ2j_div_r4[i]) * constant_term(m_c_ϕ_∂P_2[i]), order)
#                 tmp1361[i] = Taylor1(constant_term(Λ3j_div_r5[i]) * constant_term(4), order)
#                 F_J3_ξ[i] = Taylor1(constant_term(tmp1361[i]) * constant_term(P_3_sin_ϕ[i]), order)
#                 F_J3_ζ[i] = Taylor1(constant_term(Λ3j_div_r5[i]) * constant_term(m_c_ϕ_∂P_3[i]), order)
#                 F_J_ξ[i] = Taylor1(identity(constant_term(F_J2_ξ[i])), order)
#                 F_J_ζ[i] = Taylor1(identity(constant_term(F_J2_ζ[i])), order)
#                 tmp1364[i] = Taylor1(-(constant_term(X[i])), order)
#                 ξx[i] = Taylor1(constant_term(tmp1364[i]) / constant_term(r_p1d2[i]), order)
#                 tmp1366[i] = Taylor1(-(constant_term(Y[i])), order)
#                 ξy[i] = Taylor1(constant_term(tmp1366[i]) / constant_term(r_p1d2[i]), order)
#                 tmp1368[i] = Taylor1(-(constant_term(Z[i])), order)
#                 ξz[i] = Taylor1(constant_term(tmp1368[i]) / constant_term(r_p1d2[i]), order)
#                 ηx1[i] = Taylor1(constant_term(M_[2, 3, i]) * constant_term(ξz[i]), order)
#                 ηy1[i] = Taylor1(constant_term(M_[3, 3, i]) * constant_term(ξx[i]), order)
#                 ηz1[i] = Taylor1(constant_term(M_[1, 3, i]) * constant_term(ξy[i]), order)
#                 ηx2[i] = Taylor1(constant_term(M_[3, 3, i]) * constant_term(ξy[i]), order)
#                 ηy2[i] = Taylor1(constant_term(M_[1, 3, i]) * constant_term(ξz[i]), order)
#                 ηz2[i] = Taylor1(constant_term(M_[2, 3, i]) * constant_term(ξx[i]), order)
#                 ηx[i] = Taylor1(constant_term(ηx1[i]) - constant_term(ηx2[i]), order)
#                 ηy[i] = Taylor1(constant_term(ηy1[i]) - constant_term(ηy2[i]), order)
#                 ηz[i] = Taylor1(constant_term(ηz1[i]) - constant_term(ηz2[i]), order)
#                 ζx1[i] = Taylor1(constant_term(ξy[i]) * constant_term(ηz[i]), order)
#                 ζy1[i] = Taylor1(constant_term(ξz[i]) * constant_term(ηx[i]), order)
#                 ζz1[i] = Taylor1(constant_term(ξx[i]) * constant_term(ηy[i]), order)
#                 ζx2[i] = Taylor1(constant_term(ξz[i]) * constant_term(ηy[i]), order)
#                 ζy2[i] = Taylor1(constant_term(ξx[i]) * constant_term(ηz[i]), order)
#                 ζz2[i] = Taylor1(constant_term(ξy[i]) * constant_term(ηx[i]), order)
#                 ζx[i] = Taylor1(constant_term(ζx1[i]) - constant_term(ζx2[i]), order)
#                 ζy[i] = Taylor1(constant_term(ζy1[i]) - constant_term(ζy2[i]), order)
#                 ζz[i] = Taylor1(constant_term(ζz1[i]) - constant_term(ζz2[i]), order)
#                 F_J2_x1[i] = Taylor1(constant_term(F_J_ξ[i]) * constant_term(ξx[i]), order)
#                 F_J2_y1[i] = Taylor1(constant_term(F_J_ξ[i]) * constant_term(ξy[i]), order)
#                 F_J2_z1[i] = Taylor1(constant_term(F_J_ξ[i]) * constant_term(ξz[i]), order)
#                 F_J2_x2[i] = Taylor1(constant_term(F_J_ζ[i]) * constant_term(ζx[i]), order)
#                 F_J2_y2[i] = Taylor1(constant_term(F_J_ζ[i]) * constant_term(ζy[i]), order)
#                 F_J2_z2[i] = Taylor1(constant_term(F_J_ζ[i]) * constant_term(ζz[i]), order)
#                 F_J2_x[i] = Taylor1(constant_term(F_J2_x1[i]) + constant_term(F_J2_x2[i]), order)
#                 F_J2_y[i] = Taylor1(constant_term(F_J2_y1[i]) + constant_term(F_J2_y2[i]), order)
#                 F_J2_z[i] = Taylor1(constant_term(F_J2_z1[i]) + constant_term(F_J2_z2[i]), order)
#             end
#             tmp1398[i] = Taylor1(constant_term(ui[i]) ^ float(constant_term(2)), order)
#             tmp1488[i] = Taylor1(zero(constant_term(ui[i])), order)
#             tmp1400[i] = Taylor1(constant_term(vi[i]) ^ float(constant_term(2)), order)
#             tmp1489[i] = Taylor1(zero(constant_term(vi[i])), order)
#             tmp1401[i] = Taylor1(constant_term(tmp1398[i]) + constant_term(tmp1400[i]), order)
#             tmp1403[i] = Taylor1(constant_term(wi[i]) ^ float(constant_term(2)), order)
#             tmp1490[i] = Taylor1(zero(constant_term(wi[i])), order)
#             v2[i] = Taylor1(constant_term(tmp1401[i]) + constant_term(tmp1403[i]), order)
#         end
#     tmp1406 = Taylor1(constant_term(q[4]) ^ float(constant_term(2)), order)
#     tmp1491 = Taylor1(zero(constant_term(q[4])), order)
#     tmp1408 = Taylor1(constant_term(q[5]) ^ float(constant_term(2)), order)
#     tmp1492 = Taylor1(zero(constant_term(q[5])), order)
#     tmp1409 = Taylor1(constant_term(tmp1406) + constant_term(tmp1408), order)
#     tmp1411 = Taylor1(constant_term(q[6]) ^ float(constant_term(2)), order)
#     tmp1493 = Taylor1(zero(constant_term(q[6])), order)
#     v2[N] = Taylor1(constant_term(tmp1409) + constant_term(tmp1411), order)
#     temp_004 = Array{Taylor1{_S}}(undef, size(newtonian1b_Potential))
#     for i = eachindex(temp_004)
#         temp_004[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1414 = Array{Taylor1{_S}}(undef, size(μ))
#     for i = eachindex(tmp1414)
#         tmp1414[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1416 = Array{Taylor1{_S}}(undef, size(μ))
#     for i = eachindex(tmp1416)
#         tmp1416[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1418 = Array{Taylor1{_S}}(undef, size(μ))
#     for i = eachindex(tmp1418)
#         tmp1418[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     for i = 1:Nm1
#         temp_004[i] = Taylor1(constant_term(newtonian1b_Potential[i]) + constant_term(newtonianNb_Potential[N]), order)
#         newtonianNb_Potential[N] = Taylor1(identity(constant_term(temp_004[i])), order)
#         if UJ_interaction[i]
#             tmp1414[i] = Taylor1(constant_term(μ[i]) * constant_term(F_J2_x[i]), order)
#             temp_accX_i[i] = Taylor1(constant_term(accX) - constant_term(tmp1414[i]), order)
#             accX = Taylor1(identity(constant_term(temp_accX_i[i])), order)
#             tmp1416[i] = Taylor1(constant_term(μ[i]) * constant_term(F_J2_y[i]), order)
#             temp_accY_i[i] = Taylor1(constant_term(accY) - constant_term(tmp1416[i]), order)
#             accY = Taylor1(identity(constant_term(temp_accY_i[i])), order)
#             tmp1418[i] = Taylor1(constant_term(μ[i]) * constant_term(F_J2_z[i]), order)
#             temp_accZ_i[i] = Taylor1(constant_term(accZ) - constant_term(tmp1418[i]), order)
#             accZ = Taylor1(identity(constant_term(temp_accZ_i[i])), order)
#         end
#     end
#     _4ϕj[N] = Taylor1(constant_term(4) * constant_term(newtonianNb_Potential[N]), order)
#     tmp1424 = Array{Taylor1{_S}}(undef, size(v2))
#     for i = eachindex(tmp1424)
#         tmp1424[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1426 = Array{Taylor1{_S}}(undef, size(vi_dot_vj))
#     for i = eachindex(tmp1426)
#         tmp1426[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1427 = Array{Taylor1{_S}}(undef, size(tmp1424))
#     for i = eachindex(tmp1427)
#         tmp1427[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Xij_t_Ui = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(Xij_t_Ui)
#         Xij_t_Ui[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Yij_t_Vi = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(Yij_t_Vi)
#         Yij_t_Vi[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Zij_t_Wi = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(Zij_t_Wi)
#         Zij_t_Wi[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1433 = Array{Taylor1{_S}}(undef, size(Xij_t_Ui))
#     for i = eachindex(tmp1433)
#         tmp1433[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     Rij_dot_Vi = Array{Taylor1{_S}}(undef, size(tmp1433))
#     for i = eachindex(Rij_dot_Vi)
#         Rij_dot_Vi[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1436 = Array{Taylor1{_S}}(undef, size(Rij_dot_Vi))
#     for i = eachindex(tmp1436)
#         tmp1436[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1494 = Array{Taylor1{_S}}(undef, size(Rij_dot_Vi))
#     for i = eachindex(tmp1494)
#         tmp1494[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn1t7 = Array{Taylor1{_S}}(undef, size(tmp1436))
#     for i = eachindex(pn1t7)
#         pn1t7[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1439 = Array{Taylor1{_S}}(undef, size(pn1t7))
#     for i = eachindex(tmp1439)
#         tmp1439[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     pn1t2_7 = Array{Taylor1{_S}}(undef, size(ϕs_and_vs))
#     for i = eachindex(pn1t2_7)
#         pn1t2_7[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1446 = Array{Taylor1{_S}}(undef, size(pNX_t_X))
#     for i = eachindex(tmp1446)
#         tmp1446[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1447 = Array{Taylor1{_S}}(undef, size(tmp1446))
#     for i = eachindex(tmp1447)
#         tmp1447[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1448 = Array{Taylor1{_S}}(undef, size(tmp1447))
#     for i = eachindex(tmp1448)
#         tmp1448[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1034 =# Threads.@threads for i = 1:10
#             ϕi_plus_4ϕj[i] = Taylor1(constant_term(newtonianNb_Potential_t[i]) + constant_term(_4ϕj[N]), order)
#             tmp1424[i] = Taylor1(constant_term(2) * constant_term(v2[i]), order)
#             tmp1426[i] = Taylor1(constant_term(4) * constant_term(vi_dot_vj[i]), order)
#             tmp1427[i] = Taylor1(constant_term(tmp1424[i]) - constant_term(tmp1426[i]), order)
#             sj2_plus_2si2_minus_4vivj[i] = Taylor1(constant_term(tmp1427[i]) + constant_term(v2[N]), order)
#             ϕs_and_vs[i] = Taylor1(constant_term(sj2_plus_2si2_minus_4vivj[i]) - constant_term(ϕi_plus_4ϕj[i]), order)
#             Xij_t_Ui[i] = Taylor1(constant_term(X[i]) * constant_term(ui[i]), order)
#             Yij_t_Vi[i] = Taylor1(constant_term(Y[i]) * constant_term(vi[i]), order)
#             Zij_t_Wi[i] = Taylor1(constant_term(Z[i]) * constant_term(wi[i]), order)
#             tmp1433[i] = Taylor1(constant_term(Xij_t_Ui[i]) + constant_term(Yij_t_Vi[i]), order)
#             Rij_dot_Vi[i] = Taylor1(constant_term(tmp1433[i]) + constant_term(Zij_t_Wi[i]), order)
#             tmp1436[i] = Taylor1(constant_term(Rij_dot_Vi[i]) ^ float(constant_term(2)), order)
#             tmp1494[i] = Taylor1(zero(constant_term(Rij_dot_Vi[i])), order)
#             pn1t7[i] = Taylor1(constant_term(tmp1436[i]) / constant_term(r_p2[i]), order)
#             tmp1439[i] = Taylor1(constant_term(1.5) * constant_term(pn1t7[i]), order)
#             pn1t2_7[i] = Taylor1(constant_term(ϕs_and_vs[i]) - constant_term(tmp1439[i]), order)
#             pn1t1_7[i] = Taylor1(constant_term(c_p2) + constant_term(pn1t2_7[i]), order)
#             pNX_t_X[i] = Taylor1(constant_term(acceph_t[3i - 2]) * constant_term(X[i]), order)
#             pNY_t_Y[i] = Taylor1(constant_term(acceph_t[3i - 1]) * constant_term(Y[i]), order)
#             pNZ_t_Z[i] = Taylor1(constant_term(acceph_t[3i]) * constant_term(Z[i]), order)
#             tmp1446[i] = Taylor1(constant_term(pNX_t_X[i]) + constant_term(pNY_t_Y[i]), order)
#             tmp1447[i] = Taylor1(constant_term(tmp1446[i]) + constant_term(pNZ_t_Z[i]), order)
#             tmp1448[i] = Taylor1(constant_term(0.5) * constant_term(tmp1447[i]), order)
#             pn1[i] = Taylor1(constant_term(pn1t1_7[i]) + constant_term(tmp1448[i]), order)
#             X_t_pn1[i] = Taylor1(constant_term(newton_acc_X[i]) * constant_term(pn1[i]), order)
#             Y_t_pn1[i] = Taylor1(constant_term(newton_acc_Y[i]) * constant_term(pn1[i]), order)
#             Z_t_pn1[i] = Taylor1(constant_term(newton_acc_Z[i]) * constant_term(pn1[i]), order)
#             pNX_t_pn3[i] = Taylor1(constant_term(acceph_t[3i - 2]) * constant_term(pn3[i]), order)
#             pNY_t_pn3[i] = Taylor1(constant_term(acceph_t[3i - 1]) * constant_term(pn3[i]), order)
#             pNZ_t_pn3[i] = Taylor1(constant_term(acceph_t[3i]) * constant_term(pn3[i]), order)
#         end
#     tmp1456 = Array{Taylor1{_S}}(undef, size(U_t_pn2))
#     for i = eachindex(tmp1456)
#         tmp1456[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     termpnx = Array{Taylor1{_S}}(undef, size(X_t_pn1))
#     for i = eachindex(termpnx)
#         termpnx[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     sumpnx = Array{Taylor1{_S}}(undef, size(termpnx))
#     for i = eachindex(sumpnx)
#         sumpnx[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1459 = Array{Taylor1{_S}}(undef, size(V_t_pn2))
#     for i = eachindex(tmp1459)
#         tmp1459[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     termpny = Array{Taylor1{_S}}(undef, size(Y_t_pn1))
#     for i = eachindex(termpny)
#         termpny[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     sumpny = Array{Taylor1{_S}}(undef, size(termpny))
#     for i = eachindex(sumpny)
#         sumpny[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1462 = Array{Taylor1{_S}}(undef, size(W_t_pn2))
#     for i = eachindex(tmp1462)
#         tmp1462[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     termpnz = Array{Taylor1{_S}}(undef, size(Z_t_pn1))
#     for i = eachindex(termpnz)
#         termpnz[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     sumpnz = Array{Taylor1{_S}}(undef, size(termpnz))
#     for i = eachindex(sumpnz)
#         sumpnz[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     for i = 1:10
#         tmp1456[i] = Taylor1(constant_term(U_t_pn2[i]) + constant_term(pNX_t_pn3[i]), order)
#         termpnx[i] = Taylor1(constant_term(X_t_pn1[i]) + constant_term(tmp1456[i]), order)
#         sumpnx[i] = Taylor1(constant_term(pntempX) + constant_term(termpnx[i]), order)
#         pntempX = Taylor1(identity(constant_term(sumpnx[i])), order)
#         tmp1459[i] = Taylor1(constant_term(V_t_pn2[i]) + constant_term(pNY_t_pn3[i]), order)
#         termpny[i] = Taylor1(constant_term(Y_t_pn1[i]) + constant_term(tmp1459[i]), order)
#         sumpny[i] = Taylor1(constant_term(pntempY) + constant_term(termpny[i]), order)
#         pntempY = Taylor1(identity(constant_term(sumpny[i])), order)
#         tmp1462[i] = Taylor1(constant_term(W_t_pn2[i]) + constant_term(pNZ_t_pn3[i]), order)
#         termpnz[i] = Taylor1(constant_term(Z_t_pn1[i]) + constant_term(tmp1462[i]), order)
#         sumpnz[i] = Taylor1(constant_term(pntempZ) + constant_term(termpnz[i]), order)
#         pntempZ = Taylor1(identity(constant_term(sumpnz[i])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1087 =# Threads.@threads for i = 11:Nm1
#             X_t_pn1[i] = Taylor1(constant_term(c_p2) * constant_term(newton_acc_X[i]), order)
#             Y_t_pn1[i] = Taylor1(constant_term(c_p2) * constant_term(newton_acc_Y[i]), order)
#             Z_t_pn1[i] = Taylor1(constant_term(c_p2) * constant_term(newton_acc_Z[i]), order)
#         end
#     for i = 11:Nm1
#         termpnx[i] = Taylor1(identity(constant_term(X_t_pn1[i])), order)
#         sumpnx[i] = Taylor1(constant_term(pntempX) + constant_term(termpnx[i]), order)
#         pntempX = Taylor1(identity(constant_term(sumpnx[i])), order)
#         termpny[i] = Taylor1(identity(constant_term(Y_t_pn1[i])), order)
#         sumpny[i] = Taylor1(constant_term(pntempY) + constant_term(termpny[i]), order)
#         pntempY = Taylor1(identity(constant_term(sumpny[i])), order)
#         termpnz[i] = Taylor1(identity(constant_term(Z_t_pn1[i])), order)
#         sumpnz[i] = Taylor1(constant_term(pntempZ) + constant_term(termpnz[i]), order)
#         pntempZ = Taylor1(identity(constant_term(sumpnz[i])), order)
#     end
#     postNewtonX = Taylor1(constant_term(pntempX) * constant_term(c_m2), order)
#     postNewtonY = Taylor1(constant_term(pntempY) * constant_term(c_m2), order)
#     postNewtonZ = Taylor1(constant_term(pntempZ) * constant_term(c_m2), order)
#     dq[4] = Taylor1(constant_term(postNewtonX) + constant_term(accX), order)
#     dq[5] = Taylor1(constant_term(postNewtonY) + constant_term(accY), order)
#     dq[6] = Taylor1(constant_term(postNewtonZ) + constant_term(accZ), order)
#     return TaylorIntegration.RetAlloc{Taylor1{_S}}([tmp1406, tmp1408, tmp1409, tmp1411, pntempX, pntempY, pntempZ, accX, accY, accZ, tmp1491, tmp1492, tmp1493, postNewtonX, postNewtonY, postNewtonZ], [X, Y, Z, r_p2, r_p1d2, r_p3d2, r_p7d2, newtonianCoeff, ui, vi, wi, U, V, W, _4dq, _4U_m_3X, _4V_m_3Y, _4W_m_3Z, UU, VV, WW, newtonian1b_Potential, newtonianNb_Potential, newton_acc_X, newton_acc_Y, newton_acc_Z, v2, vi_dot_vj, pn2, U_t_pn2, V_t_pn2, W_t_pn2, pn3, pNX_t_pn3, pNY_t_pn3, pNZ_t_pn3, _4ϕj, ϕi_plus_4ϕj, sj2_plus_2si2_minus_4vivj, ϕs_and_vs, pn1t1_7, pNX_t_X, pNY_t_Y, pNZ_t_Z, pn1, X_t_pn1, Y_t_pn1, Z_t_pn1, t31, t32, t33, r_sin_ϕ, sin_ϕ, ϕ, cos_ϕ, sin2_ϕ, sin3_ϕ, F_J2_x, F_J2_y, F_J2_z, F_J2_x1, F_J2_y1, F_J2_z1, F_J2_x2, F_J2_y2, F_J2_z2, temp_accX_i, temp_accY_i, temp_accZ_i, P_2_sin_ϕ, ∂P_2_sin_ϕ, P_3_sin_ϕ, ∂P_3_sin_ϕ, m_c_ϕ_∂P_2, m_c_ϕ_∂P_3, Λ2j_div_r4, Λ3j_div_r5, F_J_ξ, F_J_ζ, F_J2_ξ, F_J2_ζ, F_J3_ξ, F_J3_ζ, ξx, ξy, ξz, ηx, ηy, ηz, ηx1, ηy1, ηz1, ηx2, ηy2, ηz2, ζx, ζy, ζz, ζx1, ζy1, ζz1, ζx2, ζy2, ζz2, tmp1272, tmp1275, tmp1278, pn2x, pn2y, pn2z, tmp1286, tmp1289, tmp1477, tmp1291, tmp1478, tmp1292, tmp1294, tmp1479, tmp1480, tmp1481, tmp1302, tmp1303, tmp1314, tmp1316, tmp1318, tmp1320, tmp1482, tmp1483, tmp1484, tmp1485, tmp1330, tmp1336, tmp1338, tmp1342, tmp1345, tmp1486, tmp1346, tmp1349, tmp1487, tmp1350, tmp1352, tmp1354, tmp1357, tmp1361, tmp1364, tmp1366, tmp1368, tmp1398, tmp1488, tmp1400, tmp1489, tmp1401, tmp1403, tmp1490, temp_004, tmp1414, tmp1416, tmp1418, tmp1424, tmp1426, tmp1427, Xij_t_Ui, Yij_t_Vi, Zij_t_Wi, tmp1433, Rij_dot_Vi, tmp1436, tmp1494, pn1t7, tmp1439, pn1t2_7, tmp1446, tmp1447, tmp1448, tmp1456, termpnx, sumpnx, tmp1459, termpny, sumpny, tmp1462, termpnz, sumpnz], [Array{Taylor1{_S}, 2}(undef, 0, 0)], [Array{Taylor1{_S}, 3}(undef, 0, 0, 0)], [Array{Taylor1{_S}, 4}(undef, 0, 0, 0, 0)])
# end

# # TaylorIntegration.jetcoeffs! method for src/propagation/asteroid_dynamical_models.jl: RNp1BP_pN_A_J23E_J2S_eph_threads!
# function TaylorIntegration.jetcoeffs!(::Val{RNp1BP_pN_A_J23E_J2S_eph_threads!}, t::Taylor1{_T}, q::AbstractArray{Taylor1{_S}, _N}, dq::AbstractArray{Taylor1{_S}, _N}, params, __ralloc::TaylorIntegration.RetAlloc{Taylor1{_S}}) where {_T <: Real, _S <: Number, _N}
#     order = t.order
#     tmp1406 = __ralloc.v0[1]
#     tmp1408 = __ralloc.v0[2]
#     tmp1409 = __ralloc.v0[3]
#     tmp1411 = __ralloc.v0[4]
#     pntempX = __ralloc.v0[5]
#     pntempY = __ralloc.v0[6]
#     pntempZ = __ralloc.v0[7]
#     accX = __ralloc.v0[8]
#     accY = __ralloc.v0[9]
#     accZ = __ralloc.v0[10]
#     tmp1491 = __ralloc.v0[11]
#     tmp1492 = __ralloc.v0[12]
#     tmp1493 = __ralloc.v0[13]
#     postNewtonX = __ralloc.v0[14]
#     postNewtonY = __ralloc.v0[15]
#     postNewtonZ = __ralloc.v0[16]
#     X = __ralloc.v1[1]
#     Y = __ralloc.v1[2]
#     Z = __ralloc.v1[3]
#     r_p2 = __ralloc.v1[4]
#     r_p1d2 = __ralloc.v1[5]
#     r_p3d2 = __ralloc.v1[6]
#     r_p7d2 = __ralloc.v1[7]
#     newtonianCoeff = __ralloc.v1[8]
#     ui = __ralloc.v1[9]
#     vi = __ralloc.v1[10]
#     wi = __ralloc.v1[11]
#     U = __ralloc.v1[12]
#     V = __ralloc.v1[13]
#     W = __ralloc.v1[14]
#     _4dq = __ralloc.v1[15]
#     _4U_m_3X = __ralloc.v1[16]
#     _4V_m_3Y = __ralloc.v1[17]
#     _4W_m_3Z = __ralloc.v1[18]
#     UU = __ralloc.v1[19]
#     VV = __ralloc.v1[20]
#     WW = __ralloc.v1[21]
#     newtonian1b_Potential = __ralloc.v1[22]
#     newtonianNb_Potential = __ralloc.v1[23]
#     newton_acc_X = __ralloc.v1[24]
#     newton_acc_Y = __ralloc.v1[25]
#     newton_acc_Z = __ralloc.v1[26]
#     v2 = __ralloc.v1[27]
#     vi_dot_vj = __ralloc.v1[28]
#     pn2 = __ralloc.v1[29]
#     U_t_pn2 = __ralloc.v1[30]
#     V_t_pn2 = __ralloc.v1[31]
#     W_t_pn2 = __ralloc.v1[32]
#     pn3 = __ralloc.v1[33]
#     pNX_t_pn3 = __ralloc.v1[34]
#     pNY_t_pn3 = __ralloc.v1[35]
#     pNZ_t_pn3 = __ralloc.v1[36]
#     _4ϕj = __ralloc.v1[37]
#     ϕi_plus_4ϕj = __ralloc.v1[38]
#     sj2_plus_2si2_minus_4vivj = __ralloc.v1[39]
#     ϕs_and_vs = __ralloc.v1[40]
#     pn1t1_7 = __ralloc.v1[41]
#     pNX_t_X = __ralloc.v1[42]
#     pNY_t_Y = __ralloc.v1[43]
#     pNZ_t_Z = __ralloc.v1[44]
#     pn1 = __ralloc.v1[45]
#     X_t_pn1 = __ralloc.v1[46]
#     Y_t_pn1 = __ralloc.v1[47]
#     Z_t_pn1 = __ralloc.v1[48]
#     t31 = __ralloc.v1[49]
#     t32 = __ralloc.v1[50]
#     t33 = __ralloc.v1[51]
#     r_sin_ϕ = __ralloc.v1[52]
#     sin_ϕ = __ralloc.v1[53]
#     ϕ = __ralloc.v1[54]
#     cos_ϕ = __ralloc.v1[55]
#     sin2_ϕ = __ralloc.v1[56]
#     sin3_ϕ = __ralloc.v1[57]
#     F_J2_x = __ralloc.v1[58]
#     F_J2_y = __ralloc.v1[59]
#     F_J2_z = __ralloc.v1[60]
#     F_J2_x1 = __ralloc.v1[61]
#     F_J2_y1 = __ralloc.v1[62]
#     F_J2_z1 = __ralloc.v1[63]
#     F_J2_x2 = __ralloc.v1[64]
#     F_J2_y2 = __ralloc.v1[65]
#     F_J2_z2 = __ralloc.v1[66]
#     temp_accX_i = __ralloc.v1[67]
#     temp_accY_i = __ralloc.v1[68]
#     temp_accZ_i = __ralloc.v1[69]
#     P_2_sin_ϕ = __ralloc.v1[70]
#     ∂P_2_sin_ϕ = __ralloc.v1[71]
#     P_3_sin_ϕ = __ralloc.v1[72]
#     ∂P_3_sin_ϕ = __ralloc.v1[73]
#     m_c_ϕ_∂P_2 = __ralloc.v1[74]
#     m_c_ϕ_∂P_3 = __ralloc.v1[75]
#     Λ2j_div_r4 = __ralloc.v1[76]
#     Λ3j_div_r5 = __ralloc.v1[77]
#     F_J_ξ = __ralloc.v1[78]
#     F_J_ζ = __ralloc.v1[79]
#     F_J2_ξ = __ralloc.v1[80]
#     F_J2_ζ = __ralloc.v1[81]
#     F_J3_ξ = __ralloc.v1[82]
#     F_J3_ζ = __ralloc.v1[83]
#     ξx = __ralloc.v1[84]
#     ξy = __ralloc.v1[85]
#     ξz = __ralloc.v1[86]
#     ηx = __ralloc.v1[87]
#     ηy = __ralloc.v1[88]
#     ηz = __ralloc.v1[89]
#     ηx1 = __ralloc.v1[90]
#     ηy1 = __ralloc.v1[91]
#     ηz1 = __ralloc.v1[92]
#     ηx2 = __ralloc.v1[93]
#     ηy2 = __ralloc.v1[94]
#     ηz2 = __ralloc.v1[95]
#     ζx = __ralloc.v1[96]
#     ζy = __ralloc.v1[97]
#     ζz = __ralloc.v1[98]
#     ζx1 = __ralloc.v1[99]
#     ζy1 = __ralloc.v1[100]
#     ζz1 = __ralloc.v1[101]
#     ζx2 = __ralloc.v1[102]
#     ζy2 = __ralloc.v1[103]
#     ζz2 = __ralloc.v1[104]
#     tmp1272 = __ralloc.v1[105]
#     tmp1275 = __ralloc.v1[106]
#     tmp1278 = __ralloc.v1[107]
#     pn2x = __ralloc.v1[108]
#     pn2y = __ralloc.v1[109]
#     pn2z = __ralloc.v1[110]
#     tmp1286 = __ralloc.v1[111]
#     tmp1289 = __ralloc.v1[112]
#     tmp1477 = __ralloc.v1[113]
#     tmp1291 = __ralloc.v1[114]
#     tmp1478 = __ralloc.v1[115]
#     tmp1292 = __ralloc.v1[116]
#     tmp1294 = __ralloc.v1[117]
#     tmp1479 = __ralloc.v1[118]
#     tmp1480 = __ralloc.v1[119]
#     tmp1481 = __ralloc.v1[120]
#     tmp1302 = __ralloc.v1[121]
#     tmp1303 = __ralloc.v1[122]
#     tmp1314 = __ralloc.v1[123]
#     tmp1316 = __ralloc.v1[124]
#     tmp1318 = __ralloc.v1[125]
#     tmp1320 = __ralloc.v1[126]
#     tmp1482 = __ralloc.v1[127]
#     tmp1483 = __ralloc.v1[128]
#     tmp1484 = __ralloc.v1[129]
#     tmp1485 = __ralloc.v1[130]
#     tmp1330 = __ralloc.v1[131]
#     tmp1336 = __ralloc.v1[132]
#     tmp1338 = __ralloc.v1[133]
#     tmp1342 = __ralloc.v1[134]
#     tmp1345 = __ralloc.v1[135]
#     tmp1486 = __ralloc.v1[136]
#     tmp1346 = __ralloc.v1[137]
#     tmp1349 = __ralloc.v1[138]
#     tmp1487 = __ralloc.v1[139]
#     tmp1350 = __ralloc.v1[140]
#     tmp1352 = __ralloc.v1[141]
#     tmp1354 = __ralloc.v1[142]
#     tmp1357 = __ralloc.v1[143]
#     tmp1361 = __ralloc.v1[144]
#     tmp1364 = __ralloc.v1[145]
#     tmp1366 = __ralloc.v1[146]
#     tmp1368 = __ralloc.v1[147]
#     tmp1398 = __ralloc.v1[148]
#     tmp1488 = __ralloc.v1[149]
#     tmp1400 = __ralloc.v1[150]
#     tmp1489 = __ralloc.v1[151]
#     tmp1401 = __ralloc.v1[152]
#     tmp1403 = __ralloc.v1[153]
#     tmp1490 = __ralloc.v1[154]
#     temp_004 = __ralloc.v1[155]
#     tmp1414 = __ralloc.v1[156]
#     tmp1416 = __ralloc.v1[157]
#     tmp1418 = __ralloc.v1[158]
#     tmp1424 = __ralloc.v1[159]
#     tmp1426 = __ralloc.v1[160]
#     tmp1427 = __ralloc.v1[161]
#     Xij_t_Ui = __ralloc.v1[162]
#     Yij_t_Vi = __ralloc.v1[163]
#     Zij_t_Wi = __ralloc.v1[164]
#     tmp1433 = __ralloc.v1[165]
#     Rij_dot_Vi = __ralloc.v1[166]
#     tmp1436 = __ralloc.v1[167]
#     tmp1494 = __ralloc.v1[168]
#     pn1t7 = __ralloc.v1[169]
#     tmp1439 = __ralloc.v1[170]
#     pn1t2_7 = __ralloc.v1[171]
#     tmp1446 = __ralloc.v1[172]
#     tmp1447 = __ralloc.v1[173]
#     tmp1448 = __ralloc.v1[174]
#     tmp1456 = __ralloc.v1[175]
#     termpnx = __ralloc.v1[176]
#     sumpnx = __ralloc.v1[177]
#     tmp1459 = __ralloc.v1[178]
#     termpny = __ralloc.v1[179]
#     sumpny = __ralloc.v1[180]
#     tmp1462 = __ralloc.v1[181]
#     termpnz = __ralloc.v1[182]
#     sumpnz = __ralloc.v1[183]
#     local jd0 = params.jd0
#     local dsj2k = t + (jd0 - JD_J2000)
#     local ss16asteph_t = evaleph(params.sseph, dsj2k, q[1])
#     local acceph_t = evaleph(params.acceph, dsj2k, q[1])
#     local newtonianNb_Potential_t = evaleph(params.poteph, dsj2k, q[1])
#     local S = eltype(q)
#     local UJ_interaction = params.UJ_interaction
#     local N = params.N
#     local Nm1 = N - 1
#     local μ = params.μ
#     local zero_q_1 = auxzero(q[1])
#     TaylorSeries.zero!(pntempX)
#     pntempX.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(pntempY)
#     pntempY.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(pntempZ)
#     pntempZ.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accX)
#     accX.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accY)
#     accY.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accZ)
#     accZ.coeffs[1] = identity(constant_term(zero_q_1))
#     local M_ = Array{S}(undef, 3, 3, N)
#     local M_[:, :, ea] = t2c_jpl_de430(dsj2k) .+ zero_q_1
#     TaylorSeries.zero!(dq[1])
#     (dq[1]).coeffs[1] = identity(constant_term(q[4]))
#     TaylorSeries.zero!(dq[2])
#     (dq[2]).coeffs[1] = identity(constant_term(q[5]))
#     TaylorSeries.zero!(dq[3])
#     (dq[3]).coeffs[1] = identity(constant_term(q[6]))
#     TaylorSeries.zero!(newtonianNb_Potential[N])
#     (newtonianNb_Potential[N]).coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(_4dq[1])
#     (_4dq[1]).coeffs[1] = constant_term(4) * constant_term(dq[1])
#     TaylorSeries.zero!(_4dq[2])
#     (_4dq[2]).coeffs[1] = constant_term(4) * constant_term(dq[2])
#     TaylorSeries.zero!(_4dq[3])
#     (_4dq[3]).coeffs[1] = constant_term(4) * constant_term(dq[3])
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:832 =# Threads.@threads for i = 1:Nm1
#             TaylorSeries.zero!(ui[i])
#             (ui[i]).coeffs[1] = identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 2]))
#             TaylorSeries.zero!(vi[i])
#             (vi[i]).coeffs[1] = identity(constant_term(ss16asteph_t[3 * ((N - 1) + i) - 1]))
#             TaylorSeries.zero!(wi[i])
#             (wi[i]).coeffs[1] = identity(constant_term(ss16asteph_t[3 * ((N - 1) + i)]))
#             TaylorSeries.zero!(X[i])
#             (X[i]).coeffs[1] = constant_term(ss16asteph_t[3i - 2]) - constant_term(q[1])
#             TaylorSeries.zero!(Y[i])
#             (Y[i]).coeffs[1] = constant_term(ss16asteph_t[3i - 1]) - constant_term(q[2])
#             TaylorSeries.zero!(Z[i])
#             (Z[i]).coeffs[1] = constant_term(ss16asteph_t[3i]) - constant_term(q[3])
#             TaylorSeries.zero!(U[i])
#             (U[i]).coeffs[1] = constant_term(ui[i]) - constant_term(dq[1])
#             TaylorSeries.zero!(V[i])
#             (V[i]).coeffs[1] = constant_term(vi[i]) - constant_term(dq[2])
#             TaylorSeries.zero!(W[i])
#             (W[i]).coeffs[1] = constant_term(wi[i]) - constant_term(dq[3])
#             TaylorSeries.zero!(tmp1272[i])
#             (tmp1272[i]).coeffs[1] = constant_term(-3) * constant_term(ui[i])
#             TaylorSeries.zero!(_4U_m_3X[i])
#             (_4U_m_3X[i]).coeffs[1] = constant_term(tmp1272[i]) + constant_term(_4dq[1])
#             TaylorSeries.zero!(tmp1275[i])
#             (tmp1275[i]).coeffs[1] = constant_term(-3) * constant_term(vi[i])
#             TaylorSeries.zero!(_4V_m_3Y[i])
#             (_4V_m_3Y[i]).coeffs[1] = constant_term(tmp1275[i]) + constant_term(_4dq[2])
#             TaylorSeries.zero!(tmp1278[i])
#             (tmp1278[i]).coeffs[1] = constant_term(-3) * constant_term(wi[i])
#             TaylorSeries.zero!(_4W_m_3Z[i])
#             (_4W_m_3Z[i]).coeffs[1] = constant_term(tmp1278[i]) + constant_term(_4dq[3])
#             TaylorSeries.zero!(pn2x[i])
#             (pn2x[i]).coeffs[1] = constant_term(X[i]) * constant_term(_4U_m_3X[i])
#             TaylorSeries.zero!(pn2y[i])
#             (pn2y[i]).coeffs[1] = constant_term(Y[i]) * constant_term(_4V_m_3Y[i])
#             TaylorSeries.zero!(pn2z[i])
#             (pn2z[i]).coeffs[1] = constant_term(Z[i]) * constant_term(_4W_m_3Z[i])
#             TaylorSeries.zero!(UU[i])
#             (UU[i]).coeffs[1] = constant_term(ui[i]) * constant_term(dq[1])
#             TaylorSeries.zero!(VV[i])
#             (VV[i]).coeffs[1] = constant_term(vi[i]) * constant_term(dq[2])
#             TaylorSeries.zero!(WW[i])
#             (WW[i]).coeffs[1] = constant_term(wi[i]) * constant_term(dq[3])
#             TaylorSeries.zero!(tmp1286[i])
#             (tmp1286[i]).coeffs[1] = constant_term(UU[i]) + constant_term(VV[i])
#             TaylorSeries.zero!(vi_dot_vj[i])
#             (vi_dot_vj[i]).coeffs[1] = constant_term(tmp1286[i]) + constant_term(WW[i])
#             TaylorSeries.zero!(tmp1289[i])
#             (tmp1289[i]).coeffs[1] = constant_term(X[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1477[i])
#             (tmp1477[i]).coeffs[1] = zero(constant_term(X[i]))
#             TaylorSeries.zero!(tmp1291[i])
#             (tmp1291[i]).coeffs[1] = constant_term(Y[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1478[i])
#             (tmp1478[i]).coeffs[1] = zero(constant_term(Y[i]))
#             TaylorSeries.zero!(tmp1292[i])
#             (tmp1292[i]).coeffs[1] = constant_term(tmp1289[i]) + constant_term(tmp1291[i])
#             TaylorSeries.zero!(tmp1294[i])
#             (tmp1294[i]).coeffs[1] = constant_term(Z[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1479[i])
#             (tmp1479[i]).coeffs[1] = zero(constant_term(Z[i]))
#             TaylorSeries.zero!(r_p2[i])
#             (r_p2[i]).coeffs[1] = constant_term(tmp1292[i]) + constant_term(tmp1294[i])
#             TaylorSeries.zero!(r_p1d2[i])
#             (r_p1d2[i]).coeffs[1] = sqrt(constant_term(r_p2[i]))
#             TaylorSeries.zero!(r_p3d2[i])
#             (r_p3d2[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(1.5))
#             TaylorSeries.zero!(tmp1480[i])
#             (tmp1480[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#             TaylorSeries.zero!(r_p7d2[i])
#             (r_p7d2[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(3.5))
#             TaylorSeries.zero!(tmp1481[i])
#             (tmp1481[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#             TaylorSeries.zero!(newtonianCoeff[i])
#             (newtonianCoeff[i]).coeffs[1] = constant_term(μ[i]) / constant_term(r_p3d2[i])
#             TaylorSeries.zero!(tmp1302[i])
#             (tmp1302[i]).coeffs[1] = constant_term(pn2x[i]) + constant_term(pn2y[i])
#             TaylorSeries.zero!(tmp1303[i])
#             (tmp1303[i]).coeffs[1] = constant_term(tmp1302[i]) + constant_term(pn2z[i])
#             TaylorSeries.zero!(pn2[i])
#             (pn2[i]).coeffs[1] = constant_term(newtonianCoeff[i]) * constant_term(tmp1303[i])
#             TaylorSeries.zero!(newton_acc_X[i])
#             (newton_acc_X[i]).coeffs[1] = constant_term(X[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newton_acc_Y[i])
#             (newton_acc_Y[i]).coeffs[1] = constant_term(Y[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newton_acc_Z[i])
#             (newton_acc_Z[i]).coeffs[1] = constant_term(Z[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newtonian1b_Potential[i])
#             (newtonian1b_Potential[i]).coeffs[1] = constant_term(μ[i]) / constant_term(r_p1d2[i])
#             TaylorSeries.zero!(pn3[i])
#             (pn3[i]).coeffs[1] = constant_term(3.5) * constant_term(newtonian1b_Potential[i])
#             TaylorSeries.zero!(U_t_pn2[i])
#             (U_t_pn2[i]).coeffs[1] = constant_term(pn2[i]) * constant_term(U[i])
#             TaylorSeries.zero!(V_t_pn2[i])
#             (V_t_pn2[i]).coeffs[1] = constant_term(pn2[i]) * constant_term(V[i])
#             TaylorSeries.zero!(W_t_pn2[i])
#             (W_t_pn2[i]).coeffs[1] = constant_term(pn2[i]) * constant_term(W[i])
#             if UJ_interaction[i]
#                 TaylorSeries.zero!(tmp1314[i])
#                 (tmp1314[i]).coeffs[1] = -(constant_term(X[i]))
#                 TaylorSeries.zero!(t31[i])
#                 (t31[i]).coeffs[1] = constant_term(tmp1314[i]) * constant_term(M_[1, 3, i])
#                 TaylorSeries.zero!(tmp1316[i])
#                 (tmp1316[i]).coeffs[1] = -(constant_term(Y[i]))
#                 TaylorSeries.zero!(t32[i])
#                 (t32[i]).coeffs[1] = constant_term(tmp1316[i]) * constant_term(M_[2, 3, i])
#                 TaylorSeries.zero!(tmp1318[i])
#                 (tmp1318[i]).coeffs[1] = -(constant_term(Z[i]))
#                 TaylorSeries.zero!(t33[i])
#                 (t33[i]).coeffs[1] = constant_term(tmp1318[i]) * constant_term(M_[3, 3, i])
#                 TaylorSeries.zero!(tmp1320[i])
#                 (tmp1320[i]).coeffs[1] = constant_term(t31[i]) + constant_term(t32[i])
#                 TaylorSeries.zero!(r_sin_ϕ[i])
#                 (r_sin_ϕ[i]).coeffs[1] = constant_term(tmp1320[i]) + constant_term(t33[i])
#                 TaylorSeries.zero!(sin_ϕ[i])
#                 (sin_ϕ[i]).coeffs[1] = constant_term(r_sin_ϕ[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(ϕ[i])
#                 (ϕ[i]).coeffs[1] = asin(constant_term(sin_ϕ[i]))
#                 TaylorSeries.zero!(tmp1482[i])
#                 (tmp1482[i]).coeffs[1] = sqrt(1 - constant_term(sin_ϕ[i]) ^ 2)
#                 TaylorSeries.zero!(cos_ϕ[i])
#                 (cos_ϕ[i]).coeffs[1] = cos(constant_term(ϕ[i]))
#                 TaylorSeries.zero!(tmp1483[i])
#                 (tmp1483[i]).coeffs[1] = sin(constant_term(ϕ[i]))
#                 TaylorSeries.zero!(sin2_ϕ[i])
#                 (sin2_ϕ[i]).coeffs[1] = constant_term(sin_ϕ[i]) ^ float(constant_term(2))
#                 TaylorSeries.zero!(tmp1484[i])
#                 (tmp1484[i]).coeffs[1] = zero(constant_term(sin_ϕ[i]))
#                 TaylorSeries.zero!(sin3_ϕ[i])
#                 (sin3_ϕ[i]).coeffs[1] = constant_term(sin_ϕ[i]) ^ float(constant_term(3))
#                 TaylorSeries.zero!(tmp1485[i])
#                 (tmp1485[i]).coeffs[1] = zero(constant_term(sin_ϕ[i]))
#                 TaylorSeries.zero!(tmp1330[i])
#                 (tmp1330[i]).coeffs[1] = constant_term(1.5) * constant_term(sin2_ϕ[i])
#                 TaylorSeries.zero!(P_2_sin_ϕ[i])
#                 (P_2_sin_ϕ[i]).coeffs[1] = constant_term(tmp1330[i]) - constant_term(0.5)
#                 TaylorSeries.zero!(∂P_2_sin_ϕ[i])
#                 (∂P_2_sin_ϕ[i]).coeffs[1] = constant_term(3) * constant_term(sin_ϕ[i])
#                 TaylorSeries.zero!(tmp1336[i])
#                 (tmp1336[i]).coeffs[1] = constant_term(-1.5) * constant_term(sin_ϕ[i])
#                 TaylorSeries.zero!(tmp1338[i])
#                 (tmp1338[i]).coeffs[1] = constant_term(2.5) * constant_term(sin3_ϕ[i])
#                 TaylorSeries.zero!(P_3_sin_ϕ[i])
#                 (P_3_sin_ϕ[i]).coeffs[1] = constant_term(tmp1336[i]) + constant_term(tmp1338[i])
#                 TaylorSeries.zero!(tmp1342[i])
#                 (tmp1342[i]).coeffs[1] = constant_term(7.5) * constant_term(sin2_ϕ[i])
#                 TaylorSeries.zero!(∂P_3_sin_ϕ[i])
#                 (∂P_3_sin_ϕ[i]).coeffs[1] = constant_term(-1.5) + constant_term(tmp1342[i])
#                 TaylorSeries.zero!(tmp1345[i])
#                 (tmp1345[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(2))
#                 TaylorSeries.zero!(tmp1486[i])
#                 (tmp1486[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#                 TaylorSeries.zero!(tmp1346[i])
#                 (tmp1346[i]).coeffs[1] = constant_term(Λ2[i]) / constant_term(tmp1345[i])
#                 TaylorSeries.zero!(Λ2j_div_r4[i])
#                 (Λ2j_div_r4[i]).coeffs[1] = -(constant_term(tmp1346[i]))
#                 TaylorSeries.zero!(tmp1349[i])
#                 (tmp1349[i]).coeffs[1] = constant_term(r_p1d2[i]) ^ float(constant_term(5))
#                 TaylorSeries.zero!(tmp1487[i])
#                 (tmp1487[i]).coeffs[1] = zero(constant_term(r_p1d2[i]))
#                 TaylorSeries.zero!(tmp1350[i])
#                 (tmp1350[i]).coeffs[1] = constant_term(Λ3[i]) / constant_term(tmp1349[i])
#                 TaylorSeries.zero!(Λ3j_div_r5[i])
#                 (Λ3j_div_r5[i]).coeffs[1] = -(constant_term(tmp1350[i]))
#                 TaylorSeries.zero!(tmp1352[i])
#                 (tmp1352[i]).coeffs[1] = -(constant_term(cos_ϕ[i]))
#                 TaylorSeries.zero!(m_c_ϕ_∂P_2[i])
#                 (m_c_ϕ_∂P_2[i]).coeffs[1] = constant_term(tmp1352[i]) * constant_term(∂P_2_sin_ϕ[i])
#                 TaylorSeries.zero!(tmp1354[i])
#                 (tmp1354[i]).coeffs[1] = -(constant_term(cos_ϕ[i]))
#                 TaylorSeries.zero!(m_c_ϕ_∂P_3[i])
#                 (m_c_ϕ_∂P_3[i]).coeffs[1] = constant_term(tmp1354[i]) * constant_term(∂P_3_sin_ϕ[i])
#                 TaylorSeries.zero!(tmp1357[i])
#                 (tmp1357[i]).coeffs[1] = constant_term(Λ2j_div_r4[i]) * constant_term(3)
#                 TaylorSeries.zero!(F_J2_ξ[i])
#                 (F_J2_ξ[i]).coeffs[1] = constant_term(tmp1357[i]) * constant_term(P_2_sin_ϕ[i])
#                 TaylorSeries.zero!(F_J2_ζ[i])
#                 (F_J2_ζ[i]).coeffs[1] = constant_term(Λ2j_div_r4[i]) * constant_term(m_c_ϕ_∂P_2[i])
#                 TaylorSeries.zero!(tmp1361[i])
#                 (tmp1361[i]).coeffs[1] = constant_term(Λ3j_div_r5[i]) * constant_term(4)
#                 TaylorSeries.zero!(F_J3_ξ[i])
#                 (F_J3_ξ[i]).coeffs[1] = constant_term(tmp1361[i]) * constant_term(P_3_sin_ϕ[i])
#                 TaylorSeries.zero!(F_J3_ζ[i])
#                 (F_J3_ζ[i]).coeffs[1] = constant_term(Λ3j_div_r5[i]) * constant_term(m_c_ϕ_∂P_3[i])
#                 TaylorSeries.zero!(F_J_ξ[i])
#                 (F_J_ξ[i]).coeffs[1] = identity(constant_term(F_J2_ξ[i]))
#                 TaylorSeries.zero!(F_J_ζ[i])
#                 (F_J_ζ[i]).coeffs[1] = identity(constant_term(F_J2_ζ[i]))
#                 TaylorSeries.zero!(tmp1364[i])
#                 (tmp1364[i]).coeffs[1] = -(constant_term(X[i]))
#                 TaylorSeries.zero!(ξx[i])
#                 (ξx[i]).coeffs[1] = constant_term(tmp1364[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(tmp1366[i])
#                 (tmp1366[i]).coeffs[1] = -(constant_term(Y[i]))
#                 TaylorSeries.zero!(ξy[i])
#                 (ξy[i]).coeffs[1] = constant_term(tmp1366[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(tmp1368[i])
#                 (tmp1368[i]).coeffs[1] = -(constant_term(Z[i]))
#                 TaylorSeries.zero!(ξz[i])
#                 (ξz[i]).coeffs[1] = constant_term(tmp1368[i]) / constant_term(r_p1d2[i])
#                 TaylorSeries.zero!(ηx1[i])
#                 (ηx1[i]).coeffs[1] = constant_term(M_[2, 3, i]) * constant_term(ξz[i])
#                 TaylorSeries.zero!(ηy1[i])
#                 (ηy1[i]).coeffs[1] = constant_term(M_[3, 3, i]) * constant_term(ξx[i])
#                 TaylorSeries.zero!(ηz1[i])
#                 (ηz1[i]).coeffs[1] = constant_term(M_[1, 3, i]) * constant_term(ξy[i])
#                 TaylorSeries.zero!(ηx2[i])
#                 (ηx2[i]).coeffs[1] = constant_term(M_[3, 3, i]) * constant_term(ξy[i])
#                 TaylorSeries.zero!(ηy2[i])
#                 (ηy2[i]).coeffs[1] = constant_term(M_[1, 3, i]) * constant_term(ξz[i])
#                 TaylorSeries.zero!(ηz2[i])
#                 (ηz2[i]).coeffs[1] = constant_term(M_[2, 3, i]) * constant_term(ξx[i])
#                 TaylorSeries.zero!(ηx[i])
#                 (ηx[i]).coeffs[1] = constant_term(ηx1[i]) - constant_term(ηx2[i])
#                 TaylorSeries.zero!(ηy[i])
#                 (ηy[i]).coeffs[1] = constant_term(ηy1[i]) - constant_term(ηy2[i])
#                 TaylorSeries.zero!(ηz[i])
#                 (ηz[i]).coeffs[1] = constant_term(ηz1[i]) - constant_term(ηz2[i])
#                 TaylorSeries.zero!(ζx1[i])
#                 (ζx1[i]).coeffs[1] = constant_term(ξy[i]) * constant_term(ηz[i])
#                 TaylorSeries.zero!(ζy1[i])
#                 (ζy1[i]).coeffs[1] = constant_term(ξz[i]) * constant_term(ηx[i])
#                 TaylorSeries.zero!(ζz1[i])
#                 (ζz1[i]).coeffs[1] = constant_term(ξx[i]) * constant_term(ηy[i])
#                 TaylorSeries.zero!(ζx2[i])
#                 (ζx2[i]).coeffs[1] = constant_term(ξz[i]) * constant_term(ηy[i])
#                 TaylorSeries.zero!(ζy2[i])
#                 (ζy2[i]).coeffs[1] = constant_term(ξx[i]) * constant_term(ηz[i])
#                 TaylorSeries.zero!(ζz2[i])
#                 (ζz2[i]).coeffs[1] = constant_term(ξy[i]) * constant_term(ηx[i])
#                 TaylorSeries.zero!(ζx[i])
#                 (ζx[i]).coeffs[1] = constant_term(ζx1[i]) - constant_term(ζx2[i])
#                 TaylorSeries.zero!(ζy[i])
#                 (ζy[i]).coeffs[1] = constant_term(ζy1[i]) - constant_term(ζy2[i])
#                 TaylorSeries.zero!(ζz[i])
#                 (ζz[i]).coeffs[1] = constant_term(ζz1[i]) - constant_term(ζz2[i])
#                 TaylorSeries.zero!(F_J2_x1[i])
#                 (F_J2_x1[i]).coeffs[1] = constant_term(F_J_ξ[i]) * constant_term(ξx[i])
#                 TaylorSeries.zero!(F_J2_y1[i])
#                 (F_J2_y1[i]).coeffs[1] = constant_term(F_J_ξ[i]) * constant_term(ξy[i])
#                 TaylorSeries.zero!(F_J2_z1[i])
#                 (F_J2_z1[i]).coeffs[1] = constant_term(F_J_ξ[i]) * constant_term(ξz[i])
#                 TaylorSeries.zero!(F_J2_x2[i])
#                 (F_J2_x2[i]).coeffs[1] = constant_term(F_J_ζ[i]) * constant_term(ζx[i])
#                 TaylorSeries.zero!(F_J2_y2[i])
#                 (F_J2_y2[i]).coeffs[1] = constant_term(F_J_ζ[i]) * constant_term(ζy[i])
#                 TaylorSeries.zero!(F_J2_z2[i])
#                 (F_J2_z2[i]).coeffs[1] = constant_term(F_J_ζ[i]) * constant_term(ζz[i])
#                 TaylorSeries.zero!(F_J2_x[i])
#                 (F_J2_x[i]).coeffs[1] = constant_term(F_J2_x1[i]) + constant_term(F_J2_x2[i])
#                 TaylorSeries.zero!(F_J2_y[i])
#                 (F_J2_y[i]).coeffs[1] = constant_term(F_J2_y1[i]) + constant_term(F_J2_y2[i])
#                 TaylorSeries.zero!(F_J2_z[i])
#                 (F_J2_z[i]).coeffs[1] = constant_term(F_J2_z1[i]) + constant_term(F_J2_z2[i])
#             end
#             TaylorSeries.zero!(tmp1398[i])
#             (tmp1398[i]).coeffs[1] = constant_term(ui[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1488[i])
#             (tmp1488[i]).coeffs[1] = zero(constant_term(ui[i]))
#             TaylorSeries.zero!(tmp1400[i])
#             (tmp1400[i]).coeffs[1] = constant_term(vi[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1489[i])
#             (tmp1489[i]).coeffs[1] = zero(constant_term(vi[i]))
#             TaylorSeries.zero!(tmp1401[i])
#             (tmp1401[i]).coeffs[1] = constant_term(tmp1398[i]) + constant_term(tmp1400[i])
#             TaylorSeries.zero!(tmp1403[i])
#             (tmp1403[i]).coeffs[1] = constant_term(wi[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1490[i])
#             (tmp1490[i]).coeffs[1] = zero(constant_term(wi[i]))
#             TaylorSeries.zero!(v2[i])
#             (v2[i]).coeffs[1] = constant_term(tmp1401[i]) + constant_term(tmp1403[i])
#         end
#     TaylorSeries.zero!(tmp1406)
#     tmp1406.coeffs[1] = constant_term(q[4]) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp1491)
#     tmp1491.coeffs[1] = zero(constant_term(q[4]))
#     TaylorSeries.zero!(tmp1408)
#     tmp1408.coeffs[1] = constant_term(q[5]) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp1492)
#     tmp1492.coeffs[1] = zero(constant_term(q[5]))
#     TaylorSeries.zero!(tmp1409)
#     tmp1409.coeffs[1] = constant_term(tmp1406) + constant_term(tmp1408)
#     TaylorSeries.zero!(tmp1411)
#     tmp1411.coeffs[1] = constant_term(q[6]) ^ float(constant_term(2))
#     TaylorSeries.zero!(tmp1493)
#     tmp1493.coeffs[1] = zero(constant_term(q[6]))
#     TaylorSeries.zero!(v2[N])
#     (v2[N]).coeffs[1] = constant_term(tmp1409) + constant_term(tmp1411)
#     for i = 1:Nm1
#         TaylorSeries.zero!(temp_004[i])
#         (temp_004[i]).coeffs[1] = constant_term(newtonian1b_Potential[i]) + constant_term(newtonianNb_Potential[N])
#         TaylorSeries.zero!(newtonianNb_Potential[N])
#         (newtonianNb_Potential[N]).coeffs[1] = identity(constant_term(temp_004[i]))
#         if UJ_interaction[i]
#             TaylorSeries.zero!(tmp1414[i])
#             (tmp1414[i]).coeffs[1] = constant_term(μ[i]) * constant_term(F_J2_x[i])
#             TaylorSeries.zero!(temp_accX_i[i])
#             (temp_accX_i[i]).coeffs[1] = constant_term(accX) - constant_term(tmp1414[i])
#             TaylorSeries.zero!(accX)
#             accX.coeffs[1] = identity(constant_term(temp_accX_i[i]))
#             TaylorSeries.zero!(tmp1416[i])
#             (tmp1416[i]).coeffs[1] = constant_term(μ[i]) * constant_term(F_J2_y[i])
#             TaylorSeries.zero!(temp_accY_i[i])
#             (temp_accY_i[i]).coeffs[1] = constant_term(accY) - constant_term(tmp1416[i])
#             TaylorSeries.zero!(accY)
#             accY.coeffs[1] = identity(constant_term(temp_accY_i[i]))
#             TaylorSeries.zero!(tmp1418[i])
#             (tmp1418[i]).coeffs[1] = constant_term(μ[i]) * constant_term(F_J2_z[i])
#             TaylorSeries.zero!(temp_accZ_i[i])
#             (temp_accZ_i[i]).coeffs[1] = constant_term(accZ) - constant_term(tmp1418[i])
#             TaylorSeries.zero!(accZ)
#             accZ.coeffs[1] = identity(constant_term(temp_accZ_i[i]))
#         end
#     end
#     TaylorSeries.zero!(_4ϕj[N])
#     (_4ϕj[N]).coeffs[1] = constant_term(4) * constant_term(newtonianNb_Potential[N])
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1034 =# Threads.@threads for i = 1:10
#             TaylorSeries.zero!(ϕi_plus_4ϕj[i])
#             (ϕi_plus_4ϕj[i]).coeffs[1] = constant_term(newtonianNb_Potential_t[i]) + constant_term(_4ϕj[N])
#             TaylorSeries.zero!(tmp1424[i])
#             (tmp1424[i]).coeffs[1] = constant_term(2) * constant_term(v2[i])
#             TaylorSeries.zero!(tmp1426[i])
#             (tmp1426[i]).coeffs[1] = constant_term(4) * constant_term(vi_dot_vj[i])
#             TaylorSeries.zero!(tmp1427[i])
#             (tmp1427[i]).coeffs[1] = constant_term(tmp1424[i]) - constant_term(tmp1426[i])
#             TaylorSeries.zero!(sj2_plus_2si2_minus_4vivj[i])
#             (sj2_plus_2si2_minus_4vivj[i]).coeffs[1] = constant_term(tmp1427[i]) + constant_term(v2[N])
#             TaylorSeries.zero!(ϕs_and_vs[i])
#             (ϕs_and_vs[i]).coeffs[1] = constant_term(sj2_plus_2si2_minus_4vivj[i]) - constant_term(ϕi_plus_4ϕj[i])
#             TaylorSeries.zero!(Xij_t_Ui[i])
#             (Xij_t_Ui[i]).coeffs[1] = constant_term(X[i]) * constant_term(ui[i])
#             TaylorSeries.zero!(Yij_t_Vi[i])
#             (Yij_t_Vi[i]).coeffs[1] = constant_term(Y[i]) * constant_term(vi[i])
#             TaylorSeries.zero!(Zij_t_Wi[i])
#             (Zij_t_Wi[i]).coeffs[1] = constant_term(Z[i]) * constant_term(wi[i])
#             TaylorSeries.zero!(tmp1433[i])
#             (tmp1433[i]).coeffs[1] = constant_term(Xij_t_Ui[i]) + constant_term(Yij_t_Vi[i])
#             TaylorSeries.zero!(Rij_dot_Vi[i])
#             (Rij_dot_Vi[i]).coeffs[1] = constant_term(tmp1433[i]) + constant_term(Zij_t_Wi[i])
#             TaylorSeries.zero!(tmp1436[i])
#             (tmp1436[i]).coeffs[1] = constant_term(Rij_dot_Vi[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1494[i])
#             (tmp1494[i]).coeffs[1] = zero(constant_term(Rij_dot_Vi[i]))
#             TaylorSeries.zero!(pn1t7[i])
#             (pn1t7[i]).coeffs[1] = constant_term(tmp1436[i]) / constant_term(r_p2[i])
#             TaylorSeries.zero!(tmp1439[i])
#             (tmp1439[i]).coeffs[1] = constant_term(1.5) * constant_term(pn1t7[i])
#             TaylorSeries.zero!(pn1t2_7[i])
#             (pn1t2_7[i]).coeffs[1] = constant_term(ϕs_and_vs[i]) - constant_term(tmp1439[i])
#             TaylorSeries.zero!(pn1t1_7[i])
#             (pn1t1_7[i]).coeffs[1] = constant_term(c_p2) + constant_term(pn1t2_7[i])
#             TaylorSeries.zero!(pNX_t_X[i])
#             (pNX_t_X[i]).coeffs[1] = constant_term(acceph_t[3i - 2]) * constant_term(X[i])
#             TaylorSeries.zero!(pNY_t_Y[i])
#             (pNY_t_Y[i]).coeffs[1] = constant_term(acceph_t[3i - 1]) * constant_term(Y[i])
#             TaylorSeries.zero!(pNZ_t_Z[i])
#             (pNZ_t_Z[i]).coeffs[1] = constant_term(acceph_t[3i]) * constant_term(Z[i])
#             TaylorSeries.zero!(tmp1446[i])
#             (tmp1446[i]).coeffs[1] = constant_term(pNX_t_X[i]) + constant_term(pNY_t_Y[i])
#             TaylorSeries.zero!(tmp1447[i])
#             (tmp1447[i]).coeffs[1] = constant_term(tmp1446[i]) + constant_term(pNZ_t_Z[i])
#             TaylorSeries.zero!(tmp1448[i])
#             (tmp1448[i]).coeffs[1] = constant_term(0.5) * constant_term(tmp1447[i])
#             TaylorSeries.zero!(pn1[i])
#             (pn1[i]).coeffs[1] = constant_term(pn1t1_7[i]) + constant_term(tmp1448[i])
#             TaylorSeries.zero!(X_t_pn1[i])
#             (X_t_pn1[i]).coeffs[1] = constant_term(newton_acc_X[i]) * constant_term(pn1[i])
#             TaylorSeries.zero!(Y_t_pn1[i])
#             (Y_t_pn1[i]).coeffs[1] = constant_term(newton_acc_Y[i]) * constant_term(pn1[i])
#             TaylorSeries.zero!(Z_t_pn1[i])
#             (Z_t_pn1[i]).coeffs[1] = constant_term(newton_acc_Z[i]) * constant_term(pn1[i])
#             TaylorSeries.zero!(pNX_t_pn3[i])
#             (pNX_t_pn3[i]).coeffs[1] = constant_term(acceph_t[3i - 2]) * constant_term(pn3[i])
#             TaylorSeries.zero!(pNY_t_pn3[i])
#             (pNY_t_pn3[i]).coeffs[1] = constant_term(acceph_t[3i - 1]) * constant_term(pn3[i])
#             TaylorSeries.zero!(pNZ_t_pn3[i])
#             (pNZ_t_pn3[i]).coeffs[1] = constant_term(acceph_t[3i]) * constant_term(pn3[i])
#         end
#     for i = 1:10
#         TaylorSeries.zero!(tmp1456[i])
#         (tmp1456[i]).coeffs[1] = constant_term(U_t_pn2[i]) + constant_term(pNX_t_pn3[i])
#         TaylorSeries.zero!(termpnx[i])
#         (termpnx[i]).coeffs[1] = constant_term(X_t_pn1[i]) + constant_term(tmp1456[i])
#         TaylorSeries.zero!(sumpnx[i])
#         (sumpnx[i]).coeffs[1] = constant_term(pntempX) + constant_term(termpnx[i])
#         TaylorSeries.zero!(pntempX)
#         pntempX.coeffs[1] = identity(constant_term(sumpnx[i]))
#         TaylorSeries.zero!(tmp1459[i])
#         (tmp1459[i]).coeffs[1] = constant_term(V_t_pn2[i]) + constant_term(pNY_t_pn3[i])
#         TaylorSeries.zero!(termpny[i])
#         (termpny[i]).coeffs[1] = constant_term(Y_t_pn1[i]) + constant_term(tmp1459[i])
#         TaylorSeries.zero!(sumpny[i])
#         (sumpny[i]).coeffs[1] = constant_term(pntempY) + constant_term(termpny[i])
#         TaylorSeries.zero!(pntempY)
#         pntempY.coeffs[1] = identity(constant_term(sumpny[i]))
#         TaylorSeries.zero!(tmp1462[i])
#         (tmp1462[i]).coeffs[1] = constant_term(W_t_pn2[i]) + constant_term(pNZ_t_pn3[i])
#         TaylorSeries.zero!(termpnz[i])
#         (termpnz[i]).coeffs[1] = constant_term(Z_t_pn1[i]) + constant_term(tmp1462[i])
#         TaylorSeries.zero!(sumpnz[i])
#         (sumpnz[i]).coeffs[1] = constant_term(pntempZ) + constant_term(termpnz[i])
#         TaylorSeries.zero!(pntempZ)
#         pntempZ.coeffs[1] = identity(constant_term(sumpnz[i]))
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1087 =# Threads.@threads for i = 11:Nm1
#             TaylorSeries.zero!(X_t_pn1[i])
#             (X_t_pn1[i]).coeffs[1] = constant_term(c_p2) * constant_term(newton_acc_X[i])
#             TaylorSeries.zero!(Y_t_pn1[i])
#             (Y_t_pn1[i]).coeffs[1] = constant_term(c_p2) * constant_term(newton_acc_Y[i])
#             TaylorSeries.zero!(Z_t_pn1[i])
#             (Z_t_pn1[i]).coeffs[1] = constant_term(c_p2) * constant_term(newton_acc_Z[i])
#         end
#     for i = 11:Nm1
#         TaylorSeries.zero!(termpnx[i])
#         (termpnx[i]).coeffs[1] = identity(constant_term(X_t_pn1[i]))
#         TaylorSeries.zero!(sumpnx[i])
#         (sumpnx[i]).coeffs[1] = constant_term(pntempX) + constant_term(termpnx[i])
#         TaylorSeries.zero!(pntempX)
#         pntempX.coeffs[1] = identity(constant_term(sumpnx[i]))
#         TaylorSeries.zero!(termpny[i])
#         (termpny[i]).coeffs[1] = identity(constant_term(Y_t_pn1[i]))
#         TaylorSeries.zero!(sumpny[i])
#         (sumpny[i]).coeffs[1] = constant_term(pntempY) + constant_term(termpny[i])
#         TaylorSeries.zero!(pntempY)
#         pntempY.coeffs[1] = identity(constant_term(sumpny[i]))
#         TaylorSeries.zero!(termpnz[i])
#         (termpnz[i]).coeffs[1] = identity(constant_term(Z_t_pn1[i]))
#         TaylorSeries.zero!(sumpnz[i])
#         (sumpnz[i]).coeffs[1] = constant_term(pntempZ) + constant_term(termpnz[i])
#         TaylorSeries.zero!(pntempZ)
#         pntempZ.coeffs[1] = identity(constant_term(sumpnz[i]))
#     end
#     TaylorSeries.zero!(postNewtonX)
#     postNewtonX.coeffs[1] = constant_term(pntempX) * constant_term(c_m2)
#     TaylorSeries.zero!(postNewtonY)
#     postNewtonY.coeffs[1] = constant_term(pntempY) * constant_term(c_m2)
#     TaylorSeries.zero!(postNewtonZ)
#     postNewtonZ.coeffs[1] = constant_term(pntempZ) * constant_term(c_m2)
#     TaylorSeries.zero!(dq[4])
#     (dq[4]).coeffs[1] = constant_term(postNewtonX) + constant_term(accX)
#     TaylorSeries.zero!(dq[5])
#     (dq[5]).coeffs[1] = constant_term(postNewtonY) + constant_term(accY)
#     TaylorSeries.zero!(dq[6])
#     (dq[6]).coeffs[1] = constant_term(postNewtonZ) + constant_term(accZ)
#     for __idx = eachindex(q)
#         (q[__idx]).coeffs[2] = (dq[__idx]).coeffs[1]
#     end
#     for ord = 1:order - 1
#         ordnext = ord + 1
#         TaylorSeries.identity!(pntempX, zero_q_1, ord)
#         TaylorSeries.identity!(pntempY, zero_q_1, ord)
#         TaylorSeries.identity!(pntempZ, zero_q_1, ord)
#         TaylorSeries.identity!(accX, zero_q_1, ord)
#         TaylorSeries.identity!(accY, zero_q_1, ord)
#         TaylorSeries.identity!(accZ, zero_q_1, ord)
#         TaylorSeries.identity!(dq[1], q[4], ord)
#         TaylorSeries.identity!(dq[2], q[5], ord)
#         TaylorSeries.identity!(dq[3], q[6], ord)
#         TaylorSeries.identity!(newtonianNb_Potential[N], zero_q_1, ord)
#         TaylorSeries.mul!(_4dq[1], 4, dq[1], ord)
#         TaylorSeries.mul!(_4dq[2], 4, dq[2], ord)
#         TaylorSeries.mul!(_4dq[3], 4, dq[3], ord)
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:832 =# Threads.@threads for i = 1:Nm1
#                 TaylorSeries.identity!(ui[i], ss16asteph_t[3 * ((N - 1) + i) - 2], ord)
#                 TaylorSeries.identity!(vi[i], ss16asteph_t[3 * ((N - 1) + i) - 1], ord)
#                 TaylorSeries.identity!(wi[i], ss16asteph_t[3 * ((N - 1) + i)], ord)
#                 TaylorSeries.subst!(X[i], ss16asteph_t[3i - 2], q[1], ord)
#                 TaylorSeries.subst!(Y[i], ss16asteph_t[3i - 1], q[2], ord)
#                 TaylorSeries.subst!(Z[i], ss16asteph_t[3i], q[3], ord)
#                 TaylorSeries.subst!(U[i], ui[i], dq[1], ord)
#                 TaylorSeries.subst!(V[i], vi[i], dq[2], ord)
#                 TaylorSeries.subst!(W[i], wi[i], dq[3], ord)
#                 TaylorSeries.mul!(tmp1272[i], -3, ui[i], ord)
#                 TaylorSeries.add!(_4U_m_3X[i], tmp1272[i], _4dq[1], ord)
#                 TaylorSeries.mul!(tmp1275[i], -3, vi[i], ord)
#                 TaylorSeries.add!(_4V_m_3Y[i], tmp1275[i], _4dq[2], ord)
#                 TaylorSeries.mul!(tmp1278[i], -3, wi[i], ord)
#                 TaylorSeries.add!(_4W_m_3Z[i], tmp1278[i], _4dq[3], ord)
#                 TaylorSeries.mul!(pn2x[i], X[i], _4U_m_3X[i], ord)
#                 TaylorSeries.mul!(pn2y[i], Y[i], _4V_m_3Y[i], ord)
#                 TaylorSeries.mul!(pn2z[i], Z[i], _4W_m_3Z[i], ord)
#                 TaylorSeries.mul!(UU[i], ui[i], dq[1], ord)
#                 TaylorSeries.mul!(VV[i], vi[i], dq[2], ord)
#                 TaylorSeries.mul!(WW[i], wi[i], dq[3], ord)
#                 TaylorSeries.add!(tmp1286[i], UU[i], VV[i], ord)
#                 TaylorSeries.add!(vi_dot_vj[i], tmp1286[i], WW[i], ord)
#                 TaylorSeries.pow!(tmp1289[i], X[i], tmp1477[i], 2, ord)
#                 TaylorSeries.pow!(tmp1291[i], Y[i], tmp1478[i], 2, ord)
#                 TaylorSeries.add!(tmp1292[i], tmp1289[i], tmp1291[i], ord)
#                 TaylorSeries.pow!(tmp1294[i], Z[i], tmp1479[i], 2, ord)
#                 TaylorSeries.add!(r_p2[i], tmp1292[i], tmp1294[i], ord)
#                 TaylorSeries.sqrt!(r_p1d2[i], r_p2[i], ord)
#                 TaylorSeries.pow!(r_p3d2[i], r_p2[i], tmp1480[i], 1.5, ord)
#                 TaylorSeries.pow!(r_p7d2[i], r_p2[i], tmp1481[i], 3.5, ord)
#                 TaylorSeries.div!(newtonianCoeff[i], μ[i], r_p3d2[i], ord)
#                 TaylorSeries.add!(tmp1302[i], pn2x[i], pn2y[i], ord)
#                 TaylorSeries.add!(tmp1303[i], tmp1302[i], pn2z[i], ord)
#                 TaylorSeries.mul!(pn2[i], newtonianCoeff[i], tmp1303[i], ord)
#                 TaylorSeries.mul!(newton_acc_X[i], X[i], newtonianCoeff[i], ord)
#                 TaylorSeries.mul!(newton_acc_Y[i], Y[i], newtonianCoeff[i], ord)
#                 TaylorSeries.mul!(newton_acc_Z[i], Z[i], newtonianCoeff[i], ord)
#                 TaylorSeries.div!(newtonian1b_Potential[i], μ[i], r_p1d2[i], ord)
#                 TaylorSeries.mul!(pn3[i], 3.5, newtonian1b_Potential[i], ord)
#                 TaylorSeries.mul!(U_t_pn2[i], pn2[i], U[i], ord)
#                 TaylorSeries.mul!(V_t_pn2[i], pn2[i], V[i], ord)
#                 TaylorSeries.mul!(W_t_pn2[i], pn2[i], W[i], ord)
#                 if UJ_interaction[i]
#                     TaylorSeries.subst!(tmp1314[i], X[i], ord)
#                     TaylorSeries.mul!(t31[i], tmp1314[i], M_[1, 3, i], ord)
#                     TaylorSeries.subst!(tmp1316[i], Y[i], ord)
#                     TaylorSeries.mul!(t32[i], tmp1316[i], M_[2, 3, i], ord)
#                     TaylorSeries.subst!(tmp1318[i], Z[i], ord)
#                     TaylorSeries.mul!(t33[i], tmp1318[i], M_[3, 3, i], ord)
#                     TaylorSeries.add!(tmp1320[i], t31[i], t32[i], ord)
#                     TaylorSeries.add!(r_sin_ϕ[i], tmp1320[i], t33[i], ord)
#                     TaylorSeries.div!(sin_ϕ[i], r_sin_ϕ[i], r_p1d2[i], ord)
#                     TaylorSeries.asin!(ϕ[i], sin_ϕ[i], tmp1482[i], ord)
#                     TaylorSeries.sincos!(tmp1483[i], cos_ϕ[i], ϕ[i], ord)
#                     TaylorSeries.pow!(sin2_ϕ[i], sin_ϕ[i], tmp1484[i], 2, ord)
#                     TaylorSeries.pow!(sin3_ϕ[i], sin_ϕ[i], tmp1485[i], 3, ord)
#                     TaylorSeries.mul!(tmp1330[i], 1.5, sin2_ϕ[i], ord)
#                     TaylorSeries.subst!(P_2_sin_ϕ[i], tmp1330[i], 0.5, ord)
#                     TaylorSeries.mul!(∂P_2_sin_ϕ[i], 3, sin_ϕ[i], ord)
#                     TaylorSeries.mul!(tmp1336[i], -1.5, sin_ϕ[i], ord)
#                     TaylorSeries.mul!(tmp1338[i], 2.5, sin3_ϕ[i], ord)
#                     TaylorSeries.add!(P_3_sin_ϕ[i], tmp1336[i], tmp1338[i], ord)
#                     TaylorSeries.mul!(tmp1342[i], 7.5, sin2_ϕ[i], ord)
#                     TaylorSeries.add!(∂P_3_sin_ϕ[i], -1.5, tmp1342[i], ord)
#                     TaylorSeries.pow!(tmp1345[i], r_p2[i], tmp1486[i], 2, ord)
#                     TaylorSeries.div!(tmp1346[i], Λ2[i], tmp1345[i], ord)
#                     TaylorSeries.subst!(Λ2j_div_r4[i], tmp1346[i], ord)
#                     TaylorSeries.pow!(tmp1349[i], r_p1d2[i], tmp1487[i], 5, ord)
#                     TaylorSeries.div!(tmp1350[i], Λ3[i], tmp1349[i], ord)
#                     TaylorSeries.subst!(Λ3j_div_r5[i], tmp1350[i], ord)
#                     TaylorSeries.subst!(tmp1352[i], cos_ϕ[i], ord)
#                     TaylorSeries.mul!(m_c_ϕ_∂P_2[i], tmp1352[i], ∂P_2_sin_ϕ[i], ord)
#                     TaylorSeries.subst!(tmp1354[i], cos_ϕ[i], ord)
#                     TaylorSeries.mul!(m_c_ϕ_∂P_3[i], tmp1354[i], ∂P_3_sin_ϕ[i], ord)
#                     TaylorSeries.mul!(tmp1357[i], Λ2j_div_r4[i], 3, ord)
#                     TaylorSeries.mul!(F_J2_ξ[i], tmp1357[i], P_2_sin_ϕ[i], ord)
#                     TaylorSeries.mul!(F_J2_ζ[i], Λ2j_div_r4[i], m_c_ϕ_∂P_2[i], ord)
#                     TaylorSeries.mul!(tmp1361[i], Λ3j_div_r5[i], 4, ord)
#                     TaylorSeries.mul!(F_J3_ξ[i], tmp1361[i], P_3_sin_ϕ[i], ord)
#                     TaylorSeries.mul!(F_J3_ζ[i], Λ3j_div_r5[i], m_c_ϕ_∂P_3[i], ord)
#                     TaylorSeries.identity!(F_J_ξ[i], F_J2_ξ[i], ord)
#                     TaylorSeries.identity!(F_J_ζ[i], F_J2_ζ[i], ord)
#                     TaylorSeries.subst!(tmp1364[i], X[i], ord)
#                     TaylorSeries.div!(ξx[i], tmp1364[i], r_p1d2[i], ord)
#                     TaylorSeries.subst!(tmp1366[i], Y[i], ord)
#                     TaylorSeries.div!(ξy[i], tmp1366[i], r_p1d2[i], ord)
#                     TaylorSeries.subst!(tmp1368[i], Z[i], ord)
#                     TaylorSeries.div!(ξz[i], tmp1368[i], r_p1d2[i], ord)
#                     TaylorSeries.mul!(ηx1[i], M_[2, 3, i], ξz[i], ord)
#                     TaylorSeries.mul!(ηy1[i], M_[3, 3, i], ξx[i], ord)
#                     TaylorSeries.mul!(ηz1[i], M_[1, 3, i], ξy[i], ord)
#                     TaylorSeries.mul!(ηx2[i], M_[3, 3, i], ξy[i], ord)
#                     TaylorSeries.mul!(ηy2[i], M_[1, 3, i], ξz[i], ord)
#                     TaylorSeries.mul!(ηz2[i], M_[2, 3, i], ξx[i], ord)
#                     TaylorSeries.subst!(ηx[i], ηx1[i], ηx2[i], ord)
#                     TaylorSeries.subst!(ηy[i], ηy1[i], ηy2[i], ord)
#                     TaylorSeries.subst!(ηz[i], ηz1[i], ηz2[i], ord)
#                     TaylorSeries.mul!(ζx1[i], ξy[i], ηz[i], ord)
#                     TaylorSeries.mul!(ζy1[i], ξz[i], ηx[i], ord)
#                     TaylorSeries.mul!(ζz1[i], ξx[i], ηy[i], ord)
#                     TaylorSeries.mul!(ζx2[i], ξz[i], ηy[i], ord)
#                     TaylorSeries.mul!(ζy2[i], ξx[i], ηz[i], ord)
#                     TaylorSeries.mul!(ζz2[i], ξy[i], ηx[i], ord)
#                     TaylorSeries.subst!(ζx[i], ζx1[i], ζx2[i], ord)
#                     TaylorSeries.subst!(ζy[i], ζy1[i], ζy2[i], ord)
#                     TaylorSeries.subst!(ζz[i], ζz1[i], ζz2[i], ord)
#                     TaylorSeries.mul!(F_J2_x1[i], F_J_ξ[i], ξx[i], ord)
#                     TaylorSeries.mul!(F_J2_y1[i], F_J_ξ[i], ξy[i], ord)
#                     TaylorSeries.mul!(F_J2_z1[i], F_J_ξ[i], ξz[i], ord)
#                     TaylorSeries.mul!(F_J2_x2[i], F_J_ζ[i], ζx[i], ord)
#                     TaylorSeries.mul!(F_J2_y2[i], F_J_ζ[i], ζy[i], ord)
#                     TaylorSeries.mul!(F_J2_z2[i], F_J_ζ[i], ζz[i], ord)
#                     TaylorSeries.add!(F_J2_x[i], F_J2_x1[i], F_J2_x2[i], ord)
#                     TaylorSeries.add!(F_J2_y[i], F_J2_y1[i], F_J2_y2[i], ord)
#                     TaylorSeries.add!(F_J2_z[i], F_J2_z1[i], F_J2_z2[i], ord)
#                 end
#                 TaylorSeries.pow!(tmp1398[i], ui[i], tmp1488[i], 2, ord)
#                 TaylorSeries.pow!(tmp1400[i], vi[i], tmp1489[i], 2, ord)
#                 TaylorSeries.add!(tmp1401[i], tmp1398[i], tmp1400[i], ord)
#                 TaylorSeries.pow!(tmp1403[i], wi[i], tmp1490[i], 2, ord)
#                 TaylorSeries.add!(v2[i], tmp1401[i], tmp1403[i], ord)
#             end
#         TaylorSeries.pow!(tmp1406, q[4], tmp1491, 2, ord)
#         TaylorSeries.pow!(tmp1408, q[5], tmp1492, 2, ord)
#         TaylorSeries.add!(tmp1409, tmp1406, tmp1408, ord)
#         TaylorSeries.pow!(tmp1411, q[6], tmp1493, 2, ord)
#         TaylorSeries.add!(v2[N], tmp1409, tmp1411, ord)
#         for i = 1:Nm1
#             TaylorSeries.add!(temp_004[i], newtonian1b_Potential[i], newtonianNb_Potential[N], ord)
#             TaylorSeries.identity!(newtonianNb_Potential[N], temp_004[i], ord)
#             if UJ_interaction[i]
#                 TaylorSeries.mul!(tmp1414[i], μ[i], F_J2_x[i], ord)
#                 TaylorSeries.subst!(temp_accX_i[i], accX, tmp1414[i], ord)
#                 TaylorSeries.identity!(accX, temp_accX_i[i], ord)
#                 TaylorSeries.mul!(tmp1416[i], μ[i], F_J2_y[i], ord)
#                 TaylorSeries.subst!(temp_accY_i[i], accY, tmp1416[i], ord)
#                 TaylorSeries.identity!(accY, temp_accY_i[i], ord)
#                 TaylorSeries.mul!(tmp1418[i], μ[i], F_J2_z[i], ord)
#                 TaylorSeries.subst!(temp_accZ_i[i], accZ, tmp1418[i], ord)
#                 TaylorSeries.identity!(accZ, temp_accZ_i[i], ord)
#             end
#         end
#         TaylorSeries.mul!(_4ϕj[N], 4, newtonianNb_Potential[N], ord)
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1034 =# Threads.@threads for i = 1:10
#                 TaylorSeries.add!(ϕi_plus_4ϕj[i], newtonianNb_Potential_t[i], _4ϕj[N], ord)
#                 TaylorSeries.mul!(tmp1424[i], 2, v2[i], ord)
#                 TaylorSeries.mul!(tmp1426[i], 4, vi_dot_vj[i], ord)
#                 TaylorSeries.subst!(tmp1427[i], tmp1424[i], tmp1426[i], ord)
#                 TaylorSeries.add!(sj2_plus_2si2_minus_4vivj[i], tmp1427[i], v2[N], ord)
#                 TaylorSeries.subst!(ϕs_and_vs[i], sj2_plus_2si2_minus_4vivj[i], ϕi_plus_4ϕj[i], ord)
#                 TaylorSeries.mul!(Xij_t_Ui[i], X[i], ui[i], ord)
#                 TaylorSeries.mul!(Yij_t_Vi[i], Y[i], vi[i], ord)
#                 TaylorSeries.mul!(Zij_t_Wi[i], Z[i], wi[i], ord)
#                 TaylorSeries.add!(tmp1433[i], Xij_t_Ui[i], Yij_t_Vi[i], ord)
#                 TaylorSeries.add!(Rij_dot_Vi[i], tmp1433[i], Zij_t_Wi[i], ord)
#                 TaylorSeries.pow!(tmp1436[i], Rij_dot_Vi[i], tmp1494[i], 2, ord)
#                 TaylorSeries.div!(pn1t7[i], tmp1436[i], r_p2[i], ord)
#                 TaylorSeries.mul!(tmp1439[i], 1.5, pn1t7[i], ord)
#                 TaylorSeries.subst!(pn1t2_7[i], ϕs_and_vs[i], tmp1439[i], ord)
#                 TaylorSeries.add!(pn1t1_7[i], c_p2, pn1t2_7[i], ord)
#                 TaylorSeries.mul!(pNX_t_X[i], acceph_t[3i - 2], X[i], ord)
#                 TaylorSeries.mul!(pNY_t_Y[i], acceph_t[3i - 1], Y[i], ord)
#                 TaylorSeries.mul!(pNZ_t_Z[i], acceph_t[3i], Z[i], ord)
#                 TaylorSeries.add!(tmp1446[i], pNX_t_X[i], pNY_t_Y[i], ord)
#                 TaylorSeries.add!(tmp1447[i], tmp1446[i], pNZ_t_Z[i], ord)
#                 TaylorSeries.mul!(tmp1448[i], 0.5, tmp1447[i], ord)
#                 TaylorSeries.add!(pn1[i], pn1t1_7[i], tmp1448[i], ord)
#                 TaylorSeries.mul!(X_t_pn1[i], newton_acc_X[i], pn1[i], ord)
#                 TaylorSeries.mul!(Y_t_pn1[i], newton_acc_Y[i], pn1[i], ord)
#                 TaylorSeries.mul!(Z_t_pn1[i], newton_acc_Z[i], pn1[i], ord)
#                 TaylorSeries.mul!(pNX_t_pn3[i], acceph_t[3i - 2], pn3[i], ord)
#                 TaylorSeries.mul!(pNY_t_pn3[i], acceph_t[3i - 1], pn3[i], ord)
#                 TaylorSeries.mul!(pNZ_t_pn3[i], acceph_t[3i], pn3[i], ord)
#             end
#         for i = 1:10
#             TaylorSeries.add!(tmp1456[i], U_t_pn2[i], pNX_t_pn3[i], ord)
#             TaylorSeries.add!(termpnx[i], X_t_pn1[i], tmp1456[i], ord)
#             TaylorSeries.add!(sumpnx[i], pntempX, termpnx[i], ord)
#             TaylorSeries.identity!(pntempX, sumpnx[i], ord)
#             TaylorSeries.add!(tmp1459[i], V_t_pn2[i], pNY_t_pn3[i], ord)
#             TaylorSeries.add!(termpny[i], Y_t_pn1[i], tmp1459[i], ord)
#             TaylorSeries.add!(sumpny[i], pntempY, termpny[i], ord)
#             TaylorSeries.identity!(pntempY, sumpny[i], ord)
#             TaylorSeries.add!(tmp1462[i], W_t_pn2[i], pNZ_t_pn3[i], ord)
#             TaylorSeries.add!(termpnz[i], Z_t_pn1[i], tmp1462[i], ord)
#             TaylorSeries.add!(sumpnz[i], pntempZ, termpnz[i], ord)
#             TaylorSeries.identity!(pntempZ, sumpnz[i], ord)
#         end
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1087 =# Threads.@threads for i = 11:Nm1
#                 TaylorSeries.mul!(X_t_pn1[i], c_p2, newton_acc_X[i], ord)
#                 TaylorSeries.mul!(Y_t_pn1[i], c_p2, newton_acc_Y[i], ord)
#                 TaylorSeries.mul!(Z_t_pn1[i], c_p2, newton_acc_Z[i], ord)
#             end
#         for i = 11:Nm1
#             TaylorSeries.identity!(termpnx[i], X_t_pn1[i], ord)
#             TaylorSeries.add!(sumpnx[i], pntempX, termpnx[i], ord)
#             TaylorSeries.identity!(pntempX, sumpnx[i], ord)
#             TaylorSeries.identity!(termpny[i], Y_t_pn1[i], ord)
#             TaylorSeries.add!(sumpny[i], pntempY, termpny[i], ord)
#             TaylorSeries.identity!(pntempY, sumpny[i], ord)
#             TaylorSeries.identity!(termpnz[i], Z_t_pn1[i], ord)
#             TaylorSeries.add!(sumpnz[i], pntempZ, termpnz[i], ord)
#             TaylorSeries.identity!(pntempZ, sumpnz[i], ord)
#         end
#         TaylorSeries.mul!(postNewtonX, pntempX, c_m2, ord)
#         TaylorSeries.mul!(postNewtonY, pntempY, c_m2, ord)
#         TaylorSeries.mul!(postNewtonZ, pntempZ, c_m2, ord)
#         TaylorSeries.add!(dq[4], postNewtonX, accX, ord)
#         TaylorSeries.add!(dq[5], postNewtonY, accY, ord)
#         TaylorSeries.add!(dq[6], postNewtonZ, accZ, ord)
#         for __idx = eachindex(q)
#             TaylorIntegration.diffeq!(q[__idx], dq[__idx], ordnext)
#         end
#     end
#     return nothing
# end

# # TaylorIntegration._allocate_jetcoeffs! method for src/propagation/asteroid_dynamical_models.jl: newtonian!
# function TaylorIntegration._allocate_jetcoeffs!(::Val{newtonian!}, t::Taylor1{_T}, q::AbstractArray{Taylor1{_S}, _N}, dq::AbstractArray{Taylor1{_S}, _N}, params) where {_T <: Real, _S <: Number, _N}
#     order = t.order
#     local jd0 = params.jd0
#     local dsj2k = t + (jd0 - JD_J2000)
#     local ss16asteph_t = evaleph(params.sseph, dsj2k, q[1])
#     local S = eltype(q)
#     local N = 10
#     local Nm1 = N - 1
#     local μ = params.μ[1:10]
#     local zero_q_1 = auxzero(q[1])
#     X = Array{S}(undef, N)
#     Y = Array{S}(undef, N)
#     Z = Array{S}(undef, N)
#     r_p2 = Array{S}(undef, N)
#     r_p3d2 = Array{S}(undef, N)
#     newtonianCoeff = Array{S}(undef, N)
#     newton_acc_X = Array{S}(undef, N)
#     newton_acc_Y = Array{S}(undef, N)
#     newton_acc_Z = Array{S}(undef, N)
#     temp_accX_i = Array{S}(undef, N)
#     temp_accY_i = Array{S}(undef, N)
#     temp_accZ_i = Array{S}(undef, N)
#     accX = Taylor1(identity(constant_term(zero_q_1)), order)
#     accY = Taylor1(identity(constant_term(zero_q_1)), order)
#     accZ = Taylor1(identity(constant_term(zero_q_1)), order)
#     dq[1] = Taylor1(identity(constant_term(q[4])), order)
#     dq[2] = Taylor1(identity(constant_term(q[5])), order)
#     dq[3] = Taylor1(identity(constant_term(q[6])), order)
#     tmp1547 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp1547)
#         tmp1547[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1563 = Array{Taylor1{_S}}(undef, size(X))
#     for i = eachindex(tmp1563)
#         tmp1563[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1549 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp1549)
#         tmp1549[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1564 = Array{Taylor1{_S}}(undef, size(Y))
#     for i = eachindex(tmp1564)
#         tmp1564[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1550 = Array{Taylor1{_S}}(undef, size(tmp1547))
#     for i = eachindex(tmp1550)
#         tmp1550[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1552 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp1552)
#         tmp1552[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1565 = Array{Taylor1{_S}}(undef, size(Z))
#     for i = eachindex(tmp1565)
#         tmp1565[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     tmp1566 = Array{Taylor1{_S}}(undef, size(r_p2))
#     for i = eachindex(tmp1566)
#         tmp1566[i] = Taylor1(zero(constant_term(q[1])), order)
#     end
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1184 =# Threads.@threads for i = 1:Nm1
#             X[i] = Taylor1(constant_term(ss16asteph_t[3i - 2]) - constant_term(q[1]), order)
#             Y[i] = Taylor1(constant_term(ss16asteph_t[3i - 1]) - constant_term(q[2]), order)
#             Z[i] = Taylor1(constant_term(ss16asteph_t[3i]) - constant_term(q[3]), order)
#             tmp1547[i] = Taylor1(constant_term(X[i]) ^ float(constant_term(2)), order)
#             tmp1563[i] = Taylor1(zero(constant_term(X[i])), order)
#             tmp1549[i] = Taylor1(constant_term(Y[i]) ^ float(constant_term(2)), order)
#             tmp1564[i] = Taylor1(zero(constant_term(Y[i])), order)
#             tmp1550[i] = Taylor1(constant_term(tmp1547[i]) + constant_term(tmp1549[i]), order)
#             tmp1552[i] = Taylor1(constant_term(Z[i]) ^ float(constant_term(2)), order)
#             tmp1565[i] = Taylor1(zero(constant_term(Z[i])), order)
#             r_p2[i] = Taylor1(constant_term(tmp1550[i]) + constant_term(tmp1552[i]), order)
#             r_p3d2[i] = Taylor1(constant_term(r_p2[i]) ^ float(constant_term(1.5)), order)
#             tmp1566[i] = Taylor1(zero(constant_term(r_p2[i])), order)
#             newtonianCoeff[i] = Taylor1(constant_term(μ[i]) / constant_term(r_p3d2[i]), order)
#             newton_acc_X[i] = Taylor1(constant_term(X[i]) * constant_term(newtonianCoeff[i]), order)
#             newton_acc_Y[i] = Taylor1(constant_term(Y[i]) * constant_term(newtonianCoeff[i]), order)
#             newton_acc_Z[i] = Taylor1(constant_term(Z[i]) * constant_term(newtonianCoeff[i]), order)
#         end
#     for i = 1:Nm1
#         temp_accX_i[i] = Taylor1(constant_term(accX) + constant_term(newton_acc_X[i]), order)
#         accX = Taylor1(identity(constant_term(temp_accX_i[i])), order)
#         temp_accY_i[i] = Taylor1(constant_term(accY) + constant_term(newton_acc_Y[i]), order)
#         accY = Taylor1(identity(constant_term(temp_accY_i[i])), order)
#         temp_accZ_i[i] = Taylor1(constant_term(accZ) + constant_term(newton_acc_Z[i]), order)
#         accZ = Taylor1(identity(constant_term(temp_accZ_i[i])), order)
#     end
#     dq[4] = Taylor1(identity(constant_term(accX)), order)
#     dq[5] = Taylor1(identity(constant_term(accY)), order)
#     dq[6] = Taylor1(identity(constant_term(accZ)), order)
#     return TaylorIntegration.RetAlloc{Taylor1{_S}}([accX, accY, accZ], [X, Y, Z, r_p2, r_p3d2, newtonianCoeff, newton_acc_X, newton_acc_Y, newton_acc_Z, temp_accX_i, temp_accY_i, temp_accZ_i, tmp1547, tmp1563, tmp1549, tmp1564, tmp1550, tmp1552, tmp1565, tmp1566], [Array{Taylor1{_S}, 2}(undef, 0, 0)], [Array{Taylor1{_S}, 3}(undef, 0, 0, 0)], [Array{Taylor1{_S}, 4}(undef, 0, 0, 0, 0)])
# end

# # TaylorIntegration.jetcoeffs! method for src/propagation/asteroid_dynamical_models.jl: newtonian!
# function TaylorIntegration.jetcoeffs!(::Val{newtonian!}, t::Taylor1{_T}, q::AbstractArray{Taylor1{_S}, _N}, dq::AbstractArray{Taylor1{_S}, _N}, params, __ralloc::TaylorIntegration.RetAlloc{Taylor1{_S}}) where {_T <: Real, _S <: Number, _N}
#     order = t.order
#     accX = __ralloc.v0[1]
#     accY = __ralloc.v0[2]
#     accZ = __ralloc.v0[3]
#     X = __ralloc.v1[1]
#     Y = __ralloc.v1[2]
#     Z = __ralloc.v1[3]
#     r_p2 = __ralloc.v1[4]
#     r_p3d2 = __ralloc.v1[5]
#     newtonianCoeff = __ralloc.v1[6]
#     newton_acc_X = __ralloc.v1[7]
#     newton_acc_Y = __ralloc.v1[8]
#     newton_acc_Z = __ralloc.v1[9]
#     temp_accX_i = __ralloc.v1[10]
#     temp_accY_i = __ralloc.v1[11]
#     temp_accZ_i = __ralloc.v1[12]
#     tmp1547 = __ralloc.v1[13]
#     tmp1563 = __ralloc.v1[14]
#     tmp1549 = __ralloc.v1[15]
#     tmp1564 = __ralloc.v1[16]
#     tmp1550 = __ralloc.v1[17]
#     tmp1552 = __ralloc.v1[18]
#     tmp1565 = __ralloc.v1[19]
#     tmp1566 = __ralloc.v1[20]
#     local jd0 = params.jd0
#     local dsj2k = t + (jd0 - JD_J2000)
#     local ss16asteph_t = evaleph(params.sseph, dsj2k, q[1])
#     local S = eltype(q)
#     local N = 10
#     local Nm1 = N - 1
#     local μ = params.μ[1:10]
#     local zero_q_1 = auxzero(q[1])
#     TaylorSeries.zero!(accX)
#     accX.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accY)
#     accY.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(accZ)
#     accZ.coeffs[1] = identity(constant_term(zero_q_1))
#     TaylorSeries.zero!(dq[1])
#     (dq[1]).coeffs[1] = identity(constant_term(q[4]))
#     TaylorSeries.zero!(dq[2])
#     (dq[2]).coeffs[1] = identity(constant_term(q[5]))
#     TaylorSeries.zero!(dq[3])
#     (dq[3]).coeffs[1] = identity(constant_term(q[6]))
#     #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1184 =# Threads.@threads for i = 1:Nm1
#             TaylorSeries.zero!(X[i])
#             (X[i]).coeffs[1] = constant_term(ss16asteph_t[3i - 2]) - constant_term(q[1])
#             TaylorSeries.zero!(Y[i])
#             (Y[i]).coeffs[1] = constant_term(ss16asteph_t[3i - 1]) - constant_term(q[2])
#             TaylorSeries.zero!(Z[i])
#             (Z[i]).coeffs[1] = constant_term(ss16asteph_t[3i]) - constant_term(q[3])
#             TaylorSeries.zero!(tmp1547[i])
#             (tmp1547[i]).coeffs[1] = constant_term(X[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1563[i])
#             (tmp1563[i]).coeffs[1] = zero(constant_term(X[i]))
#             TaylorSeries.zero!(tmp1549[i])
#             (tmp1549[i]).coeffs[1] = constant_term(Y[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1564[i])
#             (tmp1564[i]).coeffs[1] = zero(constant_term(Y[i]))
#             TaylorSeries.zero!(tmp1550[i])
#             (tmp1550[i]).coeffs[1] = constant_term(tmp1547[i]) + constant_term(tmp1549[i])
#             TaylorSeries.zero!(tmp1552[i])
#             (tmp1552[i]).coeffs[1] = constant_term(Z[i]) ^ float(constant_term(2))
#             TaylorSeries.zero!(tmp1565[i])
#             (tmp1565[i]).coeffs[1] = zero(constant_term(Z[i]))
#             TaylorSeries.zero!(r_p2[i])
#             (r_p2[i]).coeffs[1] = constant_term(tmp1550[i]) + constant_term(tmp1552[i])
#             TaylorSeries.zero!(r_p3d2[i])
#             (r_p3d2[i]).coeffs[1] = constant_term(r_p2[i]) ^ float(constant_term(1.5))
#             TaylorSeries.zero!(tmp1566[i])
#             (tmp1566[i]).coeffs[1] = zero(constant_term(r_p2[i]))
#             TaylorSeries.zero!(newtonianCoeff[i])
#             (newtonianCoeff[i]).coeffs[1] = constant_term(μ[i]) / constant_term(r_p3d2[i])
#             TaylorSeries.zero!(newton_acc_X[i])
#             (newton_acc_X[i]).coeffs[1] = constant_term(X[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newton_acc_Y[i])
#             (newton_acc_Y[i]).coeffs[1] = constant_term(Y[i]) * constant_term(newtonianCoeff[i])
#             TaylorSeries.zero!(newton_acc_Z[i])
#             (newton_acc_Z[i]).coeffs[1] = constant_term(Z[i]) * constant_term(newtonianCoeff[i])
#         end
#     for i = 1:Nm1
#         TaylorSeries.zero!(temp_accX_i[i])
#         (temp_accX_i[i]).coeffs[1] = constant_term(accX) + constant_term(newton_acc_X[i])
#         TaylorSeries.zero!(accX)
#         accX.coeffs[1] = identity(constant_term(temp_accX_i[i]))
#         TaylorSeries.zero!(temp_accY_i[i])
#         (temp_accY_i[i]).coeffs[1] = constant_term(accY) + constant_term(newton_acc_Y[i])
#         TaylorSeries.zero!(accY)
#         accY.coeffs[1] = identity(constant_term(temp_accY_i[i]))
#         TaylorSeries.zero!(temp_accZ_i[i])
#         (temp_accZ_i[i]).coeffs[1] = constant_term(accZ) + constant_term(newton_acc_Z[i])
#         TaylorSeries.zero!(accZ)
#         accZ.coeffs[1] = identity(constant_term(temp_accZ_i[i]))
#     end
#     TaylorSeries.zero!(dq[4])
#     (dq[4]).coeffs[1] = identity(constant_term(accX))
#     TaylorSeries.zero!(dq[5])
#     (dq[5]).coeffs[1] = identity(constant_term(accY))
#     TaylorSeries.zero!(dq[6])
#     (dq[6]).coeffs[1] = identity(constant_term(accZ))
#     for __idx = eachindex(q)
#         (q[__idx]).coeffs[2] = (dq[__idx]).coeffs[1]
#     end
#     for ord = 1:order - 1
#         ordnext = ord + 1
#         TaylorSeries.identity!(accX, zero_q_1, ord)
#         TaylorSeries.identity!(accY, zero_q_1, ord)
#         TaylorSeries.identity!(accZ, zero_q_1, ord)
#         TaylorSeries.identity!(dq[1], q[4], ord)
#         TaylorSeries.identity!(dq[2], q[5], ord)
#         TaylorSeries.identity!(dq[3], q[6], ord)
#         #= /Users/Jorge/projects/NEOs/update_jetcoeffs.jl:1184 =# Threads.@threads for i = 1:Nm1
#                 TaylorSeries.subst!(X[i], ss16asteph_t[3i - 2], q[1], ord)
#                 TaylorSeries.subst!(Y[i], ss16asteph_t[3i - 1], q[2], ord)
#                 TaylorSeries.subst!(Z[i], ss16asteph_t[3i], q[3], ord)
#                 TaylorSeries.pow!(tmp1547[i], X[i], tmp1563[i], 2, ord)
#                 TaylorSeries.pow!(tmp1549[i], Y[i], tmp1564[i], 2, ord)
#                 TaylorSeries.add!(tmp1550[i], tmp1547[i], tmp1549[i], ord)
#                 TaylorSeries.pow!(tmp1552[i], Z[i], tmp1565[i], 2, ord)
#                 TaylorSeries.add!(r_p2[i], tmp1550[i], tmp1552[i], ord)
#                 TaylorSeries.pow!(r_p3d2[i], r_p2[i], tmp1566[i], 1.5, ord)
#                 TaylorSeries.div!(newtonianCoeff[i], μ[i], r_p3d2[i], ord)
#                 TaylorSeries.mul!(newton_acc_X[i], X[i], newtonianCoeff[i], ord)
#                 TaylorSeries.mul!(newton_acc_Y[i], Y[i], newtonianCoeff[i], ord)
#                 TaylorSeries.mul!(newton_acc_Z[i], Z[i], newtonianCoeff[i], ord)
#             end
#         for i = 1:Nm1
#             TaylorSeries.add!(temp_accX_i[i], accX, newton_acc_X[i], ord)
#             TaylorSeries.identity!(accX, temp_accX_i[i], ord)
#             TaylorSeries.add!(temp_accY_i[i], accY, newton_acc_Y[i], ord)
#             TaylorSeries.identity!(accY, temp_accY_i[i], ord)
#             TaylorSeries.add!(temp_accZ_i[i], accZ, newton_acc_Z[i], ord)
#             TaylorSeries.identity!(accZ, temp_accZ_i[i], ord)
#         end
#         TaylorSeries.identity!(dq[4], accX, ord)
#         TaylorSeries.identity!(dq[5], accY, ord)
#         TaylorSeries.identity!(dq[6], accZ, ord)
#         for __idx = eachindex(q)
#             TaylorIntegration.diffeq!(q[__idx], dq[__idx], ordnext)
#         end
#     end
#     return nothing
# end